...
class GlookoAccountFingerprintTest {
  ...
  @Test
  fun reportSubjectUsesASeparateOpaqueScope() {
    val subject = "example person|apr 29, 1985"
    val account = GlookoAccountFingerprint.encode(installationKeyA, subject)
    val report =
      GlookoAccountFingerprint.encodeReportSubject(installationKeyA, subject)

    assertTrue(report.matches(Regex("^rs1_[0-9a-f]{64}$")))
    assertNotEquals(account.removePrefix("af1_"), report.removePrefix("rs1_"))
    assertFalse(report.contains("example"))
  }
}
