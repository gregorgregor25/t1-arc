...
- Choose the **phone APK**: its filename starts with `T1-Arc-v` and ends with
  `.apk`. The version number between them changes with each release.
- Files with **Wear** or a face name are optional watch downloads. Files ending
  `.sha256` or `.build.json` are verification details, not apps.
...
3. Tap the **phone APK** described above and allow the download.
...
2. Download the newer **phone APK** from the latest release, using the same
   filename rule as above.
...
After restoring, reconnect your original source accounts and grant Android
permissions again. Check the restored history before retiring the old phone.
Older backups may need another additive restore after reconnection to bring
back history that was cleared while setting up the account. Repeating a restore
adds missing records without replacing newer ones.

Tarv1s keeps conversations from earlier connections in **Conversations →
Earlier connections and reviews** when they cannot be safely associated with
the current accounts. You can read them, but they are not used in new answers.
Restoring is not an unrestricted merge of two active conversation histories.
Keep the backup password separately: there is no password-recovery service.
