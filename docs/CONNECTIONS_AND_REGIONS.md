...
## Import delays and first imports

**Glooko supplies delayed history, not live pump status.** A successful check
can still return old records. Today’s insulin total may be incomplete until
the source uploads and a later import arrives. Health Connect and Hevy have
their own upstream delays and check schedules. See [data freshness](DATA_FRESHNESS.md).

A first Glooko export can take several minutes. Progress distinguishes
requesting, checking and saving; provider waiting time is variable. Keep the
app open for the initial import and avoid repeatedly starting it. If it fails,
use the specific retry or sign-in action shown on the source page.
