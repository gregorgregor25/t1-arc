...
- Choose the phone APK: its name starts `T1-Arc-v` and ends `.apk`. The middle
  is the release version. See [which download to choose](GETTING_STARTED.md#before-installing).
...
## Readings or imported totals are behind

Compare the measurement time with the provider’s official app. A successful
check does not prove new data arrived. Follow [data freshness and recovery](DATA_FRESHNESS.md)
for the source involved. Avoid repeated sign-ins or imports while a request is
still running.

## A Glooko import appears stuck

A large first export can take several minutes. The app shows whether it is
requesting, checking or saving records; waiting on Glooko can take a variable
amount of time. Keep T1 Arc open for the first import and avoid repeatedly
starting it. A successful import can still contain delayed records. See
[how current each source is](DATA_FRESHNESS.md).
