...
data is labelled. No AI connection is required.
...
the logging menu. Select the sensor model and actual start time, including a
past time. The form shows that model's expected warm-up duration; Other / not
sure can keep it unknown or use a duration you enter. No extra journal entry is
required. A gap overlapping a recorded warm-up can be described as consistent
with it, without claiming the cause is proven. See [sensor models](SENSOR_WARMUP.md).
...
The conversation list includes **Earlier conversations** for recovered or archived
...
Glooko and other imported records can arrive later than the original event.
Check the record time as well as the last successful refresh. See
[how current each source is](DATA_FRESHNESS.md) before interpreting a partial day.
...
lock-screen display, glucose alerts and home-screen widget. Watch setup is under
**Settings > Watch & watch faces**; see [Watch setup](WATCH_SETUP.md).
