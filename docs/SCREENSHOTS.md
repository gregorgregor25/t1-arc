# T1 Arc screenshots

Every image on this page was captured on Android 17 with temporary synthetic
demo data or an empty app state. No personal health records or real provider
accounts are shown. The examples show both light and dark themes.

## First run

![T1 Arc onboarding tour](screenshots/onboarding.png)

The first-run tour explains what the app can and cannot do before asking for
connections or permissions.

## Today and History

| Today | Seven-day history |
| --- | --- |
| ![Today screen with synthetic glucose, insulin and food data](screenshots/today.png) | ![Seven-day glucose and insulin chart using synthetic data](screenshots/history.png) |

Today brings the current glucose reading, daily range, insulin, food and health
context together. History lets you move between day, 3-day, 7-day and 30-day
views and inspect the underlying records.

## Tarv1s and Insights

Open Tarv1s from the bottom navigation. The switch at the top opens Insights.

| Insights | Tarv1s |
| --- | --- |
| ![Weekly comparison in Insights](screenshots/insights.png) | ![Tarv1s question screen](screenshots/tarvis.png) |

Insights describes measurable changes and keeps the evidence visible. Tarv1s
supports questions about the records on the phone. It is not a dosing tool or a
replacement for a clinician.

## Food logging

![Synthetic food draft showing compact tools and an expanded portion control](screenshots/food-logging.png)

Food logging includes recent foods, favourites, saved foods and meals, regional
reference search, Open Food Facts product search, barcode scanning, quick carbs
and copying from another day.

The example shows a selected food before saving, with its source and portion
controls visible. **More options** opens separate barcode, carbohydrate-entry,
custom-food and optional-catalogue pages; returning keeps the meal draft.
The illustrated quantity is an example, not a dietary recommendation. See the
[food guide](FOOD_LOGGING.md) for the complete flow and release boundary.

## Sources and regional settings

| Source connections | Regional settings |
| --- | --- |
| ![Source connection settings](screenshots/sources.png) | ![Country, locale and glucose-unit settings](screenshots/regional-settings.png) |

The regional profile controls safe defaults for formatting, food search and new
connections. Each cloud source can still use its own supported account region.

## Display, backup and health

| Display | Backup and privacy | Health Connect |
| --- | --- | --- |
| ![Display settings](screenshots/display-settings.png) | ![Backup and privacy settings](screenshots/backup-privacy.png) | ![Health Connect empty state](screenshots/health.png) |

The health screenshot deliberately shows the honest empty state. Imported
records depend on Android Health Connect permissions and data available on the
device.

## Glucose colours

![Saved green in-range colour with an mg/dL preview](screenshots/glucose-colours.png)

In **Display and alerts**, open **Phone and lock screen**, then **Glucose ranges
and colours**. Tap a range, choose a colour and press **Save**. The preview uses
illustrative values within the selected boundaries, in the selected glucose
unit. Colours are accompanied by range labels, not used on their own.
