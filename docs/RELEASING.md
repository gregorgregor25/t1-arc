...
Preparing source, documentation or a candidate APK does not publish a release.
Only an authorised maintainer should sign and publish official builds, after
reviewing the exact source, artifacts and verification results.
...
Version codes must always increase. Advance the phone code by at least **two**
for each release: the matching companion uses the phone code plus one. This
keeps both packages increasing. Never reuse a public version number for a
different APK.
