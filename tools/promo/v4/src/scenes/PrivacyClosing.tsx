...
export function PrivacyClosing({ listingStill = false }: { listingStill?: boolean }) {
  const frame = useCurrentFrame();
  const { width, height } = useVideoConfig();
  const portrait = height > width;
  return <AbsoluteFill style={{ background: '#080a10', fontFamily }}>
    <PhoneStage clip={footage.today} mode="close" approachFrames={540} phoneScale={portrait ? .86 : listingStill ? .98 : 1} phoneOffsetY={portrait ? -.6 : 0} />
    <Interactive.Div name="T1 Arc privacy wordmark" style={{
      position: 'absolute', left: portrait ? 160 : 240, top: portrait ? 220 : listingStill ? 280 : 350,
    ...
    <Interactive.Div name="Open source and local health history" style={{
      position: 'absolute', left: portrait ? 160 : 240, top: portrait ? 525 : listingStill ? 650 : 730,
    ...
    }}>Open source.<br />{listingStill ? 'Your data stays on your phone.' : 'Health history stored on your phone.'}<br />{listingStill ? 'Your data. Your control.' : 'We don’t collect your health data.'}</Interactive.Div>
    <Interactive.Div name="Direct OpenAI qualifier, visible throughout" style={{
      position: 'absolute', left: portrait ? 200 : 240, top: portrait ? 980 : listingStill ? 1170 : 1250,
    ...
    {!listingStill && <Interactive.Div name="AI narration credit" style={{
