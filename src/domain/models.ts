...
export interface HealthContextBase {
  /** Read-only presentation of a food record excluded from the chosen diary. */
  excludedFoodSource?: boolean;
  observation?: import('./personalObservations').PersonalObservation;
  id: string;
  start: number;
  end?: number;
  sourceId: string;
...
export interface ContextNoteEvent extends HealthContextBase {
  ...
  /** Model chosen by the user; absent on older sensor-change notes. */
  sensorModelId?: string;
  /** Saved expected duration, not a measured warm-up state. */
