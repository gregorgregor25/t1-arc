...
import { sensorModel, validSensorWarmupMinutes } from './sensorModels';

export function sensorWarmupWindow(event: HealthContextEvent) {
  if (!isRecordedSensorStart(event) || !validSensorWarmupMinutes(event.sensorWarmupMinutes)) return undefined;
  return { start: event.start, end: event.start + event.sensorWarmupMinutes * 60_000 };
}

export function sensorChangePresentation(event: ContextNoteEvent, now: number) {
  const window = sensorWarmupWindow(event);
  const label = sensorModel(event.sensorModelId)?.label;
  return {
    title: window && now >= window.start && now < window.end
      ? `${label && event.sensorModelId !== 'other' ? label : 'Sensor'} warming up` : 'Waiting for readings',
    expectedEnd: window?.end,
    warmingUp: Boolean(window && now >= window.start && now < window.end),
  };
}
