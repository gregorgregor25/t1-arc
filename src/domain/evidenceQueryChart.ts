...
function evidenceFormatter(options: Intl.DateTimeFormatOptions) {
...
export function formatEvidenceQueryTick(
  ...
  const isOneLocalDay =
    evidenceFormatter({ dateStyle: 'short' }).format(range.start) ===
    evidenceFormatter({ dateStyle: 'short' }).format(range.end);
  if (isOneLocalDay)
    return evidenceFormatter({
      hour: '2-digit',
      minute: '2-digit',
  ...
  return range.end - range.start <= 2 * 24 * 60 * 60_000
    ...
    : evidenceFormatter({ day: 'numeric', month: 'short' }).format(timestamp);
}
...
export function buildEvidenceQueryAccessibilitySummary(
  ...
  const windowCopy = visualization.windows.map((window) => {
    ...
    const sampling = window.sampling
      ? `This chart shows ${count(window.sampling.displayedPointCount)} of ${count(window.sampling.sourceSampleCount)} readings to stay clear; the calculation and Records view still use all ${count(window.sampling.sourceRecordCount)} saved records.`
      : '';
    ...
    if (visualization.kind === 'event-timeline-v1') {
      ...
      return `${base} ${sampling} ${count(events.length)} sustained ${visualization.eventKind} event${
        events.length === 1 ? '' : 's'
      } started in this period${
        extreme === null
          ? '.'
          : `; the most extreme value was ${regionalGlucose(extreme)}.`
      }${
        continuing
          ? ` ${count(continuing)} event${continuing === 1 ? '' : 's'} may continue beyond the requested period.`
          : ''
      }${gaps ? ` The trace contains ${count(gaps)} sensor gap${gaps === 1 ? '' : 's'}.` : ''}`;
    }
    const mean =
      window.meanMmolL === null
        ? 'The average is unavailable.'
        : `The observed arithmetic mean is ${regionalGlucose(window.meanMmolL)}.`;
    return `${base} ${sampling} ${mean}${
      gaps
        ? ` The trace contains ${count(gaps)} sensor gap${gaps === 1 ? '' : 's'}; lines stop across missing time.`
        : ''
    }`;
  });
  const comparison =
    visualization.windows.length > 1
      ? `This view compares ${count(visualization.windows.length)} exact periods.`
      : '';
  return [
    visualization.title,
    comparison,
    ...windowCopy,
    `The shaded target range is ${regionalGlucose(
      visualization.targetRange.minimum,
    )} to ${regionalGlucose(visualization.targetRange.maximum)}.`,
...
export function isEvidenceQueryVisualizationReference(
  ...
  const storedWindows = value.windows as Record<string, unknown>[];
  if (
