...
export function presentInsulinRangeSummary(
  ...
  const sourceBasis = summary.sourceCoversEveryDay
    ? 'Source-reported daily total'
    : 'Source totals plus detailed delivery records';
  const basis = summary.extraInjectionUnits ? `${sourceBasis} plus ${formatRegionalFixedNumber(summary.extraInjectionUnits, regional.locale, 1)} U of separately recorded extra injections` : sourceBasis;
