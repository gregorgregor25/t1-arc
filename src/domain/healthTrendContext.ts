...
export function nutritionCoverageNeedsReview(
  ...
  return (
    coverage.incompleteLogging === true ||
...
export function summarizeHealthTrendContext(
  ...
  const incomplete = inRange.some(event => event.observation?.kind === 'food-incomplete');
  if (incomplete) {
    for (const nutrient of [carbs, energy, protein, fat, fibre, sugars, saturatedFat]) {
      nutrient.coverage = { ...nutrient.coverage, incompleteLogging: true } as typeof nutrient.coverage;
    }
  }
  return {
    ...(incomplete ? { foodLoggingIncomplete: true } : {}),
