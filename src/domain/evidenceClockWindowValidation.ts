...
export function isEvidenceClockWindowVisualizationReference(
  ...
  const windowsValid = storedWindows.every((window) => {
    ...
    const segmentRecordIds = new Set(
      (window.segments as Array<{ points: Record<string, unknown>[] }>).flatMap(
        (segment) =>
  ...
  if (isRecord(value.valueDomain)) {
    ...
    const plottedValues = [
      ...(storedWindows as Array<{ points: Array<{ mmolL: number }> }>).flatMap(
        (window) => window.points.map(({ mmolL }) => mmolL),
      ),
      ...(value.aggregatePoints as Array<{ mmolL: number }>).map(
        ({ mmolL }) => mmolL,
      ),
