...
export function summarizeInsulinByDay(
  ...
  for (let index = 0; index < 370; index += 1) {
    ...
    if (end > start) {
      ...
      const extraDoses = reported ? boluses.filter(delivery => isExtraPumpInjection(delivery) && delivery.timestamp >= start && delivery.timestamp < end && !matchesDailyTotalProvenance(delivery, reported)) : [];
      const extraUnits = extraDoses.reduce((sum, delivery) => sum + delivery.units, 0);
      const extraBasal = extraDoses.filter(isManualBasalInjection).reduce((sum, delivery) => sum + delivery.units, 0);
      const basalUnits = (reported?.basalUnits ?? calculated.basalUnits) + extraBasal;
      const bolusUnits = (reported?.bolusUnits ?? calculated.bolusUnits) + extraUnits - extraBasal;
      const totalUnits = reported ? reported.totalUnits + extraUnits : basalUnits + bolusUnits;
      summaries.push({
        dateKey,
        start,
        end,
        ...
        bolusUnits,
        totalUnits,
        ...(extraUnits > 0 ? { extraInjectionUnits: extraUnits } : {}),
...
export function summarizeInsulinRange(
  ...
  return {
    ...
    sourceTotals,
    ...(days.some(day => day.extraInjectionUnits) ? { extraInjectionUnits: rounded(days.reduce((sum, day) => sum + (day.extraInjectionUnits ?? 0), 0)) } : {}),
    sourceAsOf,
