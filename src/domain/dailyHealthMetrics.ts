...
export interface DailyMetricRecord {
  id: string;
  /** Shared parent identity for components emitted from one measurement. */
...
export interface DailyHealthMetrics {
  ...
  nutritionCarbohydrateGrams?: number;
  nutritionEnergyKcal?: number;
  nutritionProteinGrams?: number;
  nutritionFatGrams?: number;
  nutritionFibreGrams?: number;
  nutritionSugarsGrams?: number;
  nutritionSaturatedFatGrams?: number;
  nutritionNutrientCoverage?: MealNutrientCoverage;
...
export function nutritionMetricsFromContextSummary(
  summary: HealthTrendContextSummary,
): Partial<DailyHealthMetrics> {
  return {
    nutritionMealCount: summary.mealCount || undefined,
    nutritionCarbohydrateGrams: summary.mealCarbsGrams,
    nutritionEnergyKcal: summary.mealEnergyKcal,
    nutritionProteinGrams: summary.mealProteinGrams,
    nutritionFatGrams: summary.mealFatGrams,
    nutritionFibreGrams: summary.mealFibreGrams,
    nutritionSugarsGrams: summary.mealSugarsGrams,
    nutritionSaturatedFatGrams: summary.mealSaturatedFatGrams,
    nutritionNutrientCoverage: summary.mealCount
      ? summary.mealNutrientCoverage
      : undefined,
    nutritionSourceLabels: summary.nutritionSourceLabels.length
      ? summary.nutritionSourceLabels
      : undefined,
    nutritionPossibleDuplicatePairs:
      summary.nutritionPossibleDuplicatePairs || undefined,
  };
}
...
function overlapsRange(record: DailyMetricRecord, range: TimeRange) {
  return record.end <= record.start
    ? record.start >= range.start && record.start < range.end
    : record.start < range.end && record.end > range.start;
}
...
function latestBloodPressurePair(records: DailyMetricRecord[]) {
  const groups = new Map<
    string,
    {
      systolic: DailyMetricRecord[];
      diastolic: DailyMetricRecord[];
    }
  >();
  for (const record of records) {
    if (
      record.kind !== 'blood_pressure_systolic' &&
      record.kind !== 'blood_pressure_diastolic'
    ) {
      continue;
    }
    const measurementId = record.measurementId?.trim();
    const measurementKey = measurementId
      ? `identity:${measurementId}`
      : `timestamp:${record.start}:${record.end}`;
    const key = `${record.sourcePackage}:${record.start}:${record.end}:${measurementKey}`;
    const group = groups.get(key) ?? { systolic: [], diastolic: [] };
    if (record.kind === 'blood_pressure_systolic') {
      group.systolic.push(record);
    } else {
      group.diastolic.push(record);
    }
  ...
  const pairs = [...groups.values()]
    .filter(
      (group) => group.systolic.length === 1 && group.diastolic.length === 1,
    )
    .map((group) => ({
      systolic: group.systolic[0]!,
      diastolic: group.diastolic[0]!,
    }))
    .sort((left, right) => right.systolic.start - left.systolic.start);
  const latest = pairs[0];
  if (!latest) return undefined;
  if (pairs[1]?.systolic.start === latest.systolic.start) {
    return undefined;
  }
  return latest;
}
...
export function aggregateDailyHealthMetrics(
  ...
  const bloodPressure = latestBloodPressurePair(vitals);
  ...
  return {
    ...
    bloodPressureSystolic: bloodPressure?.systolic.value,
    bloodPressureDiastolic: bloodPressure?.diastolic.value,
