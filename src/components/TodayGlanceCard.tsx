...
export function TodayGlanceCard({
  ...
  if (meals.length > 0) {
    metrics.push({
      ...
      detail: [
        `${formatRegionalNumber(meals.length, regional.locale, { maximumFractionDigits: 0 })} ${meals.some(meal => meal.kind === 'meal' && meal.purpose === 'hypo-treatment') ? (meals.length === 1 ? 'food / treatment entry' : 'food / treatment entries') : meals.length === 1 ? "meal" : "meals"} logged`,
        carbohydrates === undefined
