...
import { healthConnectReadError, presentHealthConnectState } from './healthConnect/presentation';
import { getHealthConnectSyncProgress, subscribeHealthConnectSyncProgress } from '@/data/healthConnect/healthConnectSyncProgress';
...
export function HealthConnectCard({
  ...
  const wasSyncing = useRef(Boolean(syncProgress));
  ...
  useEffect(() => {
    if (wasSyncing.current && !syncProgress) {
      void load().catch(() => {
        if (mounted.current) setError('The import finished. Reopen Health Connect settings to check its result.');
      });
    }
    wasSyncing.current = Boolean(syncProgress);
  }, [load, syncProgress]);
  ...
  const busy = operation !== undefined || Boolean(syncProgress);
  ...
  const healthConnectPresentation = presentHealthConnectState({
    ...
    syncing: Boolean(syncProgress),
    syncCategory: syncProgress?.category ? healthCategoryLabel(syncProgress.category) : undefined,
  });
        ...
        <View
          style={[
            styles.connectionSummary,
            {
              ...
              borderColor:
                !syncProgress && !importsPaused && syncFailures.length
                  ? `${colors.warning}66`
                  : colors.border,
              borderRadius: radius.md,
            },
        ...
        >
          <Ionicons
            ...
            color={
              importsPaused
                ? colors.textTertiary
                : !syncProgress && syncFailures.length
                  ? colors.warning
                  : colors.accent
            }
            name={
          ...
          <View style={styles.connectionSummaryCopy}>
            ...
            >
              {syncProgress
                ? `${regionalCount(syncProgress.recordsChecked)} records checked. A large first import can take several minutes. You can keep using T1 Arc.`
                : importsPaused
        ...
        <>
          ...
          {overview?.sources.length ? (
            <View style={styles.sourcesBlock}>
              ...
              {HEALTH_CONNECT_CATEGORIES.map((category) => {
                if (category.id === 'nutrition') return null; // The Food totals chooser below owns this preference.
        ...
        >
          ...
          <View style={styles.syncCategoryList}>
            {selectedSyncStates.map(({ category, state }) => {
              const importing = syncProgress?.categories.includes(category);
              const checked = syncProgress?.completedCategories.includes(category);
              const failed = !importing && Boolean(state?.lastErrorMessage);
              return (
                <View key={category} style={styles.syncCategoryRow}>
                  ...
                  >
                    {importing
                      ? checked ? 'Checked' : syncProgress?.category === category ? 'Importing…' : 'Queued for this import'
                      : failed
                      ? healthConnectReadError(state?.lastErrorMessage)
                      : state?.lastSuccessAt
          ...
          >
            {operation === 'connecting'
              ...
              : operation === 'enabling_history'
                ...
                : operation === 'syncing' || syncProgress
