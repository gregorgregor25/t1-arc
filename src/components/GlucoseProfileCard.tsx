...
function evidenceForProfile(data: TimelineData): EvidenceReference {
  ...
  return {
    ...
    description: `${readings.length} glucose readings grouped into local half-hour clock windows. The line is the median, the solid band is the middle 50%, and whiskers show the 10th to 90th percentile`,
    range: data.range,
  ...
  return (
    <>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable
            ...
            onPress={() => onInspect(evidenceForProfile(data))}
            style={({ pressed }) => [
              styles.inspect,
              {
      ...
      {!enoughCoverage ? (
        <View style={styles.empty}>
          <EmptyState
            ...
            detail={`T1 Arc needs readings from at least ${profile.minimumDaysPerBin} separate days in enough half-hour windows. ${profile.representedBins} of ${profile.bins.length} windows are ready.`}
          />
        </View>
      ) : (
        <>
          <View
            accessibilityLabel={`Aggregate glucose profile across ${profile.expectedDays} days. ${profile.representedBins} of ${profile.bins.length} half-hour windows represented.`}
            onLayout={onLayout}
      ...
      <Text style={[styles.footnote, { color: colors.textTertiary }]}>
        Built from local clock time across {profile.expectedDays} days. This is
      ...
      {!expanded ? (
        <FullscreenChartModal
          detail={`Typical day from ${profile.expectedDays} local days`}
