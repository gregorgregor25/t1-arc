...
function thresholdDraft(
  ...
  return {
    veryLowMax: settings.veryLowMax.toFixed(1),
    targetMin: settings.targetMin.toFixed(1),
    targetMax: settings.targetMax.toFixed(1),
    veryHighMin: settings.veryHighMin.toFixed(1),
  };
}
...
function parseDraft(
  ...
  return {
    veryLowMax: Number.parseFloat(thresholds.veryLowMax),
    targetMin: Number.parseFloat(thresholds.targetMin),
    targetMax: Number.parseFloat(thresholds.targetMax),
    veryHighMin: Number.parseFloat(thresholds.veryHighMin),
    colors,
  };
}
...
export function GlucoseAppearanceSettingsScreen({
  ...
  useEffect(() => {
    if (!visible) return;
    setThresholds(thresholdDraft(settings));
  ...
  const draft = useMemo(
    () => parseDraft(thresholds, rangeColors),
    [rangeColors, thresholds],
  );
  ...
  const descriptions = useMemo<Record<GlucoseRange, string>>(() => {
    ...
    return {
      veryLow: `At or below ${low} mmol/L`,
      low: `Above ${low}, below ${targetMin}`,
      target: `${targetMin} to ${targetMax} mmol/L`,
      high: `Above ${targetMax}, below ${high}`,
      veryHigh: `At or above ${high} mmol/L`,
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Restore',
          onPress: () => {
            setThresholds(thresholdDraft(DEFAULT_GLUCOSE_APPEARANCE));
        ...
        >
          ...
          >
            ...
            <Text style={[styles.helper, { color: themeColors.textSecondary }]}>
              mmol/L · values must increase from very low to very high
            </Text>
            <View
              style={[
