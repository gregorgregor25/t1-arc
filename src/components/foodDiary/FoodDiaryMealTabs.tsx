...
const tabs: { value: FoodDiaryMealFilter; label: string }[] = [
  ...
  { value: 'hypo-treatment', label: 'Hypo treatments' },
];
...
export function FoodDiaryMealTabs({
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        {tabs.filter(tab => tab.value !== 'hypo-treatment' || summary['hypo-treatment']?.count).map((tab) => {
          ...
          const detail =
            tab.value === 'all'
              ? `${countLabel} ${summary['hypo-treatment'] ? 'food entries' : count === 1 ? 'meal' : 'meals'}`
              : count === 0
                ? 'No entries'
                : carbs === undefined
