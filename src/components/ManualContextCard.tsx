...
import { EMPTY_OBSERVATION_FIELDS, fieldsFromObservation, observationFromFields, PersonalObservationFields } from './PersonalObservationFields';
import { SensorWarmupFields } from './SensorWarmupFields';
...
const kindOptions: {
  ...
  { value: 'hypo-treatment', label: 'Hypo treatment', icon: 'heart-outline' },
  { value: 'food-incomplete', label: 'Food not fully logged', icon: 'restaurant-outline' },
  { value: 'lab-result', label: 'Lab result', icon: 'flask-outline' },
  { value: 'site-change', label: 'Pump / sensor site', icon: 'body-outline' },
];
...
export function ManualContextCard({
  ...
  const { saveManualContext, saveManualInsulin, ownerIdentity, demoMode } = useDataContext();
  ...
  const [insulinPurpose, setInsulinPurpose] = useState<'pen' | 'extra'>('pen');
  ...
  const [sensorModelId, setSensorModelId] = useState<string>();
  const [sensorWarmupMinutes, setSensorWarmupMinutes] = useState('');
  const [observationFields, setObservationFields] = useState({ ...EMPTY_OBSERVATION_FIELDS });
  const observations = usePersonalObservations('site-change');
  const recentSites = observations.events.flatMap(event => event.observation?.kind === 'site-change' && event.observation.device === (kind === 'sensor-start' ? 'sensor' : observationFields.device) && event.id !== editingEvent?.id ? [{ location: event.observation.location, date: formatDate(toDateKey(event.start)) }] : []).slice(0, 8);
  const sensorChoiceTouched = useRef(false);
  ...
  function resetForm(nextTimestamp: number, nextKind: ContextKind = "meal") {
    ...
    setMedicationUnit("");
    setInsulinUnits("");
    setInsulinPurpose('pen'); setInsulinConfirmed(false);
    ...
    setSensorModelId(undefined);
    setSensorWarmupMinutes('');
    setObservationFields({ ...EMPTY_OBSERVATION_FIELDS });
    sensorChoiceTouched.current = false;
    setError(undefined);
  }
  ...
  function populateForm(event: HealthContextEvent) {
    ...
    switch (draft.kind) {
      case 'hypo-treatment':
      case 'food-incomplete':
      case 'lab-result':
      case 'site-change':
        setObservationFields(fieldsFromObservation(draft.observation, regional.locale));
        break;
      case "meal":
      ...
      case "note":
        ...
        break;
      case "sensor-start":
        setObservationFields({ ...EMPTY_OBSERVATION_FIELDS, location: draft.location });
        setSensorSourceId(draft.glucoseSourceId);
        setSensorModelId(draft.sensorModelId ?? 'other');
        setSensorWarmupMinutes(draft.sensorWarmupMinutes === undefined ? '' : formatRegionalNumberInput(draft.sensorWarmupMinutes, regional.locale));
        break;
    }
  }
  ...
  function populateInsulinForm(delivery: BolusDelivery) {
    ...
    setInsulinPurpose(draft.additionalToPump ? 'extra' : 'pen');
  }
  ...
  useEffect(() => {
    let active = true;
    if (!open || kind !== 'sensor-start' || editing || demoMode) return;
    new SqliteHealthRecordStore().getLastSensorSelection(sensorSourceId).then((event) => {
      if (!active || sensorChoiceTouched.current || !event) return;
      setSensorModelId(event.sensorModelId);
      setSensorWarmupMinutes(event.sensorWarmupMinutes === undefined ? '' : formatRegionalNumberInput(event.sensorWarmupMinutes, regional.locale));
    }).catch(() => { /* The picker remains available if no saved choice can be read. */ });
    return () => { active = false; };
  }, [open, kind, editing, sensorSourceId, ownerIdentity, demoMode, regional.locale]);
  ...
  function buildDraft(): ManualContextDraft {
    ...
    switch (kind) {
      ...
      case 'lab-result':
      case 'site-change':
        return { kind, timestamp, observation: observationFromFields(kind, observationFields, regional.locale) };
      case "sensor-start":
        if (!sensorModelId) throw new Error('Choose your sensor model, or Other / not sure.');
        return { kind, timestamp, glucoseSourceId: sensorSourceId, sensorModelId, location: observationFields.location,
          sensorWarmupMinutes: sensorModelId === 'other' ? optionalNumberFromInput(sensorWarmupMinutes, 'warm-up time', regional.locale) : undefined };
      case "meal":
        return {
          kind,
          timestamp,
  ...
  async function save() {
    ...
    try {
      ...
      if (kind === "insulin") {
        if (!insulinConfirmed) throw new Error('Confirm this dose is not already included in your imported insulin records.');
        const draft: ManualInsulinDraft = {
          additionalToPump: insulinPurpose === 'extra',
          timestamp,
        ...
        >
          ...
          >
            ...
            >
              ...
              {kind === "insulin" ? (
                <>
                  <Text style={[styles.helper, { color: colors.textSecondary }]}>Pump doses arrive with your import. Use this form for a pen dose or an extra injection that is absent from those records.</Text>
                  <ChoiceChips label="Where did this dose come from?" value={insulinPurpose} onChange={value => { setInsulinPurpose(value); setInsulinConfirmed(false); }} options={[{ value: 'pen', label: 'Pen / syringe' }, { value: 'extra', label: 'Extra injection alongside pump' }]} />
                  ...
                  <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: insulinConfirmed }} onPress={() => setInsulinConfirmed(value => !value)} style={{ minHeight: 48, paddingVertical: 14 }}><Text style={{ color: colors.primary }}>{insulinConfirmed ? '✓ ' : '□ '}This dose is not already included in my imported records</Text></Pressable>
                </>
              ) : null}
              ...
              {kind === "sensor-start" ? (
                <>
                <SensorWarmupFields modelId={sensorModelId} customMinutes={sensorWarmupMinutes} timestamp={timestamp}
                  onModelChange={(id) => { sensorChoiceTouched.current = true; setSensorModelId(id); setSensorWarmupMinutes(''); }}
                  onMinutesChange={(value) => { sensorChoiceTouched.current = true; setSensorWarmupMinutes(value); }} />
                <BodyLocationPicker value={observationFields.location} onChange={location => setObservationFields(value => ({ ...value, location }))} recent={recentSites} />
                </>
              ) : null}

              {kind === 'hypo-treatment' || kind === 'food-incomplete' || kind === 'lab-result' || kind === 'site-change' ? (
                <PersonalObservationFields kind={kind} value={observationFields} onChange={setObservationFields} recentSites={recentSites} />
              ) : null}
