...
export function WearCompanionCard() {
  ...
  const refresh = useCallback(async () => {
    setChecking(true);
    try {
      setStatus(await DaymarkGlucoseDisplay.getWearStatusAsync());
    } catch {
      setStatus({
        supported: false,
  ...
  return (
    <SectionCard accessibilityLabel="T1 Arc Wear OS companion">
      <View style={styles.hero}>
        ...
        <View
          accessible
          accessibilityLabel="Meridian watch face preview, 6.8 millimoles per litre, steady and current"
          style={styles.face}
        >
          <View style={styles.faceArc} />
          <Text style={styles.faceTime}>10:08</Text>
          <Text style={styles.faceValue}>6.8 →</Text>
          <Text style={styles.faceUnit}>MMOL/L · CURRENT</Text>
        </View>
      </View>
