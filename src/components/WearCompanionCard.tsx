...
import WatchInstaller from '../../modules/t1arc-watch-installer';
import { WatchSetupWizard } from './watchSetup/WatchSetupWizard';
...
export function WearCompanionCard() {
  ...
  return (
    <SectionCard accessibilityLabel="T1 Arc Wear OS companion">
      {WatchInstaller?.available && <>
        <Pressable accessibilityRole="button" onPress={() => setSetupOpen(true)}
          style={[styles.refresh, { backgroundColor: colors.primary, borderColor: colors.primary, borderRadius: radius.md, marginTop: 0, marginBottom: 20 }]}>
          <Text style={[styles.refreshText, { color: colors.onPrimary }]}>{status?.companionAvailable ? 'Install or update watch companion' : 'Set up my watch'}</Text>
        </Pressable>
        {setupOpen && <WatchSetupWizard visible onClose={() => { setSetupOpen(false); setFaceRefreshKey(value => value + 1); void refresh(); }} />}
      </>}
      <View style={styles.hero}>
        ...
        <View
          accessible
          accessibilityLabel={`Example watch display, ${formatGlucoseAccessible(6.8, regional)}, not live glucose`}
          style={styles.face}
        >
      ...
      <WatchFaceChooser refreshKey={faceRefreshKey} />
