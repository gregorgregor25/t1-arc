...
import { evidenceRequestedPeriodLabel } from '@/domain/evidenceTimeZonePresentation';
...
function coverageLabel(window: EvidenceQueryChartWindow, locale: string) {
  if (window.coverageStatus === 'unavailable' || window.recordCount === 0) {
    return 'No sensor readings';
  }
  return `${formatRegionalFixedNumber(window.coveragePercent, locale, 1)}% coverage${
    ...
    <View style={styles.windowHeading}>
      ...
      >
        ...
        <Text style={[styles.coverageText, { color: colors.textSecondary }]}>
          {coverageLabel(window, regional.locale)}
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <View onLayout={onLayout}>
                  ...
                  <Svg accessibilityElementsHidden height={panelHeight} width={width}>
                    ...
                    {window.meanMmolL !== null ? (
                      <G>
                        ...
                        >
                          MEAN{' '}
                          {formatGlucose(window.meanMmolL, regional, {
                            withUnit: false,
                          })}
                        </SvgText>
                      </G>
                    ...
                    <SvgText
                      ...
                      y={bottom + 36}
                    >
                      {evidenceRequestedPeriodLabel(regional.timeZone)}
                    </SvgText>
                  </Svg>
                ) : null}
              </View>
      ...
      <View style={styles.noteRow}>
        ...
        <Text style={[styles.noteText, { color: colors.textSecondary }]}>
          Shaded band:{' '}
          {formatGlucose(visualization.targetRange.minimum, regional, {
            withUnit: false,
          })}{' '}
          to {formatGlucose(visualization.targetRange.maximum, regional)}.
    ...
    <View>
      <View style={styles.thresholdRow}>
        ...
        <Text style={[styles.thresholdText, { color: colors.textSecondary }]}>
          Observed-time range:{' '}
          {formatGlucose(visualization.lowerBoundMmolL, regional, {
            withUnit: false,
          })}{' '}
          to {formatGlucose(visualization.upperBoundMmolL, regional)}
        </Text>
      </View>
      {visualization.windows.map((window, windowIndex) => {
    ...
    <View onLayout={onLayout}>
      <View style={styles.thresholdRow}>
        ...
        <Text style={[styles.thresholdText, { color: colors.textSecondary }]}>
          ...
          {formatGlucose(visualization.thresholdMmolL, regional)}
        </Text>
      </View>
      {visualization.windows.map((window, windowIndex) => {
            ...
            {!window.points.length ? (
              ...
              >
                <Text style={[styles.eventCount, { color: eventColor }]}>
                  {formatRegionalNumber(events.length, regional.locale, {
                    maximumFractionDigits: 0,
                  })}{' '}
                  sustained {visualization.eventKind} event
                    ...
                    {events.map((event, index) => (
                      <View key={event.id} style={styles.eventRow}>
                        ...
                        <Text style={[styles.eventDetail, { color: colors.textSecondary }]}>
                          ...
                          · extreme{' '}
                          {formatGlucose(event.extremeMmolL, regional)}
                        </Text>
                      </View>
                    ))}
                  </View>
