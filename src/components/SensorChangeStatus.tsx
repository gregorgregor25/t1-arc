...
import { sensorChangePresentation, sensorWarmupWindow, type SensorChangeStatusValue } from '@/domain/sensorChange';
...
export function SensorChangeStatus({ sourceId, onRecord }: {
  ...
  const candidate = status?.event;
  const event = status?.waiting || (candidate && !candidate.sensorGlucoseSourceId && now < (sensorWarmupWindow(candidate)?.end ?? 0))
    ? candidate : undefined;
  const presentation = event ? sensorChangePresentation(event, now) : undefined;
  const displayTime = (time: number) => `${toDateKey(time) !== toDateKey(now) ? `${formatDate(toDateKey(time))} at ` : ''}${formatTime(time)}`;

  function editOrRemove() {
    if (!event) return;
    Alert.alert('Recorded sensor change', 'Edit the sensor model or start time, or remove this note if it was added by mistake.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Edit sensor change', onPress: () => setEditingEvent(event) },
  ...
  return (
    <View style={styles.container}>
      {event ? (
        <Pressable accessibilityRole="button" accessibilityLabel={`${presentation?.title}. Edit recorded sensor change`}
          ...
          <View style={styles.copy}>
            ...
            <Text style={[styles.detail, { color: colors.textSecondary }]}>
              {presentation?.expectedEnd === undefined ? `New sensor recorded at ${displayTime(event.start)} · warm-up time unknown`
                : `${presentation.warmingUp ? 'Readings expected from' : 'Expected warm-up ended at'} ${displayTime(presentation.expectedEnd)}`}
            </Text>
          </View>
