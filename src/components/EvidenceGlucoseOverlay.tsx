...
import { GLUCOSE_COLOR_PALETTE } from '@/domain/glucoseAppearance';
...
} from './evidenceGlucoseOverlayClock';
...
interface DayTrace {
  key: DateKey;
  label: string;
  readings: (GlucoseReading & { clockMinute: number })[];
}
...
export function EvidenceGlucoseOverlay({
  ...
  const traces = useMemo(() => {
    ...
    readings.forEach((reading) => {
      const key = evidenceOverlayDateKey(
        reading.timestamp,
        regional.timeZone,
      );
      ...
      entry.push({
        ...reading,
        clockMinute: evidenceOverlayClockMinute(
          reading.timestamp,
          regional.locale,
          regional.timeZone,
        ),
