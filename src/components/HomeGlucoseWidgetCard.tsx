...
export function HomeGlucoseWidgetCard() {
  ...
  const reload = useCallback(async () => {
    try {
      setStatus(await DaymarkGlucoseDisplay.getHomeWidgetStatusAsync());
    } catch {
      setStatus({
        supported: false,
  ...
  async function addWidget() {
    ...
    try {
      const opened = await DaymarkGlucoseDisplay.requestPinHomeWidgetAsync();
      setMessage(
        opened
    ...
    <SectionCard>
      ...
      >
        ...
        <View style={styles.previewValueRow}>
          <Text style={styles.previewValue}>6.8</Text>
          <Text style={styles.previewArrow}>→</Text>
          <Text style={styles.previewUnit}>mmol/L</Text>
        </View>
