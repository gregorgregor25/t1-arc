...
const AOD_RESTRICTED_SETTINGS_STEP =
  'In T1 Arc app info, tap the three-dot menu and choose “Allow restricted settings”. If the menu is missing after an ADB install, back up your T1 Arc data before removing that test build; Android exposes this control only after a fresh install from your browser or Files.';
...
export function GlucoseDisplayCard({
  ...
  function requestAod(desired: boolean) {
    ...
    Alert.alert(
      ...
      `Because this APK was installed outside Google Play, Android may ask you to trust its restricted settings. ${AOD_RESTRICTED_SETTINGS_STEP} Then return here and open Accessibility. T1 Arc cannot approve this for you.`,
      [
        { text: 'Not now', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            void T1ArcGlucoseDisplay.setAodDesiredAsync(true)
              .then(async (next) => {
                setStatus(next);
                await T1ArcGlucoseDisplay.openAppDetailsSettingsAsync();
              })
              .catch((error) =>
                setMessage(
                  error instanceof Error
    ...
    <SectionCard>
      ...
      {status?.aodDesired && !status.aodServiceEnabled ? (
        ...
        >
          ...
          >
            1. {AOD_RESTRICTED_SETTINGS_STEP}
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={() => {
          ...
          >
            {`2. Return here, open Accessibility, select “${AOD_ACCESSIBILITY_SERVICE_LABEL}”, and turn on the main service switch. Leave its Accessibility shortcut off—the floating T1 Arc button is not the always-on display.`}
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={() => {
