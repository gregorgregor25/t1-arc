...
const AOD_ACCESSIBILITY_SERVICE_LABEL = 'T1 Arc always-on glucose';
const AOD_RESTRICTED_SETTINGS_STEP =
  'If Android blocks Accessibility for a downloaded APK, open T1 Arc app info and look for “Allow restricted settings” in the three-dot menu. This extra step may not be needed for a Google Play install. Do not uninstall the app or clear its data to enable this feature.';

const AOD_POSITIONS: { label: string; value: AodPosition }[] = [
...
const AOD_SIZES: { label: string; value: AodSize }[] = [
...
export function GlucoseDisplayCard({
  ...
  const refreshStatus = useCallback(async () => {
    try {
      const next = await T1ArcGlucoseDisplay.getStatusAsync();
      setStatus(next);
  ...
  useEffect(() => {
    let active = true;
    void T1ArcGlucoseDisplay.getStatusAsync()
      .then((next) => {
  ...
  async function setEnabled(enabled: boolean) {
    ...
    try {
      if (enabled) {
        ...
        const next = await T1ArcGlucoseDisplay.enableAsync();
        setStatus(next);
        await updateGlucoseDisplayFromHistory();
        await refreshStatus();
        setMessage(
          next.lockScreenVisible
            ? 'Glucose at a glance is active, including the lock-screen visibility you previously chose.'
            : 'Glucose at a glance is active. The value stays hidden until your phone is unlocked unless you separately turn on lock-screen visibility.',
        );
      } else {
        setStatus(await T1ArcGlucoseDisplay.disableAsync());
  ...
  async function setLockScreenVisible(visible: boolean) {
    ...
    try {
      setStatus(
        await T1ArcGlucoseDisplay.setLockScreenVisibleAsync(visible),
      );
      setMessage(
        visible
  ...
  function requestLockScreenVisible(visible: boolean) {
    if (!visible) {
      void setLockScreenVisible(false);
      return;
    }
    Alert.alert(
      'Show glucose before unlock?',
      'Your glucose value, direction and freshness may be visible to anyone who can see your lock screen. Android privacy settings still have final control.',
      [
        { text: 'Keep private', style: 'cancel' },
        {
          text: 'Show glucose',
          onPress: () => void setLockScreenVisible(true),
        },
      ],
    );
  }
  ...
  function requestAod(desired: boolean) {
    if (!desired) {
      setBusy(true);
      void T1ArcGlucoseDisplay.setAodDesiredAsync(false)
        .then(setStatus)
        .finally(() => setBusy(false));
      return;
    }
    Alert.alert(
      'Allow always-on glucose display?',
      'T1 Arc uses Android Accessibility to notice system display changes and show your saved glucose reading over the always-on screen. This service does not read text from other apps, perform actions for you, or send Accessibility event data off your phone. Anyone who can see the screen may see your glucose. This is optional: other app features work without it. You can turn it off here or in Android Accessibility settings.',
      [
        { text: 'Not now', style: 'cancel' },
        {
          ...
          onPress: () => {
            setBusy(true);
            setMessage(undefined);
            void T1ArcGlucoseDisplay.setAodDesiredAsync(true)
              .then(async (next) => {
                setStatus(next);
                await T1ArcGlucoseDisplay.openAccessibilitySettingsAsync();
              })
              .catch((error) =>
                setMessage(
                  error instanceof Error
  ...
  async function setAodPosition(position: AodPosition) {
    ...
    setMessage(undefined);
    try {
      setStatus(await T1ArcGlucoseDisplay.setAodPositionAsync(position));
      const label =
  ...
  async function setAodSize(size: AodSize) {
    ...
    setMessage(undefined);
    try {
      setStatus(await T1ArcGlucoseDisplay.setAodSizeAsync(size));
      const label =
  ...
  async function setAndroidAutoEnabled(enabled: boolean) {
    ...
    try {
      setStatus(
        await T1ArcGlucoseDisplay.setAndroidAutoEnabledAsync(enabled),
      );
      setMessage(
        enabled
          ? 'The experimental Android Auto glance is ready. A GitHub APK needs Android Auto developer mode and its “Unknown sources” option once.'
    ...
    <SectionCard>
        ...
        detail={
          connected
            ? enabled && status?.latestMmolL !== undefined
              ? `${formatGlucose(status.latestMmolL, regional)} · ${
        ...
        detail={
          status?.androidAutoEnabled
            ? status.androidAutoProjected
              ? 'Connected · available in the car’s media and notification surfaces'
              : 'Experimental · GitHub APK · Android Auto developer mode and “Unknown sources” required'
            : 'Off · no car media or notification glance'
        }
        disabled={busy || !status?.supported}
        label="Experimental Android Auto glance"
      ...
      {enabled ? (
        <>
          <SettingRow
            ...
            onValueChange={requestLockScreenVisible}
      ...
      {permissionDenied || (enabled && !status?.notificationsAllowed) ? (
        <Pressable
          accessibilityRole="button"
          onPress={() =>
            void T1ArcGlucoseDisplay.openNotificationSettingsAsync()
          }
          style={({ pressed }) => [
        ...
        >
          <View style={styles.recoveryHeader}>
            ...
            <Text style={[styles.recoveryTitle, { color: colors.text }]}>
              Finish Android Accessibility setup
            </Text>
          </View>
          <Text
            style={[
          ...
          >
            {AOD_RESTRICTED_SETTINGS_STEP}
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={() => {
              void T1ArcGlucoseDisplay.openAppDetailsSettingsAsync();
            }}
            style={({ pressed }) => [
              styles.recoveryButton,
              {
          ...
          >
            {`In Accessibility, select “${AOD_ACCESSIBILITY_SERVICE_LABEL}” and turn on the main service switch. Leave its Accessibility shortcut off: the floating T1 Arc button is not the always-on display.`}
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={() => {
              void T1ArcGlucoseDisplay.openAccessibilitySettingsAsync();
            }}
            style={({ pressed }) => [
              styles.recoveryButton,
              {
