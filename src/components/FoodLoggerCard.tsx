...
import { FoodPhoto } from './foodLogger/FoodPhoto';
import { FoodPhotoField } from './foodLogger/FoodPhotoField';
import { LabelConventionPicker } from './foodLogger/LabelConventionPicker';
...
function FoodArtwork({
  ...
  return <View accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[styles.foodArtwork, { backgroundColor, borderRadius, height: size, width: size }]}>
    <Ionicons color={iconColor} name="nutrition-outline" size={20} />
    <FoodPhoto owner={{ kind: 'food', id: food.id }} remoteUrl={allowNetworkImage ? food.imageUrl : undefined} size={size} />
  </View>;
}
...
export function FoodLoggerCard({
  ...
  const [mealPhoto, setMealPhoto] = useState<string | null>();
  const [recipePhoto, setRecipePhoto] = useState<string | null>();
  ...
  }, [selected, title, recipeName, recipeServings, mealType, editingRecipeId, open, recipePhoto]);
  ...
  function close() {
    ...
    const customFoodChanged =
      ...
      customFood.fat.trim() ||
      customFood.fibre.trim() || customFood.photoData !== undefined || customFood.labelConvention !== undefined;
    const hasUnsavedDraft = Boolean(
      ...
      quickCarbs.trim() ||
      quickCarbLabel.trim() ||
      customFoodChanged || mealPhoto !== undefined || recipePhoto !== undefined,
    );
    const discard = () => {
      barcodeRequestGeneration.current += 1;
      barcodeLookupLock.current = false;
  ...
  function beginLabelFood(capture: boolean) {
    Keyboard.dismiss();
    takeCustomFoodFormOwnership();
    setCustomFood(current => ({ ...current, name: current.name || query.trim() }));
    setQuickCarbOpen(false); setBarcodeEntryOpen(false); setCatalogueToolsOpen(false);
    setMoreWaysOpen(true); setCustomOpen(true);
    if (capture) setLabelCaptureOpen(true);
  }
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <Text style={{ color: colors.primary, fontSize: 12 }}>Scan</Text>
                </Pressable>
              </View>
              ...
              <View style={{ gap: 10 }}>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                  {[{ title: 'Read label', icon: 'camera-outline' as const, run: () => beginLabelFood(true) },
                    { title: 'Quick carbs', icon: 'add-circle-outline' as const, run: () => { Keyboard.dismiss(); setCustomOpen(false); setBarcodeEntryOpen(false); setCatalogueToolsOpen(false); setQuickCarbOpen(true); setMoreWaysOpen(true); } },
                    { title: 'Enter food', icon: 'create-outline' as const, run: () => beginLabelFood(false) }].map(action => <Pressable key={action.title} accessibilityRole="button" onPress={action.run} style={{ flexGrow: 1, flexBasis: 90, padding: 12, minHeight: 64, gap: 6, alignItems: 'center', borderRadius: radius.md, backgroundColor: colors.surfaceMuted }}><Ionicons name={action.icon} size={24} color={colors.primary} /><Text style={{ color: colors.text, fontWeight: '600', textAlign: 'center' }}>{action.title}</Text></Pressable>)}
                </View>
                <View style={styles.foodActions}>
                  <Pressable accessibilityLabel="Copy foods from yesterday or another day" accessibilityRole="button" onPress={beginCopyFromDay} style={[styles.foodAction, { minHeight: 48 }]}><Text style={{ color: colors.primary }}>Copy from a day</Text></Pressable>
                  <Pressable accessibilityLabel="More ways to add food" accessibilityRole="button" onPress={() => { Keyboard.dismiss(); setMoreWaysOpen(true); }} style={[styles.foodAction, { minHeight: 48 }]}><Text style={{ color: colors.textSecondary }}>More options</Text></Pressable>
                </View>
              </View>
                ...
                <View style={styles.recentMeals}>
                  ...
                  >
                    {library.recipes.map((recipe) => (
                      ...
                      >
                        <FoodPhoto owner={{ kind: 'recipe', id: recipe.id }} wide size={120} label={`${recipe.name} photo`} />
                        <Pressable
              ...
              {selected.length ? (
                <View style={styles.selectedBlock}>
                  ...
                  {!editingRecipeId ? <FoodPhotoField key={editingLog?.id ?? 'new-meal'} owner={editingLog ? { kind: 'meal', id: editingLog.id } : undefined} label="Meal photo" value={mealPhoto} onChange={setMealPhoto} disabled={saving} /> : null}
                  {selected.map((item) => {
                  ...
                  {!editingLog ? (
                    ...
                    >
                      ...
                      {recipeSaveOpen ? (
                        ...
                        >
                          <FoodPhotoField key={editingRecipeId ?? 'new-recipe'} owner={editingRecipeId ? { kind: 'recipe', id: editingRecipeId } : undefined} label="Recipe photo" value={recipePhoto} onChange={setRecipePhoto} disabled={recipeSaving} />
                          <TextInput
                ...
                <View style={styles.resultsBlock}>
                  ...
                  !visibleResults.length ? (
                    ...
                    >
                      ...
                      <Pressable
                        accessibilityLabel="Read the label for this missing food"
                        accessibilityRole="button"
                        onPress={() => beginLabelFood(true)}
                        style={({ pressed }) => [
                ...
                <View style={styles.advancedEntryPanel}>
                  ...
                  {customOpen ? (
                    ...
                    >
                      ...
                      <LabelConventionPicker value={customFood.labelConvention} onChange={labelConvention => { takeCustomFoodFormOwnership(); setCustomFood(current => ({ ...current, labelConvention })); }} />
                      <FoodPhotoField key={editingMyFoodId ?? 'new-food'} owner={editingMyFoodId ? { kind: 'food', id: editingMyFoodId } : undefined} value={customFood.photoData} onChange={photoData => { takeCustomFoodFormOwnership(); setCustomFood(current => ({ ...current, photoData })); }} disabled={customSaving} />
                      <TextInput
