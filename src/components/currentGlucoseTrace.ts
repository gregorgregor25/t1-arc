...
export function glucoseTraceGeometry(
  ...
  const points = readings
    ...
    .filter(item => {
      // Repeated imports of the same point must not split an otherwise
      // continuous trace. Conflicting simultaneous values remain separate.
      const key = `${item.timestamp}:${item.mmolL}`;
