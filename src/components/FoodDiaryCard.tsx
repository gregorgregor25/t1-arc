...
function calories(value: number | undefined) {
  return value === undefined ? '—' : `${Math.round(value)} kcal`;
}
...
function portionNumber(value: string) {
...
export function FoodDiaryCard({
  ...
  const loggedEnergy = effectiveLogs.reduce(
    (total, log) => total + (log.nutrition.energyKcal ?? 0),
    0,
  );
  ...
  function beginPortionEdit(log: FoodLog) {
    ...
    setPortionDrafts(
      Object.fromEntries(
        log.items.map((item) => [item.id, String(item.amount)]),
      ),
    );
  ...
  async function savePortions(log: FoodLog) {
    ...
    const amounts = Object.fromEntries(
      log.items.map((item) => [
        item.id,
        portionNumber(portionDrafts[item.id] ?? ''),
      ]),
    );
    setPortionBusy(true);
    setPortionError(undefined);
  ...
  return (
    <SectionCard
      accessibilityLabel={`${meals.length} meals, ${formatFoodNumber(totalCarbs)} grams carbohydrate logged in this range.`}
    >
      ...
      {loggedEnergy > 0 ? (
        <Text style={[styles.energy, { color: colors.textSecondary }]}>
          {Math.round(loggedEnergy).toLocaleString('en-GB')} kcal across
          itemised T1 Arc logs
        </Text>
      ) : null}
      {repeatMessage ? (
        <Text
      ...
      <View style={styles.rows}>
        {visibleMeals.map((meal) => {
          ...
          const displayLog =
            log && editing ? previewPortions(log, portionDrafts) : log;
          ...
          return (
            ...
            >
              <Pressable
                accessibilityLabel={`${meal.title}, ${Math.round(meal.carbsGrams)} grams carbohydrate at ${formatTime(meal.start)}${log ? '. Tap for item details' : canEditSourceMeal ? '. Tap to edit' : ''}`}
                accessibilityRole={
              ...
              >
                ...
                <View style={styles.mealCopy}>
                  ...
                  >
                    {mealTypeLabel(meal.mealType)} · {formatTime(meal.start)}
                    {log ? ` · ${log.items.length} item${log.items.length === 1 ? '' : 's'}` : ''}
                  </Text>
                </View>
                <View style={styles.carbsCopy}>
                  <Text style={[styles.carbs, { color: colors.text }]}>
                    {Math.round(
                      displayLog?.nutrition.carbohydrateGrams ??
                        meal.carbsGrams,
                    )}{' '}
                    g
                  </Text>
                ...
                >
                    ...
                    >
                      <View style={styles.itemCopy}>
                        ...
                        >
                          ...
                          {editing
                            ? item.sourceLabel
                            : `${item.amount} ${item.unit} · ${item.sourceLabel}`}
                        </Text>
                      </View>
                      {editing ? (
                  ...
                  {response && responseEvidence ? (
                    <Pressable
                      ...
                      accessibilityLabel={`Observed glucose response. ${response.baseline.mmolL.toFixed(1)} millimoles per litre near the meal, peak ${response.peak.mmolL.toFixed(1)}, change ${response.riseMmolL >= 0 ? 'plus ' : 'minus '}${Math.abs(response.riseMmolL).toFixed(1)}. ${response.readings.length} supporting readings.`}
                    ...
                    >
                      <View style={styles.responseHeader}>
                        ...
                        <View style={styles.responseDelta}>
                          ...
                          >
                            {response.riseMmolL >= 0 ? '+' : ''}
                            {response.riseMmolL.toFixed(1)}
                          </Text>
                          <Text
                            style={[
                      ...
                      >
                        ...
                        {minutesToPeak !== undefined
                          ? ` · ${minutesToPeak} min after`
                          : ''}{' '}
                        · {response.readings.length} supporting readings
                      </Text>
                  ...
                  <View style={styles.macros}>
                    <View style={styles.macro}>
                      ...
                      >
                        {calories(displayLog!.nutrition.energyKcal)}
                      </Text>
                      <Text
                        style={[
                          styles.macroLabel,
      ...
      {visibleMeals.length < meals.length ? (
        ...
        >
          <Text style={[styles.showMoreText, { color: colors.primary }]}>
            Show more meals ({meals.length - visibleMeals.length} remaining)
          </Text>
        </Pressable>
      ) : null}
