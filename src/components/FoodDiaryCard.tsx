...
export function FoodDiaryCard({
  ...
  const nutritionSummary = useMemo(
    () =>
      summarizeHealthTrendContext(events, {
  ...
  return (
    <SectionCard
      accessibilityLabel={`${formatRegionalNumber(meals.length, regional.locale, { maximumFractionDigits: 0 })} food entries. ${
    ...
    >
      <View style={styles.header}>
        <View>
          ...
          <Text style={[styles.title, { color: colors.text }]}>
            {meals.some(meal => meal.purpose === 'hypo-treatment') ? 'Food & hypo treatments' : 'Logged meals'}
          </Text>
        </View>
      ...
      {nutritionSummary.foodLoggingIncomplete ? <Text style={{ color: colors.warning }}>Some food was not logged. These totals cover recorded food only.</Text> : null}
      ...
      <View style={styles.rows}>
        ...
        {visibleMeals.map((meal) => {
          ...
          return (
            ...
            >
              {log ? <FoodPhoto owner={{ kind: 'meal', id: log.id }} wide size={160} label={`${meal.title} photo`} /> : null}
              <Pressable
              ...
              >
                ...
                <View style={styles.mealCopy}>
                  ...
                  >
                    {meal.purpose === 'hypo-treatment' ? 'Hypo treatment' : mealTypeLabel(meal.mealType)} ·{" "}
