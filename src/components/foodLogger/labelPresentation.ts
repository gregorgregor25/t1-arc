...
export function customFoodFormFromLabelDraft(current: CustomFoodForm, draft: FoodLabelDraft, regional: Pick<T1ArcRegionalDefaults, 'locale' | 'energyUnit'>): CustomFoodForm {
  ...
  const displayed = {
    ...
    sugars: input(fields.sugars),
    saturatedFat: input(fields.saturatedFat),
  };
  return {
    ...displayed,
    photoData: current.photoData,
    labelConvention: draft.labelConvention ?? current.labelConvention,
    canonicalBaseline: fields.serving && fields.unit ? {
      displayed,
      servingAmount: fields.serving,
      servingUnit: fields.unit,
      nutritionPerServing: { carbohydrateGrams: fields.carbs, energyKcal: fields.energyKcal, proteinGrams: fields.protein, fatGrams: fields.fat, fibreGrams: fields.fibre, sugarsGrams: fields.sugars, saturatedFatGrams: fields.saturatedFat },
    } : undefined,
  };
}
