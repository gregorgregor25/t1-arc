...
export interface CustomFoodForm extends CustomFoodEditableValues {
  ...
  labelConvention?: import('@/data/food/labelConvention').LabelConvention;
  /**
...
function exactOrParsedNutrition(
  form: CustomFoodForm,
  key: 'carbs' | 'energy' | 'protein' | 'fat' | 'fibre' | 'sugars' | 'saturatedFat',
  canonicalKey: keyof Pick<
    FoodNutrition,
    'carbohydrateGrams' | 'energyKcal' | 'proteinGrams' | 'fatGrams' | 'fibreGrams' | 'sugarsGrams' | 'saturatedFatGrams'
  >,
...
export function customFoodFormFromCandidate(
  ...
  const displayed: CustomFoodEditableValues = {
    ...
    sugars: inputOptional(food.nutritionPerBasis.sugarsGrams),
    saturatedFat: inputOptional(food.nutritionPerBasis.saturatedFatGrams),
  };
  return {
    ...displayed,
    labelConvention: food.nutrientDefinitions?.note?.startsWith('UK / EU label.') ? 'uk-eu' : food.nutrientDefinitions?.note?.startsWith('US label.') ? 'us' : food.nutrientDefinitions?.note?.startsWith('Other / unsure label.') ? 'other' : undefined,
    canonicalBaseline: {
      displayed,
...
export function customFoodDraftFromForm(
  ...
  return {
    ...
    labelConvention: form.labelConvention,
    ...
    nutritionPerServing: {
      ...
      sugarsGrams: exactOrParsedNutrition(form, 'sugars', 'sugarsGrams', optionalFormNumber(form.sugars ?? '', regional.locale)),
      saturatedFatGrams: exactOrParsedNutrition(form, 'saturatedFat', 'saturatedFatGrams', optionalFormNumber(form.saturatedFat ?? '', regional.locale)),
