...
export function EvidenceClockWindowOverlay({
  ...
  const displayUnits = glucoseUnitLabel(regional.glucoseUnit);
  const formatValue = useCallback(
    (mmolL: number) =>
      formatGlucose(
        mmolL,
        {
          glucoseUnit: regional.glucoseUnit,
          locale: regional.locale,
        },
        { withUnit: false },
      ),
    [regional.glucoseUnit, regional.locale],
  );
  ...
  const effectiveTargetRange = useMemo(() => {
    ...
    return {
      maximum: appearance.targetMax,
      minimum: appearance.targetMin,
    };
  }, [
          ...
          <View style={styles.emptyState}>
            ...
            >
              {invalidDomain
                ...
                : resolved
                  ? `${formatEvidenceClockWindow(resolved, regional.locale)} · ${coverageSummary}`
                  : coverageSummary}
            </Text>
            {missing.length ? (
          ...
          <Svg accessibilityElementsHidden height={height} width={width}>
            ...
            {overallMeanMmolL !== undefined && overallMeanMmolL !== null ? (
              <G>
                ...
                >
                  OVERALL {formatValue(overallMeanMmolL)}
                </SvgText>
              </G>
            ) : null}
            ...
            {ticks.map((minute, index) => (
              <G key={minute}>
                ...
                <SvgText
                  ...
                  y={bottom + 19}
                >
                  {formatEvidenceClockMinute(minute, regional.locale)}
                </SvgText>
              </G>
            ))}
            <SvgText
        ...
        >
          ...
          {overallMeanMmolL !== undefined && overallMeanMmolL !== null ? (
            <View style={styles.legendItem}>
              ...
              <Text style={[styles.legendText, { color: colors.textSecondary }]}>
                Exact overall answer mean {formatValue(overallMeanMmolL)}{' '}
                {displayUnits}
              </Text>
            </View>
          ) : null}
        ...
        >
          ...
          {hasClockChanges ? (
            <View style={styles.noteRow}>
              ...
              />
              <Text style={[styles.noteText, { color: colors.textSecondary }]}>
                The trace splits where the {regional.timeZone} clock changes;
                this is not a missing sensor interval.
              </Text>
            </View>
          ) : null}
          {missing.length ? (
