...
export function GlucoseAppearanceProvider({ children }: PropsWithChildren) {
  ...
  const reload = useCallback(async () => {
    const stored = await T1ArcGlucoseDisplay.getAppearanceSettingsAsync();
  ...
  useEffect(() => {
    let active = true;
    void T1ArcGlucoseDisplay.getAppearanceSettingsAsync()
      .then((stored) => {
  ...
  const save = useCallback(async (next: GlucoseAppearanceSettings) => {
    ...
    const stored = await T1ArcGlucoseDisplay.setAppearanceSettingsAsync(next);
    setSettings(stored);
  }, []);
