...
import { TarvisReportButton } from '@/components/TarvisReportButton';
      ...
      {grouped.length ? (
        ...
        <SectionCard>
          <Text style={[styles.historyEmptyTitle, { color: colors.text }]}>
            No conversations for this connection yet
          </Text>
          <Text style={[styles.historyDetail, { color: colors.textSecondary }]}>
...
export function TarvisScreen({
  ...
  useEffect(() => {
    ...
    settingsOpenedFromMenu.current = true;
    setSettingsVisible(true);
    onSettingsRequestHandled?.();
  }, [settingsRequested, onSettingsRequestHandled]);
  ...
  const compactHeader = (
    <View style={styles.compactHeader}>
      <Pressable
        accessibilityLabel={
          settingsVisible
            ? settingsOpenedFromMenu.current ? "Back to Settings" : "Back to Tarv1s"
            : historyVisible
              ? "Back to Tarv1s home"
              : conversationVisible
                ? "Back to Tarv1s home"
      ...
      <Pressable
        accessibilityLabel={
          historyVisible || !conversationVisible ? "Open Tarv1s settings" : "Start a new conversation"
        }
        accessibilityRole="button"
        accessibilityState={{ disabled: settingsVisible || working }}
        disabled={settingsVisible || working}
        onPress={() => {
          if (historyVisible || !conversationVisible) {
            setSettingsVisible(true);
            return;
          }
      ...
      >
        <Ionicons
          accessibilityElementsHidden
          color={colors.textSecondary}
          name={historyVisible || !conversationVisible ? "settings-outline" : "create-outline"}
          size={22}
        />
      </Pressable>
    </View>
    ...
    >
      ...
      >
        ...
        {loadingSettings && settingsVisible ? (
          ...
          />
          <TarvisConversationArchive key={JSON.stringify(conversationScope)} scope={conversationScope} />
          </>
        ) : (
          <>
            {conversationCorrupt ? (
                  ...
                  >
                    ...
                    <View style={styles.assistantMessage}>
                      ...
                      <View style={styles.assistantContent}>
                        ...
                        <TarvisReportButton key={`report-${exchange.id}-${exchange.scope.kind === 'legacy-unknown' ? 'legacy' : `${exchange.scope.ownerIdentity}-${exchange.scope.dataMode}`}`} response={exchange.answer.answer} disabled={working} />
                        <TarvisRouteMarker
