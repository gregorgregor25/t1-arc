...
function scheduleSummary(
  ...
  return `${label}: ${segments
    .map((segment) => `${segment.startTime} ${segment.value} ${segment.unit}`)
    .join(', ')}`;
}
    ...
    findings.push({
      ...
      summary:
        summary
          ...
          : pumpStates.length
            ? `${pumpStates.length} exact Activity mode or automated-pause windows are available for this period.`
            : `Daily operating-mode evidence is available for ${dailyModes.length} days in this period.`,
      caveat:
        pumpStates.length
  ...
  if (settingsRecord) {
    ...
    if (payload) {
      ...
      findings.push({
        ...
        summary: `The retained Glooko report contains the active pump configuration and ${scheduleSegments} time-based basal, carb-ratio, sensitivity, target or correction-threshold entries.`,
        caveat:
