...
export async function loadNotebook(scope: NotebookScope): Promise<NotebookSections> {
  assertScope(scope);
  if (parseRecoveredOwnerProof(await readPersonalAppState(RECOVERED_OWNER_PROOF_PREFIX+NOTEBOOK_STORAGE_KEY))) {
    const { reconcileRecoveredPersonalState } = await import('@/data/tarvis/recoveredPersonalState');
