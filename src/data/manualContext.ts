...
import { sensorModel, sensorSelection } from "@/domain/sensorModels";
import { observationDescription, validatePersonalObservation, type PersonalObservation, type BodyLocation } from '@/domain/personalObservations';
...
export type ManualContextDraft =
  | (DraftBase & { kind: PersonalObservation['kind']; observation: PersonalObservation })
  | (DraftBase & {
      kind: "meal";
...
function defaultTitle(draft: ManualContextDraft) {
  switch (draft.kind) {
    case 'hypo-treatment': return 'Hypo treatment';
    case 'food-incomplete': return 'Some food was not logged';
    case 'lab-result': return 'Laboratory result';
    case 'site-change': return 'Device site';
    case "meal":
...
export function manualContextDraftFromEvent(
  ...
  if (event.excludedFoodSource) throw new Error('Choose this food diary in Settings before editing its entries.');
  if (event.observation && !(event.kind === 'note' && event.sensorStarted)) {
    return { kind: event.observation.kind, observation: event.observation, timestamp: event.start };
  }
    ...
    case "note":
      {
        if (event.sensorStarted === true && event.category === "sensor") {
          draft = {
            ...
            sensorModelId: event.sensorModelId,
            sensorWarmupMinutes: event.sensorWarmupMinutes,
            location: event.observation?.kind === 'site-change' ? event.observation.location : undefined,
          };
          break;
        }
...
export function createManualContextEvent(
  ...
  switch (draft.kind) {
    ...
    case 'lab-result':
    case 'site-change': {
      if (draft.timestamp > Date.now() + 60_000) throw new Error('Recorded observations cannot be dated in the future.');
      const observation = validatePersonalObservation(draft.observation);
      if (observation.kind !== draft.kind) throw new Error('The observation type does not match this form.');
      return { ...base, kind: 'note', category: observation.kind === 'site-change' ? observation.device : 'other', observation,
        title: observation.kind === 'lab-result' ? `${observation.test} · ${observation.value} ${observation.unit}` : observation.kind === 'site-change' ? `${observation.device === 'pump' ? 'Pump' : 'Sensor'} site` : observation.kind === 'hypo-treatment' ? `Hypo treatment · ${observation.food}` : base.title,
        ...(observation.kind === 'food-incomplete' ? { start: zonedDateTimeToTimestamp(toDateKey(draft.timestamp)), end: zonedDateTimeToTimestamp(addDays(toDateKey(draft.timestamp), 1)) } : {}),
        detail: observationDescription(observation) };
    }
    case "meal":
      return {
        ...base,
    ...
    case "sensor-start": {
      const selection = sensorSelection(draft.sensorModelId, draft.sensorWarmupMinutes);
      const model = sensorModel(selection.modelId);
      const minutes = selection.warmupMinutes;
      return {
        ...base,
        kind: "note",
        category: "sensor",
        ...
        sensorModelId: selection.modelId,
        sensorWarmupMinutes: minutes,
        observation: draft.location ? validatePersonalObservation({ kind: 'site-change', device: 'sensor', location: draft.location }) : undefined,
        end: minutes === undefined ? undefined : draft.timestamp + minutes * 60_000,
        title: model && model.id !== 'other' ? `Started ${model.label}` : base.title,
        detail: (minutes === undefined
          ? 'User recorded a new sensor. Warm-up duration is unknown; no gap is attributed to warm-up.'
          : `User recorded ${model?.label ?? 'a new sensor'}. Expected warm-up: ${minutes} minutes from the recorded start. Missing readings within this window are consistent with sensor warm-up; missing time outside it remains unexplained. This is an expected window, not confirmation from the sensor. Actual readings are retained.`) + (draft.location ? ` ${observationDescription({ kind: 'site-change', device: 'sensor', location: draft.location })}` : ''),
      };
    }
    case "note": {
