...
export interface DexcomClarityImportRegionalSettings {
  /** IANA zone used to turn the export's local wall-clock values into instants. */
  timeZone: string;
  /** Numeric date order used by the selected Dexcom Clarity export. */
  dateOrder: 'day-first' | 'month-first';
}

export const DEFAULT_DEXCOM_CLARITY_IMPORT_REGIONAL_SETTINGS: DexcomClarityImportRegionalSettings =
  {
...
function isGlucoseEventType(value: string) {
  ...
  return (
    ...
    [
      ...
      '推定グルコース',
      'センサーグルコース',
    ].some((label) => value.includes(normaliseHeader(label)))
  );
}
...
function glucoseUnitFromHeader(header: string) {
  ...
  if (normalised.includes('mg dl')) return 'mg/dL' as const;
  return undefined;
}
...
function rateUnitFromHeader(header: string) {
  ...
  if (normalised.includes('mg dl')) return 'mg/dL/min' as const;
  return undefined;
}
...
export function parseDexcomClarityCsv(
  ...
  const eventTypeColumn = findColumn(header, [
    'event type',
    'record type',
    'type événement',
    'type d’événement',
  ...
  if (!glucoseUnit) {
    throw new Error(
      'The Dexcom glucose column does not declare mg/dL or mmol/L, so T1 Arc cannot safely normalise this export.',
    );
  }
  const rateUnit =
    rateColumn >= 0
  ...
  for (const row of iterator) {
    ...
    const eventType =
      eventTypeColumn >= 0
        ...
        : 'egv';
    if (!isGlucoseEventType(eventType)) {
      continue;
    }
    ...
    const sourceRate =
      rateColumn >= 0 && rateUnit
        ? parseNumber(row[rateColumn])
        : undefined;
  ...
  if (skippedRows) {
    warnings.push(
      `${count(skippedRows)} glucose ${skippedRows === 1 ? 'row was' : 'rows were'} incomplete or outside the supported sensor range.`,
    );
  }
  if (duplicateRows) {
    warnings.push(
      `${count(duplicateRows)} duplicate ${duplicateRows === 1 ? 'timestamp was' : 'timestamps were'} replaced by the last value in the export.`,
    );
  }
  if (rateColumn >= 0 && !rateUnit) {
    warnings.push(
