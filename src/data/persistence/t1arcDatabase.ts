...
import { HEALTH_CONNECT_IDENTITY_INDEX_SQL } from './healthConnectIdentityIndex';
...
async function openAndMigrate() {
  ...
  await database.execAsync(`
    ...
    CREATE TABLE IF NOT EXISTS food_photos (
      id TEXT NOT NULL PRIMARY KEY,
      food_id TEXT UNIQUE REFERENCES food_catalog_cache(id) ON DELETE CASCADE,
      log_id TEXT UNIQUE REFERENCES food_logs(id) ON DELETE CASCADE,
      recipe_id TEXT UNIQUE REFERENCES food_recipes(id) ON DELETE CASCADE,
      image_data TEXT NOT NULL CHECK(length(image_data) <= 349551),
      source_url TEXT,
      created_at_ms INTEGER NOT NULL,
      CHECK ((food_id IS NOT NULL) + (log_id IS NOT NULL) + (recipe_id IS NOT NULL) = 1)
    );
  ...
  // Additive, idempotent index; run after any legacy table reconstruction.
  // Existing rows and portable provider-identity semantics are unchanged.
  await database.execAsync(HEALTH_CONNECT_IDENTITY_INDEX_SQL);
  await ensureColumn(
    database,
    'glucose_readings',
    'imported_at_ms',
  ...
  await ensureColumn(database, 'context_notes', 'sensor_model_id', 'TEXT');
  await ensureColumn(database, 'context_notes', 'observation_json', 'TEXT');
  await ensureColumn(database, 'context_notes', 'sensor_warmup_minutes', 'INTEGER');
