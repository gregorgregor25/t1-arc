...
import { selectRetainedGlookoSources } from '@/data/glooko/glookoSourceRetention';
...
export interface ImportSourceEntry {
  name: string;
  /** Present in archives retained by legacy 1.3.5 and earlier builds. */
...
/**
 * Exact field/value evidence from a validated source row. This encrypted,
 * deduplicated index remains usable after the bounded recovery archive has
 * expired, without reparsing a ZIP for each screen or analysis.
 */
...
export interface ManualContextSaveOptions {
  /**
   * Keeps derived review state consistent with the primary manual record in
   * the same durable transaction. Creates only dirty the inputs; edits also
   * remove reports that may contain a readable copy of the previous value.
   */
...
export interface HealthRecordStore {
  ...
  getManualInsulinBounds(): Promise<StoredRecordBounds>;
...
export class MemoryHealthRecordStore implements HealthRecordStore {
  ...
  async writeImport(
    ...
    if (batch.sourceId === 'glooko-export') {
      const candidates = [...this.sourcePayloads.entries()].flatMap(
        ([payloadKey, payload]) => {
          const payloadBatch = this.batches.get(payloadKey);
          return payloadBatch?.sourceId === 'glooko-export'
            ? [
                {
                  id: payloadKey,
                  kind: 'archive' as const,
                  byteLength: payload.bytes.length,
                  storedAt: payloadBatch.importedAt,
                  dataStart: payloadBatch.dataStart,
                  dataThrough: payloadBatch.dataThrough,
                },
              ]
            : [];
        },
      );
      const retained = selectRetainedGlookoSources(candidates).retainedIds;
      for (const payloadKey of this.sourcePayloads.keys()) {
        if (
          payloadKey.startsWith('glooko-export:') &&
          !retained.has(payloadKey)
        ) {
          this.sourcePayloads.delete(payloadKey);
        }
      }
    }
    return {
  ...
  async getManualInsulinBounds(): Promise<StoredRecordBounds> {
    const records = [...this.boluses.values()].filter(isManualInsulinDelivery);
    if (!records.length) return { count: 0 };
    return {
      earliest: Math.min(...records.map((record) => record.timestamp)),
      latest: Math.max(...records.map((record) => record.timestamp)),
      lastRecordedAt: Math.max(
        ...records.map((record) => record.importedAt ?? record.timestamp),
      ),
      count: records.length,
    };
  }

  async saveManualInsulin(delivery: BolusDelivery) {
  ...
  async deleteManualInsulin(id: string) {
    const delivery = this.boluses.get(id);
    if (!delivery || !isManualInsulinDelivery(delivery)) return false;
    return this.boluses.delete(id);
  }
