...
import { parsePersonalObservation, presentPersonalObservation, validatePersonalObservation } from '@/domain/personalObservations';
import { NUTRITION_SOURCE_KEY, selectNutritionSource, validateNutritionSource } from '@/domain/nutritionSource';
...
import { healthConnectSourceDisplayName } from '@/data/healthConnect/healthConnectSourcePolicy';
import {
  isManualInsulinDelivery,
  MANUAL_INSULIN_DELIVERY_TYPES,
...
interface ContextRow {
  ...
  source_file: string | null;
  source_row: number | null;
  hevy_payload_json?: string | null;
  health_connect_display_name?: string | null;
}
...
interface ContextNoteRow {
  ...
  sensor_warmup_minutes: number | null;
  recorded_at_ms: number;
  source_file: string | null;
  source_row: number | null;
}
...
function isMissingTable(error: unknown, table: string) {
  return new RegExp(`no such table:\\s*${table}`, 'i').test(String(error));
}

async function getHealthConnectSourceNames(database: SQLiteDatabase) {
  try {
    const rows = await database.getAllAsync<{
      package_name: string;
      display_name: string;
    }>(`SELECT package_name, display_name FROM health_connect_sources`);
    return new Map(rows.map((row) => [row.package_name, row.display_name]));
  } catch (error) {
    // Partial legacy/test databases may predate the source catalogue. The
    // package identifier still gives us a stable provenance fallback.
    if (isMissingTable(error, 'health_connect_sources')) {
...
function contextBase(
  row: ContextRow,
  healthConnectSourceNames: ReadonlyMap<string, string>,
) {
  const healthConnectPrefix = 'health-connect:';
  const healthConnectPackage = row.source_id.startsWith(healthConnectPrefix)
    ? row.source_id.slice(healthConnectPrefix.length)
    : undefined;
  const sourceLabel = healthConnectPackage
    ? healthConnectSourceDisplayName(
        healthConnectPackage,
        healthConnectSourceNames.get(healthConnectPackage) ??
          row.health_connect_display_name ??
          healthConnectPackage,
      )
    : row.source_id === 't1arc-food'
      ? 'T1 Arc food log'
      : row.source_id === 'hevy'
        ? 'Hevy'
...
function contextFromRow(
  row: ContextRow,
  items: MealItemRow[] = [],
  healthConnectSourceNames: ReadonlyMap<string, string> = new Map(),
): HealthContextEvent {
  const base = contextBase(row, healthConnectSourceNames);
  switch (row.kind) {
    case 'meal':
      return {
        ...base,
        ...
        fibreGrams: row.fibre_grams ?? undefined,
        sugarsGrams: row.sugars_grams ?? undefined,
        saturatedFatGrams: row.saturated_fat_grams ?? undefined,
        servingQuantity: row.serving_quantity ?? undefined,
        servingCount: row.serving_count ?? undefined,
        nutritionDetail: items.length ? 'itemized' : 'summary',
        items: items.length
          ? items.map((item) => ({
              id: item.id,
              name: item.name_snapshot,
              brand: item.brand_snapshot ?? undefined,
              amount: item.amount,
              unit: item.unit,
              carbohydrateGrams: item.carbohydrate_grams ?? undefined,
              energyKcal: item.energy_kcal ?? undefined,
              proteinGrams: item.protein_grams ?? undefined,
              fatGrams: item.fat_grams ?? undefined,
              fibreGrams: item.fibre_grams ?? undefined,
              sugarsGrams: item.sugars_grams ?? undefined,
              saturatedFatGrams:
                item.saturated_fat_grams ?? undefined,
              sourceLabel: item.source_label,
            }))
          : undefined,
      };
    case 'activity':
      let strengthWorkout: ActivityEvent['strengthWorkout'];
      if (row.hevy_payload_json) {
        try {
          strengthWorkout = hevyWorkoutDetail(
            parseHevyWorkout(JSON.parse(row.hevy_payload_json)),
          );
        } catch {
          // A malformed source-detail payload must not hide the activity row.
        }
      }
      return {
        ...base,
...
function noteFromRow(row: ContextNoteRow): ContextNoteEvent {
  return {
    observation: row.origin === 'manual' ? parsePersonalObservation(row.observation_json) : undefined,
    id: row.id,
    sourceId: row.source_id,
    origin: row.origin,
    kind: 'note',
    ...
    ...(row.sensor_started === 1 && row.category === 'sensor' && row.origin === 'manual'
      ? { sensorStarted: true, sensorGlucoseSourceId: row.sensor_glucose_source_id ?? undefined,
          sensorModelId: row.sensor_model_id ?? undefined, sensorWarmupMinutes: row.sensor_warmup_minutes ?? undefined }
      : {}),
    recordedAt: row.recorded_at_ms,
    sourceFile: row.source_file ?? undefined,
    sourceRow: row.source_row ?? undefined,
...
function mealItemsByContext(rows: MealItemRow[]) {
  const items = new Map<string, MealItemRow[]>();
  for (const row of rows) {
    const existing = items.get(row.context_event_id) ?? [];
    existing.push(row);
    items.set(row.context_event_id, existing);
  }
  return items;
}

async function getMealItemsForRange(
  database: SQLiteDatabase,
  range: TimeRange,
) {
  try {
    return await database.getAllAsync<MealItemRow>(
      `SELECT logs.context_event_id, items.id, items.name_snapshot,
              items.brand_snapshot, items.amount, items.unit,
              items.carbohydrate_grams, items.energy_kcal,
              items.protein_grams, items.fat_grams, items.fibre_grams,
              items.sugars_grams, items.saturated_fat_grams,
              items.source_label
         FROM food_log_items items
         INNER JOIN food_logs logs ON logs.id = items.food_log_id
        WHERE logs.timestamp_ms >= ? AND logs.timestamp_ms < ?
        ORDER BY logs.timestamp_ms ASC, items.ordinal ASC`,
      range.start,
      range.end,
    );
  } catch (error) {
    if (
      isMissingTable(error, 'food_log_items') ||
      isMissingTable(error, 'food_logs')
    ) {
      return [];
    }
    throw error;
...
async function getMealItemsForContextIds(
  database: SQLiteDatabase,
  ...
  try {
    return await database.getAllAsync<MealItemRow>(
      `SELECT logs.context_event_id, items.id, items.name_snapshot,
              items.brand_snapshot, items.amount, items.unit,
              items.carbohydrate_grams, items.energy_kcal,
              items.protein_grams, items.fat_grams,
              items.fibre_grams, items.sugars_grams,
              items.saturated_fat_grams, items.source_label
         FROM food_log_items items
         INNER JOIN food_logs logs ON logs.id = items.food_log_id
        WHERE logs.context_event_id IN (${placeholders})
        ORDER BY logs.timestamp_ms ASC, items.ordinal ASC`,
      ...contextIds,
    );
  } catch (error) {
...
export async function clearOwnedHealthSourceDataInTransaction(
  ...
  for (const statement of [
    ...
    'DELETE FROM import_source_payloads WHERE source_id = ?',
    'DELETE FROM glooko_report_payloads WHERE source_id = ?',
    'DELETE FROM import_batches WHERE source_id = ?',
  ]) {
    await transaction.runAsync(statement, sourceId);
  }
}
...
export class SqliteHealthRecordStore implements HealthRecordStore {
  ...
  withWriteLease(writeLease: LocalDataWriteLease) {
    if (
      this.sourceWriteLease &&
      this.sourceWriteLease.localDataWriteLease.epoch !== writeLease.epoch
    ) {
      throw new Error(
        'The local write lease does not match the source connection lease.',
      );
    }
    return new SqliteHealthRecordStore(writeLease, this.sourceWriteLease);
  }

  withSourceWriteLease(sourceWriteLease: SourceConnectionWriteLease) {
    if (
      this.writeLease &&
      this.writeLease.epoch !== sourceWriteLease.localDataWriteLease.epoch
    ) {
      throw new Error(
        'The local write lease does not match the source connection lease.',
      );
    }
    return new SqliteHealthRecordStore(
      this.writeLease ?? sourceWriteLease.localDataWriteLease,
      sourceWriteLease,
    );
  }
  ...
  private async assertWriteLease(
    ...
    if (this.sourceWriteLease) {
      if (!sourceId) {
        await assertLocalDataWriteLeaseInTransaction(
          transaction,
          this.sourceWriteLease.localDataWriteLease,
        );
        throw new SourceConnectionSupersededError(
          this.sourceWriteLease.sourceId,
        );
      }
    ...
    } else if (this.writeLease) {
      await assertLocalDataWriteLeaseInTransaction(transaction, this.writeLease);
    }
  }

  private assertImportSourceIds(
    batch: ImportBatch,
    basal: BasalDelivery[],
    ...
    if (!this.sourceWriteLease) return;
    const sourceId = this.sourceWriteLease.sourceId;
    if (
      batch.sourceId !== sourceId ||
      basal.some((record) => record.sourceId !== sourceId) ||
      boluses.some((record) => record.sourceId !== sourceId) ||
      context.some((record) => record.sourceId !== sourceId) ||
      dailyTotals.some((record) => record.sourceId !== sourceId) ||
      rawRecords.some((record) => record.sourceId !== sourceId)
    ) {
      throw new SourceConnectionSupersededError(sourceId);
    }
  }
  ...
  async getContextEvents(range: TimeRange) {
    ...
    const [
      ...
      mealItems,
      preferences,
      healthConnectSourceNames,
    ] = await Promise.all([
      database.getAllAsync<ContextRow>(
        `SELECT c.*, h.payload_json AS hevy_payload_json
           FROM context_events c
           LEFT JOIN hevy_workouts h ON h.context_event_id = c.id
          WHERE c.start_ms < ? AND COALESCE(c.end_ms, c.start_ms) >= ?
      ...
      getMealItemsForRange(database, range),
      ...
      getHealthConnectSourceNames(database),
    ]);
    const itemsByContext = mealItemsByContext(mealItems);
    const context = [
      ...rows.map((row) =>
        contextFromRow(
          row,
          itemsByContext.get(row.id),
          healthConnectSourceNames,
        ),
      ),
      ...notes.map(noteFromRow),
    ].sort((a, b) => a.start - b.start);
    const nutrition = await database.getFirstAsync<{ value: string }>('SELECT value FROM app_metadata WHERE key = ?', NUTRITION_SOURCE_KEY);
    let foodSource: string | undefined;
    try { if (nutrition?.value) foodSource = validateNutritionSource(nutrition.value); } catch { /* Old or invalid choices use native food. */ }
    const selected = selectHealthConnectContext(
      context,
      [...preferences.filter(preference => preference.category !== 'nutrition').map((preference) => ({
        category: preference.category,
        preferredSourcePackage:
          preference.preferred_source_package ?? undefined,
      })), { category: 'nutrition', preferredSourcePackage: foodSource?.replace(/^health-connect:/, '') }],
    ).events;
    return selectNutritionSource(mergeHevyHealthConnectActivities(selected).map(presentPersonalObservation), foodSource);
  }

  /** Only saved choices for this source are reused; opening/cancelling a form changes nothing. */
  async getPersonalObservations(kind?: 'lab-result' | 'site-change', limit = 100) {
    const database = await this.getDatabase();
    const rows = await database.getAllAsync<ContextNoteRow>(
      "SELECT * FROM context_notes WHERE origin = 'manual' AND observation_json IS NOT NULL AND (? IS NULL OR CASE WHEN json_valid(observation_json) THEN json_extract(observation_json, '$.kind') END = ?) ORDER BY start_ms DESC LIMIT ?", kind ?? null, kind ?? null, Math.max(1, Math.min(200, limit)),
    );
    return rows.map(noteFromRow).filter(event => event.observation !== undefined);
  }

  async getLastSensorSelection(sourceId?: string) {
    const database = await this.getDatabase();
    const row = await database.getFirstAsync<ContextNoteRow>(
      `SELECT * FROM context_notes WHERE origin = 'manual' AND category = 'sensor'
         AND sensor_started = 1 AND sensor_model_id IS NOT NULL
         AND sensor_glucose_source_id IS ?
       ORDER BY recorded_at_ms DESC, start_ms DESC LIMIT 1`, sourceId ?? null,
    );
  ...
  async getRawSourceRecords(
    ...
    const values: (string | number)[] = [sourceId];
    if (range) {
      clauses.push(
  ...
  async getRecordsByIds(recordIds: readonly string[]) {
    ...
    const database = await this.getDatabase();
    const healthConnectSourceNames =
      await getHealthConnectSourceNames(database);
    for (let offset = 0; offset < recordIds.length; offset += 400) {
      const batch = recordIds.slice(offset, offset + 400);
      const placeholders = batch.map(() => '?').join(',');
      const [
        ...
        await Promise.all([
          ...
          getMealItemsForContextIds(database, batch),
        ]);
      const itemsByContext = mealItemsByContext(mealItemRows);
    ...
    const nutrition = await database.getFirstAsync<{ value: string }>('SELECT value FROM app_metadata WHERE key = ?', NUTRITION_SOURCE_KEY);
    let foodSource: string | undefined;
    try { if (nutrition?.value) foodSource = validateNutritionSource(nutrition.value); } catch { /* Fall back to native food. */ }
    return { basal, boluses, dailyInsulinTotals, context: selectNutritionSource(context.map(presentPersonalObservation), foodSource) };
  }
  ...
  async getManualInsulinBounds(): Promise<StoredRecordBounds> {
    const database = await this.getDatabase();
    const row = await database.getFirstAsync<{
      earliest: number | null;
      latest: number | null;
      ...
      `SELECT MIN(timestamp_ms) AS earliest,
         MAX(timestamp_ms) AS latest,
         COUNT(*) AS count,
         MAX(imported_at_ms) AS last_recorded_at
       FROM insulin_bolus
       WHERE source_id = ? AND delivery_type IN (${MANUAL_INSULIN_DELIVERY_TYPES.map(() => '?').join(', ')})`,
  ...
  async writeImport(
    ...
    await withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, batch.sourceId);
      this.assertImportSourceIds(
        batch,
        basal,
        boluses,
        context,
      ...
      await pruneRetainedGlookoSources(transaction);
  ...
  async saveManualContext(
    ...
    options: ManualContextSaveOptions = {},
  ) {
    ...
    await withT1ArcTransaction(async (database) => {
      await this.assertWriteLease(database, event.sourceId);
      if (event.kind === 'note') {
        await database.runAsync(
        `INSERT INTO context_notes (
           id, source_id, origin, start_ms, end_ms, title, category, detail,
           glucose_mmol_l, recorded_at_ms, source_file, source_row,
           sensor_started, sensor_glucose_source_id, sensor_model_id, sensor_warmup_minutes, observation_json
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           ...
           sensor_model_id = excluded.sensor_model_id,
           sensor_warmup_minutes = excluded.sensor_warmup_minutes,
        ...
        event.sensorStarted === true ? event.sensorModelId ?? null : null,
        event.sensorStarted === true ? event.sensorWarmupMinutes ?? null : null,
        event.observation ? JSON.stringify(validatePersonalObservation(event.observation)) : null,
      );
      } else {
        const values = contextColumns(event);
        await database.runAsync(
      ...
      if (options.insightInvalidation === 'clear-saved-reports') {
        await clearSavedInsightReportsInTransaction(database);
      } else if (options.insightInvalidation === 'mark-dirty') {
  ...
  async deleteManualContext(id: string) {
    ...
    return withT1ArcTransaction(async (database) => {
      await this.assertWriteLease(database);
      ...
      const deleted = eventResult.changes + noteResult.changes > 0;
      if (deleted) {
        await clearSavedInsightReportsInTransaction(database);
      }
      return deleted;
  ...
  async saveManualInsulin(
    delivery: BolusDelivery,
    options: ManualContextSaveOptions = {},
  ) {
    if (!isManualInsulinDelivery(delivery)) {
      throw new Error(
        'Only insulin doses recorded in T1 Arc can be saved through this method.',
      );
    }
    await this.getDatabase();
    await withT1ArcTransaction(async (database) => {
      await this.assertWriteLease(database, delivery.sourceId);
      const result = await database.runAsync(
        `INSERT INTO insulin_bolus (
           id, source_id, timestamp_ms, units, delivery_type,
         ...
         WHERE insulin_bolus.source_id = excluded.source_id`,
        delivery.id,
        delivery.sourceId,
        delivery.timestamp,
        delivery.units,
        delivery.deliveryType ?? null,
        delivery.importedAt ?? Date.now(),
      );
      if (result.changes !== 1) {
        throw new Error('The insulin dose could not be saved safely.');
      }
      if (options.insightInvalidation === 'clear-saved-reports') {
        await clearSavedInsightReportsInTransaction(database);
      } else if (options.insightInvalidation === 'mark-dirty') {
  ...
  async deleteManualInsulin(id: string) {
    await this.getDatabase();
    return withT1ArcTransaction(async (database) => {
      await this.assertWriteLease(database);
      const result = await database.runAsync(
        `DELETE FROM insulin_bolus
         WHERE id = ? AND source_id = ? AND delivery_type IN (${MANUAL_INSULIN_DELIVERY_TYPES.map(() => '?').join(', ')})`,
        id,
        MANUAL_CONTEXT_SOURCE_ID,
        ...MANUAL_INSULIN_DELIVERY_TYPES,
      );
  ...
  async clearImportedSource(
    ...
    await withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, sourceId);
      removed.basal = (
        await transaction.runAsync(
          'DELETE FROM insulin_basal WHERE source_id = ?',
          sourceId,
