import { SQLiteDatabase } from 'expo-sqlite';

import {
  GlucoseHistoryBounds,
  GlucoseHistoryStore,
  SourceSyncState,
  sourceSyncAttemptDue,
  sourceSyncStateMatches,
} from './GlucoseHistoryStore';
import {
  openT1ArcDatabase,
  withT1ArcCriticalTransaction,
  withT1ArcTransaction,
} from './t1arcDatabase';
import {
  assertLocalDataWriteLeaseInTransaction,
  type LocalDataWriteLease,
} from '@/data/privacy/localDataWriteEpoch';
import {
  assertOwnedGlucoseSourceId,
  assertSourceConnectionWriteLeaseInTransaction,
  SourceConnectionSupersededError,
  type SourceConnectionWriteLease,
} from '@/data/live/sourceConnectionOwnership';
import {
  DataQuality,
  GlucoseReading,
  TimeRange,
  TrendDirection,
} from '@/domain/models';

interface GlucoseRow {
  id: string;
  source_id: string;
  timestamp_ms: number;
  received_at_ms: number;
  mmol_l: number;
  trend: TrendDirection;
  quality: DataQuality;
  source_factory_timestamp: string | null;
  source_local_timestamp: string | null;
  timestamp_discrepancy_minutes: number | null;
  imported_at_ms: number | null;
  source_file: string | null;
  source_row: number | null;
  source_device_id: string | null;
}

interface SyncRow {
  source_id: string;
  last_attempt_at_ms: number | null;
  last_success_at_ms: number | null;
  last_error_code: string | null;
  last_error_message: string | null;
  record_count: number;
}

function readingFromRow(row: GlucoseRow): GlucoseReading {
  return {
    id: row.id,
    timestamp: row.timestamp_ms,
    receivedAt: row.received_at_ms,
    mmolL: row.mmol_l,
    trend: row.trend,
    quality: row.quality,
    sourceId: row.source_id,
    sourceFactoryTimestamp: row.source_factory_timestamp ?? undefined,
    sourceLocalTimestamp: row.source_local_timestamp ?? undefined,
    timestampDiscrepancyMinutes:
      row.timestamp_discrepancy_minutes ?? undefined,
    importedAt: row.imported_at_ms ?? undefined,
    sourceFile: row.source_file ?? undefined,
    sourceRow: row.source_row ?? undefined,
    sourceDeviceId: row.source_device_id || undefined,
  };
}

export async function upsertGlucoseReadingsInTransaction(
  transaction: SQLiteDatabase,
  readings: GlucoseReading[],
) {
  const readingBatchSize = 40;
  for (let offset = 0; offset < readings.length; offset += readingBatchSize) {
    const batch = readings.slice(offset, offset + readingBatchSize);
    const values = batch.map(
      () => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    );
    const parameters = batch.flatMap((reading) => [
      reading.id,
      reading.sourceId,
      reading.timestamp,
      reading.receivedAt,
      reading.mmolL,
      reading.trend,
      reading.quality,
      reading.sourceFactoryTimestamp ?? null,
      reading.sourceLocalTimestamp ?? null,
      reading.timestampDiscrepancyMinutes ?? null,
      reading.importedAt ?? null,
      reading.sourceFile ?? null,
      reading.sourceRow ?? null,
      reading.sourceDeviceId ?? '',
    ]);
    await transaction.runAsync(
      `INSERT INTO glucose_readings (
         id, source_id, timestamp_ms, received_at_ms, mmol_l, trend, quality,
         source_factory_timestamp, source_local_timestamp,
         timestamp_discrepancy_minutes, imported_at_ms, source_file,
         source_row, source_device_id
       ) VALUES ${values.join(', ')}
       ON CONFLICT(source_id, timestamp_ms, source_device_id) DO UPDATE SET
         received_at_ms = MIN(
           glucose_readings.received_at_ms, excluded.received_at_ms
         ),
         mmol_l = excluded.mmol_l,
         trend = excluded.trend,
         quality = excluded.quality,
         source_factory_timestamp = excluded.source_factory_timestamp,
         source_local_timestamp = excluded.source_local_timestamp,
         timestamp_discrepancy_minutes =
           excluded.timestamp_discrepancy_minutes,
         imported_at_ms = excluded.imported_at_ms,
         source_file = excluded.source_file,
         source_row = excluded.source_row,
         source_device_id = excluded.source_device_id`,
      ...parameters,
    );
  }
}

export async function clearOwnedGlucoseSourceDataInTransaction(
  transaction: SQLiteDatabase,
  sourceId: string,
) {
  assertOwnedGlucoseSourceId(sourceId);
  await transaction.runAsync(
    'DELETE FROM glucose_readings WHERE source_id = ?',
    sourceId,
  );
  await transaction.runAsync(
    'DELETE FROM source_sync_state WHERE source_id = ?',
    sourceId,
  );
}

export async function commitVerifiedSnapshotInTransaction(
  transaction: SQLiteDatabase,
  sourceId: string,
  readings: GlucoseReading[],
  activatedAt: number,
): Promise<GlucoseHistoryBounds> {
  assertOwnedGlucoseSourceId(sourceId);
  if (readings.some((reading) => reading.sourceId !== sourceId)) {
    throw new SourceConnectionSupersededError(sourceId);
  }
  if (readings.length === 0) return { count: 0 };
  const current = await transaction.getFirstAsync<SyncRow>(
    'SELECT * FROM source_sync_state WHERE source_id = ?',
    sourceId,
  );
  const activationAttemptAt = Math.max(
    activatedAt,
    (current?.last_attempt_at_ms ?? activatedAt - 1) + 1,
  );
  await upsertGlucoseReadingsInTransaction(transaction, readings);
  const boundsRow = await transaction.getFirstAsync<{
    earliest: number | null;
    latest: number | null;
    count: number;
  }>(
    `SELECT MIN(timestamp_ms) AS earliest, MAX(timestamp_ms) AS latest,
            COUNT(*) AS count
       FROM glucose_readings WHERE source_id = ?`,
    sourceId,
  );
  const bounds = {
    earliest: boundsRow?.earliest ?? undefined,
    latest: boundsRow?.latest ?? undefined,
    count: boundsRow?.count ?? 0,
  };
  await transaction.runAsync(
    `INSERT INTO source_sync_state (
       source_id, last_attempt_at_ms, last_success_at_ms, last_error_code,
       last_error_message, record_count
     ) VALUES (?, ?, ?, NULL, NULL, ?)
     ON CONFLICT(source_id) DO UPDATE SET
       last_attempt_at_ms = excluded.last_attempt_at_ms,
       last_success_at_ms = CASE
         WHEN source_sync_state.last_success_at_ms IS NULL
           THEN excluded.last_success_at_ms
         ELSE MAX(
           source_sync_state.last_success_at_ms,
           excluded.last_success_at_ms
         )
       END,
       last_error_code = NULL,
       last_error_message = NULL,
       record_count = excluded.record_count`,
    sourceId,
    activationAttemptAt,
    activatedAt,
    bounds.count,
  );
  return bounds;
}

export async function glucoseHistoryBoundsInTransaction(
  transaction: SQLiteDatabase,
  sourceId: string,
): Promise<GlucoseHistoryBounds> {
  const row = await transaction.getFirstAsync<{
    earliest: number | null;
    latest: number | null;
    count: number;
  }>(
    `SELECT MIN(timestamp_ms) AS earliest, MAX(timestamp_ms) AS latest,
            COUNT(*) AS count
       FROM glucose_readings WHERE source_id = ?`,
    sourceId,
  );
  return {
    earliest: row?.earliest ?? undefined,
    latest: row?.latest ?? undefined,
    count: row?.count ?? 0,
  };
}

export function saveSourceSyncStateInTransaction(
  transaction: SQLiteDatabase,
  state: SourceSyncState,
) {
  return transaction.runAsync(
    `INSERT INTO source_sync_state (
       source_id, last_attempt_at_ms, last_success_at_ms, last_error_code,
       last_error_message, record_count
     ) VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(source_id) DO UPDATE SET
       last_attempt_at_ms = excluded.last_attempt_at_ms,
       last_success_at_ms = excluded.last_success_at_ms,
       last_error_code = excluded.last_error_code,
       last_error_message = excluded.last_error_message,
       record_count = excluded.record_count`,
    state.sourceId,
    state.lastAttemptAt ?? null,
    state.lastSuccessAt ?? null,
    state.lastErrorCode ?? null,
    state.lastErrorMessage ?? null,
    state.recordCount,
  );
}

export class SqliteGlucoseHistoryStore implements GlucoseHistoryStore {
  private database?: SQLiteDatabase;

  constructor(
    private readonly writeLease?: LocalDataWriteLease,
    private readonly sourceWriteLease?: SourceConnectionWriteLease,
  ) {}

  withWriteLease(writeLease: LocalDataWriteLease) {
    if (
      this.sourceWriteLease &&
      this.sourceWriteLease.localDataWriteLease.epoch !== writeLease.epoch
    ) {
      throw new Error(
        'The local write lease does not match the source connection lease.',
      );
    }
    return new SqliteGlucoseHistoryStore(writeLease, this.sourceWriteLease);
  }

  withSourceWriteLease(sourceWriteLease: SourceConnectionWriteLease) {
    if (
      this.writeLease &&
      this.writeLease.epoch !== sourceWriteLease.localDataWriteLease.epoch
    ) {
      throw new Error(
        'The local write lease does not match the source connection lease.',
      );
    }
    return new SqliteGlucoseHistoryStore(
      this.writeLease ?? sourceWriteLease.localDataWriteLease,
      sourceWriteLease,
    );
  }

  private async assertWriteLease(
    transaction: SQLiteDatabase,
    sourceId?: string,
  ) {
    if (this.sourceWriteLease) {
      if (!sourceId) {
        // There is no safe way for a source-owned writer to target an
        // unqualified sink.
        await assertLocalDataWriteLeaseInTransaction(
          transaction,
          this.sourceWriteLease.localDataWriteLease,
        );
        throw new SourceConnectionSupersededError(
          this.sourceWriteLease.sourceId,
        );
      }
      await assertSourceConnectionWriteLeaseInTransaction(
        transaction,
        this.sourceWriteLease,
        sourceId,
      );
    } else if (this.writeLease) {
      await assertLocalDataWriteLeaseInTransaction(
        transaction,
        this.writeLease,
      );
    }
  }

  private async getDatabase() {
    this.database ??= await openT1ArcDatabase();
    return this.database;
  }

  async initialize() {
    await this.getDatabase();
  }

  async upsertReadings(readings: GlucoseReading[]) {
    if (readings.length === 0) return;
    await this.getDatabase();
    await withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(
        transaction,
        this.sourceWriteLease?.sourceId,
      );
      if (
        this.sourceWriteLease &&
        readings.some(
          (reading) => reading.sourceId !== this.sourceWriteLease!.sourceId,
        )
      ) {
        throw new SourceConnectionSupersededError(
          this.sourceWriteLease.sourceId,
        );
      }
      await upsertGlucoseReadingsInTransaction(transaction, readings);
    });
  }

  async commitVerifiedSnapshot(
    sourceId: string,
    readings: GlucoseReading[],
    activatedAt: number,
  ): Promise<GlucoseHistoryBounds> {
    if (readings.length === 0) return { count: 0 };
    await this.getDatabase();
    return withT1ArcCriticalTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, sourceId);
      return commitVerifiedSnapshotInTransaction(
        transaction,
        sourceId,
        readings,
        activatedAt,
      );
    });
  }

  async getReadings(range: TimeRange, sourceId?: string) {
    const database = await this.getDatabase();
    const rows = sourceId
      ? await database.getAllAsync<GlucoseRow>(
          `SELECT * FROM glucose_readings
           WHERE timestamp_ms >= ? AND timestamp_ms < ? AND source_id = ?
           ORDER BY timestamp_ms ASC, source_device_id ASC, id ASC`,
          range.start,
          range.end,
          sourceId,
        )
      : await database.getAllAsync<GlucoseRow>(
          `SELECT * FROM glucose_readings
           WHERE timestamp_ms >= ? AND timestamp_ms < ?
           ORDER BY timestamp_ms ASC, source_device_id ASC, id ASC`,
          range.start,
          range.end,
        );
    return rows.map(readingFromRow);
  }

  async getReadingsByIds(recordIds: readonly string[]) {
    if (!recordIds.length) return [];
    const database = await this.getDatabase();
    const rows: GlucoseRow[] = [];
    for (let offset = 0; offset < recordIds.length; offset += 400) {
      const batch = recordIds.slice(offset, offset + 400);
      const placeholders = batch.map(() => '?').join(',');
      rows.push(
        ...(await database.getAllAsync<GlucoseRow>(
          `SELECT * FROM glucose_readings
           WHERE id IN (${placeholders})
           ORDER BY timestamp_ms ASC`,
          ...batch,
        )),
      );
    }
    return rows.map(readingFromRow).sort((a, b) => a.timestamp - b.timestamp);
  }

  async getLatestReading(sourceId?: string) {
    const database = await this.getDatabase();
    const row = sourceId
      ? await database.getFirstAsync<GlucoseRow>(
          `SELECT * FROM glucose_readings
           WHERE source_id = ?
           ORDER BY timestamp_ms DESC, received_at_ms DESC,
             source_device_id ASC, id ASC LIMIT 1`,
          sourceId,
        )
      : await database.getFirstAsync<GlucoseRow>(
          `SELECT * FROM glucose_readings
           ORDER BY timestamp_ms DESC, received_at_ms DESC,
             source_device_id ASC, id ASC LIMIT 1`,
        );
    return row ? readingFromRow(row) : undefined;
  }

  async getBounds(sourceId?: string): Promise<GlucoseHistoryBounds> {
    const database = await this.getDatabase();
    const row = sourceId
      ? await database.getFirstAsync<{
          earliest: number | null;
          latest: number | null;
          count: number;
        }>(
          `SELECT MIN(timestamp_ms) AS earliest, MAX(timestamp_ms) AS latest,
             COUNT(*) AS count
           FROM glucose_readings WHERE source_id = ?`,
          sourceId,
        )
      : await database.getFirstAsync<{
          earliest: number | null;
          latest: number | null;
          count: number;
        }>(
          `SELECT MIN(timestamp_ms) AS earliest, MAX(timestamp_ms) AS latest,
             COUNT(*) AS count FROM glucose_readings`,
        );
    return {
      earliest: row?.earliest ?? undefined,
      latest: row?.latest ?? undefined,
      count: row?.count ?? 0,
    };
  }

  async getSyncState(sourceId: string) {
    const database = await this.getDatabase();
    const row = await database.getFirstAsync<SyncRow>(
      'SELECT * FROM source_sync_state WHERE source_id = ?',
      sourceId,
    );
    if (!row) return undefined;
    return {
      sourceId: row.source_id,
      lastAttemptAt: row.last_attempt_at_ms ?? undefined,
      lastSuccessAt: row.last_success_at_ms ?? undefined,
      lastErrorCode: row.last_error_code ?? undefined,
      lastErrorMessage: row.last_error_message ?? undefined,
      recordCount: row.record_count,
    };
  }

  async saveSyncState(state: SourceSyncState) {
    await this.getDatabase();
    // Every glucose write must use the keyed transaction lane. A direct write
    // on the shared connection can collide with the short-lived connection
    // used by a foreground/headless Libre claim. On Android SQLCipher that
    // collision can surface while Expo finalises the native statement, after
    // the preceding reading upsert has already committed.
    await withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, state.sourceId);
      await saveSourceSyncStateInTransaction(transaction, state);
    });
  }

  async claimSyncAttempt(
    sourceId: string,
    attemptAt: number,
    minimumIntervalMs: number,
    observedState: SourceSyncState | null,
  ) {
    await this.getDatabase();
    return withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, sourceId);
      const current = await transaction.getFirstAsync<SyncRow>(
        'SELECT * FROM source_sync_state WHERE source_id = ?',
        sourceId,
      );
      const currentState = current
        ? {
            sourceId: current.source_id,
            lastAttemptAt: current.last_attempt_at_ms ?? undefined,
            lastSuccessAt: current.last_success_at_ms ?? undefined,
            lastErrorCode: current.last_error_code ?? undefined,
            lastErrorMessage: current.last_error_message ?? undefined,
            recordCount: current.record_count,
          }
        : undefined;
      if (
        !sourceSyncStateMatches(currentState, observedState) ||
        !sourceSyncAttemptDue(
          currentState?.lastAttemptAt,
          attemptAt,
          minimumIntervalMs,
        )
      ) {
        return false;
      }
      if (current) {
        await transaction.runAsync(
          `UPDATE source_sync_state
              SET last_attempt_at_ms = ?
            WHERE source_id = ?`,
          attemptAt,
          sourceId,
        );
      } else {
        await transaction.runAsync(
          `INSERT INTO source_sync_state (
             source_id, last_attempt_at_ms, record_count
           ) VALUES (?, ?, 0)`,
          sourceId,
          attemptAt,
        );
      }
      return true;
    });
  }

  async completeSyncAttempt(state: SourceSyncState, attemptAt: number, check?: import('@/domain/connectionHistory').ConnectionCheck) {
    await this.getDatabase();
    return withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, state.sourceId);
      const result = await transaction.runAsync(
        `UPDATE source_sync_state
            SET last_success_at_ms = CASE
                  WHEN ? IS NULL THEN last_success_at_ms
                  WHEN last_success_at_ms IS NULL THEN ?
                  ELSE MAX(last_success_at_ms, ?)
                END,
                last_error_code = ?,
                last_error_message = ?,
                record_count = ?
          WHERE source_id = ? AND last_attempt_at_ms = ?`,
        state.lastSuccessAt ?? null,
        state.lastSuccessAt ?? null,
        state.lastSuccessAt ?? null,
        state.lastErrorCode ?? null,
        state.lastErrorMessage ?? null,
        state.recordCount,
        state.sourceId,
        attemptAt,
      );
      if (result.changes > 0 && check) {
        const { appendConnectionCheck, CONNECTION_HISTORY_PREFIX } = await import('@/domain/connectionHistory');
        const key = CONNECTION_HISTORY_PREFIX + state.sourceId;
        const stored = await transaction.getFirstAsync<{ value: string }>('SELECT value FROM app_metadata WHERE key = ?', key);
        const value = appendConnectionCheck(stored?.value, check);
        await transaction.runAsync('INSERT INTO app_metadata (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', key, value);
      }
      return result.changes > 0;
    });
  }

  async pruneReadingsOutsideRange(
    sourceId: string,
    minimumTimestamp: number,
    maximumTimestamp: number,
  ) {
    if (
      !Number.isFinite(minimumTimestamp) ||
      !Number.isFinite(maximumTimestamp) ||
      minimumTimestamp > maximumTimestamp
    ) {
      throw new Error('Glucose history prune range is invalid.');
    }
    await this.getDatabase();
    return withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, sourceId);
      const result = await transaction.runAsync(
        `DELETE FROM glucose_readings
          WHERE source_id = ?
            AND (timestamp_ms < ? OR timestamp_ms > ?)`,
        sourceId,
        minimumTimestamp,
        maximumTimestamp,
      );
      return result.changes;
    });
  }

  async clearSource(sourceId: string) {
    await this.getDatabase();
    await withT1ArcTransaction(async (transaction) => {
      await this.assertWriteLease(transaction, sourceId);
      await transaction.runAsync('DELETE FROM app_metadata WHERE key = ?', `connection-history-v1:${sourceId}`);
      await transaction.runAsync(
        'DELETE FROM glucose_readings WHERE source_id = ?',
        sourceId,
      );
      await transaction.runAsync(
        'DELETE FROM source_sync_state WHERE source_id = ?',
        sourceId,
      );
    });
  }
}
