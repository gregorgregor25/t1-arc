...
} from './healthConnectSyncProgress';
...
export async function saveHealthConnectPreferences(
  ...
  await withHealthConnectWriteLease(lease, async (database) => {
    const current = await database.getAllAsync<{ category: string; enabled: number }>(
      'SELECT category, enabled FROM health_connect_preferences',
    );
    // A retry with the same choices must not invalidate an import already
    // running after the permission screen closes. Only real changes advance
    // the revision fence; the first save still persists all default choices.
    if (
      current.length === DEFAULT_HEALTH_CONNECT_CATEGORIES.length &&
      current.every(row => row.enabled === (enabled.has(row.category as HealthConnectCategoryId) ? 1 : 0))
    ) return;
    await database.runAsync(
...
export async function writeHealthConnectPageInBatches(
  ...
  for (let batchIndex = 0; batchIndex < batchCount; batchIndex += 1) {
    ...
    advanceHealthConnectSyncProgress(batch.length);
  }
  return contextUpdated;
}
...
async function performHealthConnectSync(options?: {
  ...
  beginHealthConnectSyncProgress(categories);
  ...
  for (const category of categories) {
    ...
    setHealthConnectSyncCategory(category);
    ...
    try {
      ...
      completeHealthConnectSyncCategory(category);
    } catch (error) {
      if (
