import { recordSourceCheck } from '@/data/support/diagnosticStore';
...
// Region-specific Share client identifiers, not account credentials.
// https://github.com/gagebenne/pydexcom/blob/main/pydexcom/const.py
const APPLICATION_IDS: Record<DexcomShareRegion, string> = {
  international: 'd89443d2-327c-4a6f-89e5-496bbb0317db',
...
export class DexcomShareGlucoseSource implements GlucoseSource {
  ...
  private async authenticate() {
    ...
    const common = {
      ...
      applicationId: APPLICATION_IDS[this.connection.region],
    };
  ...
  private async importReadings(minutes: number, maxCount: number) {
    ...
    try {
      ...
      recordSourceCheck('dexcom', 'refresh', 'succeeded');
    } catch (error) {
      recordSourceCheck('dexcom', 'refresh', 'failed', { code: errorCode(error) });
