import { labelConventionDefinitions } from './labelConvention';
import {
  FoodCandidate,
  FoodNutrition,
...
export function createUserFoodCandidate(
  ...
  return {
    ...
    nutrientDefinitions: labelConventionDefinitions(draft.labelConvention),
    sourceLabel: draft.labelConvention ? `My foods · ${draft.labelConvention === 'uk-eu' ? 'UK/EU carbohydrate' : draft.labelConvention === 'us' ? 'US total carbohydrate' : 'label convention unknown'}` : 'My foods',
    rawPayload: {
      createdAt,
