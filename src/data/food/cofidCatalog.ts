...
export function searchCofidFoods(query: string, limit = 30) {
  ...
  // For the broad everyday query, put the three pasteurised average entries
  // before specialist/seasonal samples. Explicit searches and all nutrition
  // records remain unchanged; this is relevance, not dietary advice.
  const everydayMilk = rankingQuery === 'milk'
    ? ['cofid:12-313', 'cofid:12-596', 'cofid:12-307'] : [];

  return searchable
    .map((item) => {
      const score = scoreFood(item, rankingQuery, tokens);
      const preferred = everydayMilk.indexOf(item.food.id);
      return { item, score: score < 0 || preferred < 0 ? score : score + 1_000 - preferred * 100 };
    })
