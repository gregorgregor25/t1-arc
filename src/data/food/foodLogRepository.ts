import { writeFoodPhotoInTransaction } from './foodPhotoRepository';
...
export async function insertFoodLogDraftInTransaction(
  ...
  await writeFoodPhotoInTransaction(database, { kind: 'meal', id }, draft.photoData);
...
export async function saveFoodRecipe(
  ...
  await withT1ArcTransaction(async (database) => {
    ...
    await writeFoodPhotoInTransaction(database, { kind: 'recipe', id }, draft.photoData);
  });
...
export async function updateFoodRecipe(
  ...
  await withT1ArcTransaction(async (database) => {
    ...
    await writeFoodPhotoInTransaction(database, { kind: 'recipe', id: recipe.id }, draft.photoData);
  });
...
export async function getFoodLogByIdInTransaction(
  ...
  const log = foodLogFromRows(rows)[0];
  if (log) {
    const photo = await database.getFirstAsync<{ image_data: string }>('SELECT image_data FROM food_photos WHERE log_id = ?', logId);
    if (photo) log.photoData = photo.image_data;
  }
  return log;
}
...
export async function restoreFoodLogInTransaction(
  ...
  await writeFoodPhotoInTransaction(database, { kind: 'meal', id: log.id }, log.photoData);
...
export async function updateFoodLog(
  ...
  await withT1ArcTransaction(async (database) => {
    ...
    await writeFoodPhotoInTransaction(database, { kind: 'meal', id: log.id }, draft.photoData);
    await invalidateFoodInsightsInTransaction(
      database,
      options.insightInvalidation,
    );
