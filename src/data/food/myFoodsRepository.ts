...
export async function createMyFoodInTransaction(
  ...
  await writeFoodPhotoInTransaction(database, { kind: 'food', id: food.id }, draft.photoData);
  return food;
}
...
export async function updateMyFoodInTransaction(
  ...
  await writeFoodPhotoInTransaction(database, { kind: 'food', id: foodId }, draft.photoData);
  return stable;
}
