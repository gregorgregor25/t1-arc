import { APP_TIME_ZONE, type TimeRange } from '@/domain/models';
import {
  addDays,
  isDateKey,
  type DateKey,
...
export interface ResolveTarvisIntentRangeInput {
  intent: Pick<
    TarvisIntentV1,
    'temporalScope' | 'clockWindow' | 'comparison'
  >;
  asOf: number;
  timezone?: string;
}
...
function localParts(timestamp: number): LocalDateTimeParts {
  const parts = localPartsFormatter.formatToParts(timestamp);
...
/**
 * Resolves a London wall-clock timestamp only when it denotes exactly one
 * instant. Spring gaps and autumn folds are rejected instead of guessed.
 */
function resolveStrictLocalDateTime(
  date: DateKey,
  time: Pick<
    LocalDateTimeParts,
    'hour' | 'minute' | 'second' | 'millisecond'
  >,
): number | 'nonexistent' | 'ambiguous' {
  const dateParts = dateKeyParts(date);
  ...
  // Europe/London offsets since Unix epoch are whole minutes. Searching a
  // deliberately wider band also covers British Standard Time history.
  for (let offsetMinutes = -180; offsetMinutes <= 180; offsetMinutes += 1) {
    const candidate = representedAsUtc - offsetMinutes * 60_000;
    if (candidate >= 0 && partsMatch(candidate, expected)) {
      candidates.add(candidate);
    }
  }
  if (candidates.size === 0) return 'nonexistent';
...
function resolveRecentLocalDays(
  ...
  if (!Number.isInteger(scope.count) || scope.count < 1 || scope.count > 3_660) {
    return rejected(
      'invalid_scope',
...
function resolveCalendarDate(
  ...
  const today = toDateKey(asOf);
  if (scope.date > today) {
    return rejected('future_range', 'The requested local date is in the future.');
  }
...
function matchingWallClockEnd(
  ...
  const resolved = resolveStrictLocalDateTime(targetDate, currentTime);
  if (resolved === 'nonexistent') {
    return rejected(
      'nonexistent_local_time',
      'Matching progress lands in a London clock-change gap.',
    );
  }
  if (resolved === 'ambiguous') {
    return rejected(
      'ambiguous_local_time',
      'Matching progress lands in a repeated London clock time.',
    );
  }
  return resolved;
}
...
/**
 * Resolves non-recurring Tarv1s time scopes to exact Europe/London half-open
...
export function resolveTarvisIntentRange(
  ...
  if (timezone !== APP_TIME_ZONE) {
    return rejected(
      'unsupported_timezone',
      `Only ${APP_TIME_ZONE} is supported, not ${timezone}.`,
    );
  }
