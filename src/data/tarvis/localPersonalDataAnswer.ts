...
import { sensorWarmupEvidence } from './sensorWarmupEvidence';
...
function contextResult(
  ...
  const presentedLabel =
    possibleDuplicateMeals > 0 || mealsWithPartialCarbohydrate > 0
      ...
      : label;
  const foodIncomplete = selected === 'food.carbohydrate_total' && summarizeHealthTrendContext(data.context, data.range).foodLoggingIncomplete === true;
  return {
    metric: selected,
    value,
    unit,
    ...
    limitations: [
      ...(foodIncomplete ? ['You marked some food as not logged. This is a total of recorded food only; total intake and fasting cannot be inferred.'] : []),
...
function dataQualityResult(
  ...
  const warmup = sensorWarmupEvidence(data);
  ...
  return {
    ...
    sentence:
      (selected === "data_quality.coverage"
        ...
        : `There ${gapCount === 1 ? "was" : "were"} ${formatTarvisNumber(gapCount, { maximumFractionDigits: 0 })} uncovered sensor gap${gapCount === 1 ? "" : "s"}; the longest was ${formatTarvisFixedNumber(completeness.longestGapMinutes, 1)} minutes.`) + warmup.summary,
    limitations: [
    ...
    recordIds: [...readings.map(({ id }) => id), ...warmup.recordIds],
    examples: [...representative(readings).map(glucosePreview), ...warmup.examples],
    recordLabel: "glucose readings",
