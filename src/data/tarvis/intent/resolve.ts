...
function recentNightCounts(
  ...
  const matches: Array<{ count: number; source: TarvisSourceSpan }> = [];
...
function unconsumedCompoundMetricCue(
  ...
  const compoundCues: Array<[TarvisMetric, RegExp]> = [
    ['glucose.mean', /\b(?:average|avg|mean)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:average|avg|mean)\b/],
    ['glucose.median', /\bmedian\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*median\b/],
    ['glucose.minimum', /\b(?:minimum|lowest)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:minimum|lowest)\b/],
    ['glucose.maximum', /\b(?:maximum|highest)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:maximum|highest)\b/],
    ['glucose.standard_deviation', /\b(?:standard deviation|sd)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:standard deviation|sd)\b/],
    ['glucose.coefficient_of_variation', /\b(?:coefficient of variation|variability|cv)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:coefficient of variation|variability|cv)\b/],
    ['glucose.gmi', /\b(?:gmi|glucose management indicator)\b\s*(?=,|\band\b|\bor\b)|(?:,|\band\b|\bor\b)\s*(?:gmi|glucose management indicator)\b/],
  ];
  return compoundCues.find(
...
function resolveTemporalScope(
  ...
  if (productOvernight?.count !== undefined) {
    if (
      ...
      return {
        problem: 'ambiguous_time_scope',
        message: 'The requested number of nights must be a positive whole number.',
      };
    }
