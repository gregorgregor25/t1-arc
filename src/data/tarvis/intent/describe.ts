...
function describeScope(intent: TarvisIntentV1) {
  ...
  switch (scope.kind) {
    case 'rolling':
      return `Past ${scope.amount} ${plural(scope.amount, scope.unit)}`;
    case 'recent_local_days':
      if (scope.include === 'most_recent_completed_windows') {
        const noun = isOvernightWindow(intent) ? 'overnight window' : 'clock window';
        return `${scope.count} completed ${plural(scope.count, noun)}`;
      }
      if (scope.include === 'completed_days') {
        return `${scope.count} completed local ${plural(scope.count, 'day')}`;
      }
      return `${scope.count} local ${plural(scope.count, 'day')} through now`;
    case 'calendar_period':
      return CALENDAR_PERIOD_LABELS[scope.period];
    case 'calendar_date':
      return scope.date;
    case 'calendar_date_range':
      return `${scope.startDate}–${scope.endDate} inclusive`;
  }
}
...
export function describeTarvisIntent(
  ...
  parts.push(options.timezone ?? DEFAULT_TIMEZONE);
  return parts.join(' · ');
}
