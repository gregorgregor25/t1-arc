...
function extractDates(
  ...
  // A leading four-digit year makes the slash order explicit, including for
  // year-first locales such as ja-JP. Parse it before the locale-dependent
  // trailing-year form so this never relaxes ambiguous DD/MM/YYYY or
  // MM/DD/YYYY input in locales whose numeric order is year-first.
  ...
  const numericDateParts = getCachedDateTimeFormat(locale, {
    year: "numeric",
    month: "numeric",
    day: "numeric",
  }).formatToParts(new Date(Date.UTC(2006, 10, 22)));
  const monthIndex = numericDateParts.findIndex((part) => part.type === "month");
  const dayIndex = numericDateParts.findIndex((part) => part.type === "day");
  const yearIndex = numericDateParts.findIndex((part) => part.type === "year");
  const numericDateOrder =
    monthIndex >= 0 && monthIndex < dayIndex && dayIndex < yearIndex
      ? "month-first"
      : dayIndex >= 0 && dayIndex < monthIndex && monthIndex < yearIndex
        ? "day-first"
        : "unknown";
  for (const match of question.matchAll(/\b(\d{1,2})\/(\d{1,2})\/(\d{4})\b/g)) {
    const raw = match[0];
    const start = match.index;
    if (numericDateOrder === "unknown") continue;
    const first = Number(match[1]);
    const second = Number(match[2]);
    const value = dateKey(
      Number(match[3]),
      numericDateOrder === "month-first" ? first : second,
      numericDateOrder === "month-first" ? second : first,
    );
    if (!value) continue;
    push({
      raw,
  ...
  const monthFirstNamedDatePattern = new RegExp(
    `\\b(${monthPattern})\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:,?\\s+(\\d{4}))?\\b`,
    "gi",
  );
  for (const match of question.matchAll(monthFirstNamedDatePattern)) {
    const raw = match[0];
    const start = match.index;
...
function parseClockToken(raw: string, start: number): TarvisTimeLiteral | null {
  ...
  if (hour === 0 || hour > 12 || trimmed.includes(":")) {
    ...
    if (hour <= 12 && trimmed.includes(":") && hour !== 0 && !hasLeadingZero) {
...
function extractThresholds(
  ...
  for (let index = 0; index < numbers.length - 1; index += 1) {
    ...
    if (
      !adjacentUnit &&
      !/\b(?:range|glucose|readings?|levels?|blood sugar|percentage|percent)\b/.test(
        context,
      )
    ) {
      continue;
...
function extractNegations(question: string): TarvisNegationLiteral[] {
  ...
  const patterns: [RegExp, TarvisNegationLiteral["kind"]][] = [
    [/\bnot\b/gi, "not"],
...
function extractComparisons(question: string): TarvisComparisonLiteral[] {
  ...
  const patterns: [RegExp, TarvisComparisonLiteral["kind"]][] = [
    [/\bcompar(?:e|ed|ing)(?:\s+(?:to|with|against))?\b/gi, "compare"],
    [/\b(?:versus|vs\.?)\b/gi, "versus"],
    [
      /\bprevious\s+(?:equal\s+)?(?:period|window|week|month|days?)\b/gi,
      "previous_period",
    ],
    [
      /\b(?:the\s+)?(?:same\s+)?(?:number\s+of\s+days\s+)?before\s+that\b/gi,
      "previous_period",
    ],
    [/\bbefore\s+and\s+after\b/gi, "before_after"],
    [
      /\b(?:higher|lower|more|less|better|worse)\s+than\b/gi,
      "relative_difference",
    ],
  ];
  for (const [pattern, kind] of patterns) {
...
export function extractTarvisLiterals(
  ...
  const timezone = options.timezone ?? getRuntimeAnalysisTimeZone();
  const locale = options.locale ?? getRuntimeLocale();
  ...
  return {
    ...
    dates: extractDates(question, now, timezone, locale),
