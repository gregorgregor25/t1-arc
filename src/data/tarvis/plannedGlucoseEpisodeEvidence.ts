...
function sensorGapFinding(
  ...
  const warmup = sensorWarmupEvidence(timeline);
  ...
  const evidence: EvidenceReference = {
    ...
    description,
    range: timeline.range,
    recordIds: unique([...represented.map(({ id }) => id), ...warmup.recordIds]),
    examples: [...represented.slice(0, 5).map(glucosePreview), ...warmup.examples],
  };
  return {
    ...
    summary: `${description}. T1 Arc does not assume glucose stayed steady during a gap or infer a glucose change across missing sensor time.${warmup.summary}`,
    caveat:
...
function requestedEvidenceCheckFinding(
  ...
  if (requested.has("data-quality")) {
    ...
    if (checks.has("gaps")) {
      const warmup = sensorWarmupEvidence(timeline);
      if (warmup.summary) summaries.push(warmup.summary.trim());
      recordIds.push(...warmup.recordIds);
      examples.push(...warmup.examples);
      const gaps = sensorGaps(timeline.glucose);
