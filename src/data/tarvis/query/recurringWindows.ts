...
function resolveUniqueLocalBoundary(
  ...
  for (let hours = -36; hours <= 36; hours += 6) {
    possibleOffsets.add(utcOffsetMinutesAt(nominalUtc + hours * 60 * 60_000));
  }
...
export function utcOffsetMinutesAt(timestamp: number) {
  const parts = localParts(timestamp);
  ...
  return (
    (representedAsUtc - Math.floor(timestamp / 1000) * 1000) / 60_000
  );
}

function findOffsetTransition(start: number, end: number) {
  const before = utcOffsetMinutesAt(start);
  const finalTimestamp = end - 1;
  const after = utcOffsetMinutesAt(finalTimestamp);
  ...
  while (high - low > 1) {
    ...
    if (utcOffsetMinutesAt(middle) === before) low = middle;
    else high = middle;
  }
  return {
    atTimestamp: high,
...
export function clockTransitionsForWindow(
  ...
  const transition = findOffsetTransition(range.start, range.end);
 ...
 * from their absolute boundaries, so Europe/London DST nights naturally have
...
export function resolveMostRecentCompletedRecurringWindows(
  ...
  if (input.timezone !== APP_TIME_ZONE) {
    throw new RangeError(`Unsupported timezone: ${input.timezone}.`);
  }
  ...
  let anchorDate = toDateKey(input.asOf);
  let candidate = absoluteRangeForAnchor(anchorDate, input.clockWindow);
  while (candidate.end > input.asOf) {
    anchorDate = addDays(anchorDate, -1);
    candidate = absoluteRangeForAnchor(anchorDate, input.clockWindow);
  }

  const newestFirst: Array<Omit<ResolvedRecurringWindow, 'sequence'>> = [];
  for (let index = 0; index < input.count; index += 1) {
    const range = absoluteRangeForAnchor(anchorDate, input.clockWindow);
    const id = [
      'clock-window',
      anchorDate,
    ...
    newestFirst.push({
      id,
      anchorDate,
      label: `${formatShortDate(anchorDate)} · ${formatClockTime(input.clockWindow.start)}–${formatClockTime(input.clockWindow.end)}`,
      range,
...
export function unwrappedClockMinute(
  ...
  const localDate = toDateKey(timestamp);
