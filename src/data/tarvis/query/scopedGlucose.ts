...
function assertQuery(query: ScopedGlucoseQuery) {
  if (!isIanaTimeZone(query.timezone)) {
    throw new RangeError(`Invalid IANA timezone: ${query.timezone}.`);
  }
...
function normalizedOptions(query: ScopedGlucoseQuery) {
  ...
  const maximumObservedGapMinutes =
    query.maximumObservedGapMinutes ?? DEFAULT_MAXIMUM_OBSERVED_GAP_MINUTES;
  if (
...
function physiologicalSampleMean(
  ...
  return mean(groupReadingsAtTimestamp(readings).map(({ mmolL }) => mmolL));
}
...
function pointForBin(
  ...
  return {
    ...
    recordIds,
    clockOccurrence,
    utcOffsetMinutes: utcOffsetMinutesAt(readings[0]!.timestamp, timeZone),
    elapsedMinute: round(
      samples.reduce(
...
function continuityGroups(
  ...
  readings.forEach((reading) => {
    ...
    const sensorGap = previous
      ...
      : false;
    if (!group || !previous || sensorGap || transition) {
      groups.push({
        readings: [reading],
        startsAfter: {
          sensorGap,
...
function chartSegments(
  ...
  return continuityGroups(readings, maximumObservedGapMilliseconds, window).map(
    (group, index) => ({
...
function aggregatePoints(
  ...
  const bins = new Map<
    string,
    { windowId: string; point: ScopedGlucoseChartPoint }[]
  >();
...
export function executeScopedGlucoseQuery(
  ...
  const missingWindows = intervals.filter(
    (_, index) => windows[index]?.status === "missing",
  );
  const queryId =
...
export function assertScopedGlucoseBundle(bundle: ScopedGlucoseEvidenceBundle) {
  ...
  if (bundle.result.readingCount !== bundle.evidence.records.length) {
    throw new Error(
      "Scoped glucose reading count does not match its evidence.",
    );
  }
  bundle.evidence.records.forEach((record) => {
    ...
    if (
      ...
      throw new Error(
        `Evidence record ${record.id} is outside its resolved scope.`,
      );
    }
  });
  bundle.chart.aggregatePoints.forEach((point) => {
    if (
      point.contributingWindowCount < bundle.chart.minimumAggregateContributors
    ) {
    ...
    point.recordIds.forEach((id) => {
      if (!evidenceIds.has(id)) {
        throw new Error(
          `Aggregate chart record ${id} is not calculation evidence.`,
        );
      }
    });
  });
