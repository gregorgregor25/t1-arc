...
export function resolveTarvisLaunchContext({
  ...
  return {
    // The current demo uses samples relative to today, just like its Today and
    // History screens. Only a deliberately selected historical/saved review
    // anchors relative language inside that report's half-open interval.
    asOf: dataMode === "demo" && isLatestCompletePeriod && reviewId === undefined
      ? now
      : Math.max(report.currentRange.start, report.currentRange.end - 1),
    liveData: false,
    scope: {
      kind: "review",
