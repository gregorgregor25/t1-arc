...
export const GLUCOSE_ANSWER_BUNDLE_IDENTITY_ALGORITHM =
  'daymark-canonical-cyrb128-v1' as const;
...
export interface GlucoseAnswerThresholdV2
...
export interface NormalizedGlucoseIntentV2 {
  ...
  readonly comparison: { kind: 'previous_equal_period' | 'explicit_periods' } | null;
...
export interface GlucoseAnswerBundleV2 {
  ...
  readonly scope: {
    readonly timezone: typeof APP_TIME_ZONE;
...
function canonicalEpisodeReadings(
  ...
  return canonicalGlucoseSamples(
    records.map(calculationReading),
    range,
  ).map((sample) => ({
...
function recomputeClaim(
  ...
  const selectedWindows = claim.windowIds.map((id) => {
    const window = bundle.scope.windows.find((candidate) => candidate.id === id);
  ...
  switch (claim.metric) {
    ...
    case 'glucose.high_readings': {
      ...
      return canonicalGlucoseSamples(
        ...
        ({ mmolL }) => matchesThreshold(mmolL, threshold),
      ).length;
    }
...
export function createGlucoseAnswerBundleV2(
  ...
  invariant(input.timezone === APP_TIME_ZONE, `Only ${APP_TIME_ZONE} is supported.`);
  ...
  invariant(input.originalIntent.domain.value === 'glucose', 'Intent must be glucose.');
  invariant(
  ...
  invariant(input.windows.length > 0, 'At least one calculation window is required.');
  ...
  const windows: GlucoseAnswerWindowV2[] = input.windows.map((window) => {
    ...
    const observationIntervals = window.observationIntervals.map((interval) => ({
  ...
  const scope = {
    ...
    asOf: input.asOf,
    calculationRange: envelope(windows.map(({ calculationRange }) => calculationRange)),
  ...
  const charts: GlucoseAnswerChartReferenceV2[] = (input.charts ?? []).map(
    (chart) => {
      ...
      return {
        ...
        range: envelope(chartWindows.map(({ calculationRange }) => calculationRange)),
...
export function assertGlucoseAnswerBundleV2(
  ...
  invariant(bundle.intent.original.domain.value === 'glucose', 'Intent is not glucose.');
  invariant(
  ...
  invariant(bundle.scope.timezone === APP_TIME_ZONE, 'Bundle timezone is unsupported.');
  invariant(
    Number.isFinite(bundle.scope.asOf) && bundle.scope.asOf >= 0,
    'Bundle asOf is invalid.',
  );
  invariant(bundle.scope.windows.length > 0, 'Bundle has no calculation windows.');
  ...
  invariant(
    bundle.scope.evidenceContextRange.start <= bundle.scope.calculationRange.start &&
      bundle.scope.evidenceContextRange.end >= bundle.scope.calculationRange.end,
  ...
  invariant(
    canonicalString(bundle.intent.normalized) ===
      canonicalString(normalizeIntent(bundle.intent.original, bundle.thresholds)),
  ...
  bundle.records.forEach((record) => {
    ...
    invariant(record.id.trim() && !recordIds.has(record.id), 'Record IDs must be unique.');
    invariant(
  ...
  bundle.scope.windows.forEach((window) => {
    ...
    invariant(window.id.trim() && !windowIds.has(window.id), 'Window IDs must be unique.');
    ...
    assertRange(window.calculationRange, `Window ${window.id} calculation range`);
    assertRange(window.evidenceContextRange, `Window ${window.id} context range`);
    invariant(
    ...
    calculationRecordIds.forEach((id) => {
      const record = recordById.get(id);
      invariant(record, `Window ${window.id} calculation record ${id} is missing.`);
      invariant(
    ...
    contextRecordIds.forEach((id) => {
      ...
      invariant(!calculationIds.has(id), `Window ${window.id} context overlaps calculation IDs.`);
      invariant(
  ...
  invariant(
    sameRange(
      ...
      envelope(bundle.scope.windows.map(({ calculationRange }) => calculationRange)),
    ),
  ...
  invariant(
    sameRange(
      ...
      envelope(
        bundle.scope.windows.map(({ evidenceContextRange }) => evidenceContextRange),
      ),
    ),
  ...
  const recomputedOverallCoverage = coverageFor(
    ...
    bundle.scope.windows.flatMap(({ observationIntervals }) => observationIntervals),
  );
  invariant(
    bundle.coverage.expectedMilliseconds === recomputedOverallCoverage.expectedMilliseconds &&
      bundle.coverage.observedMilliseconds === recomputedOverallCoverage.observedMilliseconds &&
      ...
      bundle.coverage.windowsWithData ===
        bundle.scope.windows.filter(({ calculationRecordIds }) => calculationRecordIds.length)
          .length,
  ...
  bundle.claims.forEach((claim) => {
    ...
    invariant(claim.id.trim() && !claimIds.has(claim.id), 'Claim IDs must be unique.');
    ...
    const expectedCalculationIds = orderedIds(
      idsForWindows(bundle.scope.windows, claim.windowIds, 'calculationRecordIds'),
      bundle.records,
    );
    invariant(
    ...
    const expectedContextIds = isEpisode
      ? orderedIds(
          idsForWindows(bundle.scope.windows, claim.windowIds, 'contextRecordIds'),
          bundle.records,
        )
      : [];
    invariant(
      canonicalString(claim.contextRecordIds) === canonicalString(expectedContextIds) &&
        claim.contextPolicy ===
      ...
      claim.metric === 'glucose.standard_deviation'
        ...
        : claim.metric === 'glucose.time_in_range' ||
          ...
          : claim.metric === 'glucose.low_readings' ||
            ...
            ? 'readings'
          : 'events';
    invariant(claim.unit === expectedUnit, `Claim ${claim.id} has the wrong unit.`);
    const expectedStatus = claim.calculationRecordIds.length
      ? 'available'
      : 'unavailable';
    invariant(
      claim.status === expectedStatus &&
        (expectedStatus === 'unavailable' ? claim.value === null : claim.value !== null),
    ...
    bundle.scope.windows.forEach((window) => {
      invariant(
        bundle.claims.filter(
          (claim) => claim.metric === metric && claim.windowIds.includes(window.id),
        ).length === 1,
  ...
  bundle.charts.forEach((chart) => {
    ...
    invariant(chart.id.trim() && !chartIds.has(chart.id), 'Chart IDs must be unique.');
    chartIds.add(chart.id);
    invariant(chart.kind.trim() && chart.sourceReferenceId.trim(), `Chart ${chart.id} is invalid.`);
    invariant(
    ...
    const exactRecordIds = orderedIds(
      idsForWindows(bundle.scope.windows, chart.windowIds, 'calculationRecordIds'),
      bundle.records,
    );
    invariant(
  ...
  invariant(
    canonicalString([...new Set(bundle.charts.map(({ kind }) => kind))].sort()) ===
