...
/** Move an account-bound item to the restored erase epoch without changing accounts. */
export function recoverOwnerEpoch(origin: string, target: string) {
  if (!isTarvisDatasetOwnerIdentity(origin) || !isTarvisDatasetOwnerIdentity(target)) return origin;
  const parts = origin.split('|');
  parts[1] = target.split('|')[1]!;
  return parts.join('|');
}

/** Empty-store recovery may include several source/account histories and demo threads. */
export function rebindRecoveredTarvisConversationToDatasetOwner(
  value: unknown,
  targetOwnerIdentity: string,
) {
  if (!isTarvisDatasetOwnerIdentity(targetOwnerIdentity)) throw new Error('The target Tarv1s dataset owner is invalid.');
  ...
  const scopes = liveExchanges.flatMap(exchange => exchange.scope && exchange.scope.kind !== 'legacy-unknown' ? [exchange.scope] : []);
  const owners = new Set(scopes.map(scope => scope.ownerIdentity));
  const accounts = new Set(scopes.flatMap(scope => scope.accountId ? [scope.accountId] : []));
  if (owners.size > 1 || accounts.size > 1 || [...owners].some(owner => !isTarvisDatasetOwnerIdentity(owner))) {
    // A backup can legitimately contain older conversations from before a
    // source was connected, or from a different account. Keep those boundaries:
    // only the local erase epoch changes, never the source/account identities.
    // Ambiguous legacy scopes remain byte-for-byte recoverable, not adopted.
    return validateSerializedTarvisConversation(JSON.stringify({
      ...document,
      exchanges: document.exchanges.map(exchange => {
        const scope = exchange.scope;
        if (!scope || scope.kind === 'legacy-unknown' || scope.dataMode !== 'live') return exchange;
        try {
          return { ...exchange, scope: { ...scope, ...rebindScope(scope, recoverOwnerEpoch(scope.ownerIdentity, targetOwnerIdentity)) } };
