...
export function validateTarvisApiKey(value: string, provider: TarvisProvider) {
  ...
  const prefix = provider === "gemini" ? "AIza" : provider === "claude" ? "sk-ant-" : "sk-";
  if (!key.startsWith(prefix) || key.length < 32 || /\s/.test(key) || (provider === "openai" && key.startsWith("sk-ant-"))) {
