...
function omission(
  ...
  return {
    ...reference,
    description: `${reference.description} The chart was omitted because ${formatTarvisNumber(sourcePointCount, { maximumFractionDigits: 0 })} display vertices exceeded the safe on-phone limit of ${formatTarvisNumber(maximumDisplayPoints, { maximumFractionDigits: 0 })}; every exact record ID remains available in All records.`,
...
export function compactTarvisEvidenceReference(
  ...
  const sourcePointCount = reference.visualization.windows.reduce(
    (sum, window) =>
      sum + (window.sampling?.sourceSampleCount ?? window.points.length),
    0,
  );
