...
async function reconcileInterruptedRunsInDatabase(
  database: Awaited<ReturnType<typeof openDaymarkDatabase>>,
  now: number,
) {
...
export async function reconcileInterruptedAutomationRuns(
  now = Date.now(),
) {
...
export async function finishAutomationRun(
  ...
  const stored = await database.getFirstAsync<{
    ...
    'SELECT connector FROM automation_runs WHERE id = ?',
    id,
  );
  if (!stored) return;
  await database.runAsync(
