...
import { HEVY_SYNC_INTERVAL_MS } from '@/data/hevy/policy';
...
export type AutomationTimingState =
  'off' | 'paused' | 'due' | 'waiting' | 'android-controlled';
...
function healthConnectTiming(
  ...
  const enabled = overview.preferences.filter(
    (preference) =>
      preference.enabled &&
      (!grantedCategories || grantedCategories.has(preference.category)),
  );
  if (!enabled.length) {
    return {
      state: 'off',
  ...
  const nextEligibleAt = enabled.reduce<number | undefined>(
    (earliest, preference) => {
      ...
      const categoryDueAt = lastAttempt + HEALTH_CONNECT_FOREGROUND_INTERVAL_MS;
...
function hevyTiming(
  ...
  if (!status.connected) {
    return {
      state: 'off',
      detail: 'Connect Hevy to enable automatic workout imports.',
    };
  }
  const nextEligibleAt =
    status.lastAttemptAt === undefined || status.lastAttemptAt > now
      ? now
      : status.lastAttemptAt + HEVY_SYNC_INTERVAL_MS;
  if (nextEligibleAt <= now) {
    return {
      state: 'due',
      detail: registered
        ? 'Completed workouts are ready to import when Android allows it.'
        : 'Open T1 Arc to check for completed Hevy workouts.',
    };
  }
  return {
    state: 'waiting',
    detail: registered
      ? 'Checks Hevy for completed workouts about every 30 minutes.'
      : 'Checks for completed workouts while T1 Arc is in use.',
    nextEligibleAt,
  };
}
...
export function buildAutomationTiming(input: {
  ...
  return {
    ...
    glooko: glookoTiming(input.glooko, input.registered.glooko, input.now),
    ...
    hevy: hevyTiming(input.hevy, input.registered.hevy, input.now),
