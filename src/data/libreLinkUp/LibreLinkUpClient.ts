...
export function parseLibreTimestamp(
  ...
  if (
    ...
    hourNumber < 1 || hourNumber > 12 ||
    minuteNumber < 0 || minuteNumber > 59 ||
    secondNumber < 0 || secondNumber > 59
  ) {
    return undefined;
  }
  const hour24 = twelveHourTo24Hour(hourNumber, suffix!);
  if (assumeUtc) {
    return Date.UTC(
      Number(year),
      monthNumber - 1,
  ...
  if (!sourceTimeZone) return undefined;
...
export function normaliseLibreMeasurement(
  ...
  const localTimestamp = parseLibreTimestamp(
...
export class LibreLinkUpClient {
  ...
  private async request(
    ...
    if (
      ...
      await this.authenticate(true, 0, signal);
      return this.request(
        endpoint,
        method,
        body,
    ...
    // Abbott can return a regional-routing instruction on data requests too,
    // for example after a VPN changes the phone's apparent country. Keep the
    // existing account/session and retry the same request at the named region.
    // Login redirects retain their separate authentication flow below.
    if (
      endpoint !== LOGIN_ENDPOINT &&
      isRecord(envelope.data) &&
      envelope.data.redirect === true
    ) {
      const region = envelope.data.region;
      ...
      if (regionRedirectCount >= 2 || region === this.session.region) {
        throw new LibreLinkUpError(
          'invalid-response',
          'LibreLinkUp could not route this connection to its regional server. Check your VPN or network and try again.',
        );
      }
  ...
  private async authenticate(
    ...
    if (envelope.data.redirect === true) {
      ...
      await this.persistSession();
      await this.authenticate(true, redirectCount + 1, signal);
      return;
    }
  ...
  private async connections(signal?: AbortSignal): Promise<{
    patients: LibreLinkUpPatient[];
    raw: LibreConnection[];
  }> {
    await this.authenticate(false, 0, signal);
    const envelope = await this.request(
  ...
  private choosePatient(patients: LibreLinkUpPatient[]) {
    if (this.session.patientId) {
      ...
      throw new LibreLinkUpError(
        ...
        'The saved LibreLinkUp patient is no longer available. Reconnect and verify the patient before importing glucose.',
      );
    }
  ...
  async getSnapshot(signal?: AbortSignal): Promise<LibreLinkUpSnapshot> {
    const { patients, raw } = await this.connections(signal);
    const patientId = this.choosePatient(patients);
    this.session.patientId = patientId;
    await this.persistSession();

    const graphEnvelope = await this.request(
      GRAPH_ENDPOINT(patientId),
      'GET',
      undefined,
      true,
      true,
    ...
    const graphConnection = isRecord(graphEnvelope.data.connection)
      ...
      : undefined;
    // The connections and graph endpoints can advance independently. Keep the
    // matching connections measurement even when the graph response contains
    // a connection object, then let timestamp ordering select the newest point.
    // Graph comes last to preserve its existing precedence for an exact-time
    // correction while the final snapshot still deduplicates identical points.
    [rawConnection, graphConnection].forEach((connection) => {
      if (!connection || !isRecord(connection.glucoseMeasurement)) return;
      const current = normaliseLibreMeasurement(connection.glucoseMeasurement);
    ...
    const deduplicated = [
      ...new Map(
        readings.map((reading) => [
          `${reading.sourceId}:${reading.timestamp}`,
          reading,
        ]),
      ).values(),
