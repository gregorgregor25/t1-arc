    ...
    const refresh = (async () => {
      ...
      try {
        ...
        await this.store.completeSyncAttempt(
          ...
          attemptAt,
          { at: Date.now(), outcome: bounds.latest !== undefined && (previousBounds.latest === undefined || bounds.latest > previousBounds.latest) ? 'new-reading' : 'no-new-reading', measurementAt: bounds.latest },
        );
      } catch (error) {
        ...
        await this.store.completeSyncAttempt(
          ...
          attemptAt,
          { at: Date.now(), outcome: 'failed', measurementAt: bounds.latest, reason: normalizedError.code === 'network' ? 'network' : normalizedError.code === 'rate-limited' ? 'rate-limited' : normalizedError.code === 'invalid-credentials' || normalizedError.code === 'action-required' ? 'account' : 'source' },
        );
