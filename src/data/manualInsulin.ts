...
export interface ManualInsulinDraft {
  /** The user confirms this injection is absent from imported pump totals. */
  additionalToPump?: boolean;
  timestamp: number;
  units: number;
...
export const MANUAL_INSULIN_DELIVERY_TYPES = Object.freeze(
  Object.values(DELIVERY_TYPE_BY_INSULIN_TYPE).flatMap(value => [value, `${value} · extra injection`]),
);
...
function insulinTypeFromDeliveryType(
  ...
  return (Object.entries(DELIVERY_TYPE_BY_INSULIN_TYPE) as [
    ManualInsulinType,
    string,
  ][]).find(([, value]) => value === deliveryType || `${value} · extra injection` === deliveryType)?.[0];
}

export function isExtraPumpInjection(delivery: BolusDelivery) {
  return isManualInsulinDelivery(delivery) && delivery.deliveryType?.endsWith(' · extra injection') === true;
}
export function isManualBasalInjection(delivery: BolusDelivery) {
  return isManualInsulinDelivery(delivery) && insulinTypeFromDeliveryType(delivery.deliveryType) === 'long-acting';
}
export function possibleImportedDoseOverlap(delivery: BolusDelivery, deliveries: readonly BolusDelivery[]) {
  return isManualInsulinDelivery(delivery) && !isExtraPumpInjection(delivery) && deliveries.some(other =>
    other.sourceId !== delivery.sourceId && Math.abs(other.timestamp - delivery.timestamp) <= 15 * 60_000 && Math.abs(other.units - delivery.units) < 0.05);
}
...
export function manualInsulinDraftFromDelivery(
  ...
  return {
    ...
    insulinType,
    ...(isExtraPumpInjection(delivery) ? { additionalToPump: true } : {}),
  };
}
...
export function createManualInsulinDelivery(
  ...
  return {
    ...
    units: draft.units,
    deliveryType: draft.additionalToPump ? `${deliveryType} · extra injection` : deliveryType,
