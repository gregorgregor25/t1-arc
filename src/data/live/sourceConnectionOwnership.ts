...
export async function activateSourceConnectionCandidateInTransaction(
  ...
  if (current?.identityDigest !== identityDigest.toLowerCase()) {
    await transaction.runAsync('DELETE FROM app_metadata WHERE key = ?', `connection-history-v1:${candidate.sourceId}`);
  }
