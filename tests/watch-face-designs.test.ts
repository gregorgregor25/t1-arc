...
describe('five-face resource contract', () => {
  ...
  it.each(['atelier', 'summit'])('uses the full %s dial and keeps every hand tip outside the glucose aperture', id => {
    const layout = JSON.parse(execFileSync(process.execPath, ['--input-type=module', '-e',
      `import { ${id}Layout } from './wear/designs/${id}.mjs'; process.stdout.write(JSON.stringify(${id}Layout));`],
    { cwd: process.cwd(), encoding: 'utf8' }));
    expect(layout.centerX).toBe(225);
    expect(layout.centerY).toBe(225);
    expect(layout.radius).toBeGreaterThan(200);
    const { x, y, width, height } = layout.aperture;
    for (const px of [x, x + width]) for (const py of [y, y + height]) {
      expect(Math.hypot(px - layout.centerX, py - layout.centerY))
        .toBeLessThan(Math.min(layout.hour, layout.minute, layout.second) - 8);
    }
    expect(xml(id).indexOf(`name="${id}_hands"`)).toBeLessThan(xml(id).indexOf(`name="${id}_data_aperture"`));
  });
