...
function ready(question: string) {
  ...
  if (!isReadyTarvisIntent(resolution)) throw new Error('Expected ready intent');
...
function authoritativeTotals(
  ...
  for (let index = 0; index < 32; index += 1) {
    ...
    totals.push({
      ...
      importedAt:
        naturalEnd <= range.end ? naturalEnd + 1_000 : range.end,
    });
...
describe('Tarv1s deterministic personal-data executor', () => {
  it('returns a fresh latest glucose with source trend and replayable evidence', () => {
    ...
    expect(result.answer.answer).toContain('source-reported direction was rising');
  ...
  it('labels stale glucose as last-known and does not present its old arrow as current', () => {
    ...
    expect(result.answer.answer).toContain('too old to use as a current direction');
  ...
  it('uses the latest authoritative daily insulin snapshot without double counting', () => {
    ...
    expect(result.answer.answer).toContain('daily totals saved in your records');
  ...
  it.each([
    ['What was my total insulin over the last seven days?', 'Delivered insulin'],
    ['What was my total basal insulin over the last seven days?', 'Basal insulin'],
    ['What was my total bolus insulin over the last seven days?', 'Bolus insulin'],
  ])(
  ...
  it.each([
  ...
  ])('sums only explicit local records for "$question"', ({ question, event, expected }) => {
    const result = execute(question, (range) =>
    ...
    expect(result.answer.limitations.join(' ')).toContain('explicitly recorded');
  });
