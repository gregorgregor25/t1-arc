...
describe("Tarv1s deterministic personal-data executor", () => {
  ...
  it('explains a retrospectively recorded warm-up in gap answers without changing missing coverage', () => {
    const result = execute('any sensor gaps yesterday?', (range) => timeline(range, {
      sources: [source('glucose-source', 'Glucose', { capabilities: GLOOKO_IMPORT_CAPABILITIES })],
      context: [createManualContextEvent({ kind: 'sensor-start', timestamp: range.start + 8 * 3_600_000,
        glucoseSourceId: 'glucose-source', sensorModelId: 'libre-2' }, { id: 'recorded-change', recordedAt: NOW })],
    }));
    expect(result.answer.answer).toContain('consistent with the expected 60-minute warm-up');
    expect(result.answer.answer).toContain('outside these windows remains unexplained');
    expect(result.presentation.windows[0]?.coveragePercent).toBe(0);
    expect(result.evidence[0]?.recordIds).toContain('recorded-change');
  });
