...
describe("TARV1S evidence and spending guardrails", () => {
  ...
  it("rejects model-invented evidence IDs before an answer reaches the UI", async () => {
    const lookup = await evidencePacket();
    ...
    ).toThrow(/exact supporting evidence/i);
  });
  ...
  it("rejects rather than silently truncating an over-cited answer", async () => {
    const lookup = await evidencePacket();
  ...
  it.each([
    "You should take 3 units of insulin now.",
    "Take 3 units now.",
    "Your walk caused the low.",
    "You definitely have DKA.",
  ])("rejects unsafe model-generated personal prose: %s", async (unsafeAnswer) => {
    const lookup = await evidencePacket();
    const validId = lookup.packet.evidence[0]!.id;
    expect(() =>
      parseTarvisAnswer(
        JSON.stringify({
          headline: "Personal review",
          answer: unsafeAnswer,
          confidence: "high",
          evidenceIds: [validId],
          limitations: [],
  ...
  it.each([
    ...
    "Consider lowering your basal tonight.",
    "Have 15 grams of carbohydrate now.",
    "Drink some juice to treat the low.",
    "The walk led to your low.",
    "The exercise triggered the hypo.",
    "Your meal explains the spike.",
    "This is DKA.",
    "It looks like diabetic ketoacidosis.",
  ])("rejects unsafe model-generated education prose: %s", (unsafeAnswer) => {
    expect(() =>
      parseTarvisAnswer(
        JSON.stringify({
  ...
  it.each([[[]], [["duplicate", "duplicate"]]])(
    "rejects missing or duplicate personal citations: %j",
    async (requestedIds) => {
      const lookup = await evidencePacket();
      const validId = lookup.packet.evidence[0]!.id;
      const evidenceIds = requestedIds.length
        ? requestedIds.map(() => validId)
        : [];
      expect(() =>
        parseTarvisAnswer(
          JSON.stringify({
            headline: "Personal review",
            answer: "A personal evidence claim.",
            confidence: "moderate",
            evidenceIds,
            limitations: [],
          }),
  ...
  it("reserves room for both stages of an AI evidence investigation", () => {
    const now = Date.parse("2026-07-28T12:00:00+01:00");
    const usage = {
      requestTimestamps: Array.from(
        { length: 9 },
    ...
    expect(() => checkTarvisRateLimit(usage, now, 2)).toThrow(
      /hourly safety limit/i,
    );
    expect(() => checkTarvisRateLimit(usage, now, 1)).not.toThrow();
  });
});
