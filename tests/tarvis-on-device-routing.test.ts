...
describe("Tarv1s on-device routing", () => {
  ...
  it("treats 'bolus insulin did I take' as one bolus metric", () => {
    const question = "How much bolus insulin did I take yesterday?";
    const resolution = resolveTarvisIntent(question, {
      now: NOW,
      timezone: "Europe/London",
    });
    ...
    expect(routeTarvisIntent(resolution)).toEqual({
