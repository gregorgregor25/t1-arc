...
describe('Tarv1s on-device routing', () => {
  ...
  ])('routes a conflicting scope or invalid threshold to capability: %s', (question) => {
  ...
  ])('fails closed for a likely data query without a personal pronoun: %s', (question) => {
    expect(route(question).kind).not.toBe('legacy');
  });
  ...
  ])('routes an unrepresented qualifier to capability, never scoped or legacy: %s', (question) => {
    expect(route(question).kind).toBe('capability');
  });
  ...
  it('caps the total on-phone scope at 90 days before any data load', () => {
    expect(route('What was my average glucose over the last 90 days?').kind).toBe(
      'scoped-glucose',
    );
    ...
    expect(oversized.answer.limitations[0]).toContain(
      'No glucose records were loaded',
    );
  });
  ...
  it('preserves the existing evidence path for other health questions', () => {
    ...
    expect(route('What is Type 1 diabetes?').kind).toBe('legacy');
  });
});
