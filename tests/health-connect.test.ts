...
import type { HealthConnectRecord } from '../modules/t1arc-health-connect';
import {
...
describe('Health Connect record normalisation', () => {
  ...
  it('keeps a meaningful nutrition summary when carbohydrate is absent', () => {
    expect(
      healthConnectRecordToContext(
        record({
          kind: 'nutrition',
  ...
  it('ignores an empty Health Connect nutrition shell', () => {
    expect(
      healthConnectRecordToContext(
        record({
          kind: 'nutrition',
