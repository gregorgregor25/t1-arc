...
function captureFlow() {
  const scope = {
    ...
    prepareFoodPhoto: vi.fn().mockResolvedValue('data:image/jpeg;base64,preview'), setPreview: vi.fn(),
...
describe('label capture lifecycle interactions', () => {
  ...
  it('allows only one in-flight capture and keeps unreadable error details private', async () => {
    ...
    await pending;
    expect(flow.scope.setError).toHaveBeenLastCalledWith(expect.stringContaining('Try again, or enter the values yourself'));
