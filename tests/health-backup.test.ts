...
describe('health backup validation', () => {
  it('opens version 19 backups without inventing photos or changing existing stream indexes', () => {
    const backup = emptyBackup();
    backup.manifest.version = 19;
    delete backup.tables.food_photos;
    delete backup.manifest.counts.food_photos;
    expect(validateHealthBackupDocument(backup).tables.food_photos).toEqual([]);
    expect(validateCurrentContainerManifest(backup.manifest).counts.food_photos).toBe(0);
    expect(BACKUP_TABLE_NAMES.at(-1)).toBe('food_photos');
  });
  it('preserves lab provenance and migrates version 18 observations without inventing results', () => {
    const backup = emptyBackup();
    const observation = { kind: 'lab-result', test: 'HbA1c', value: 48, unit: 'mmol/mol', laboratory: 'Clinic' };
    const row = { id: 'lab', source_id: 't1arc-manual', origin: 'manual', start_ms: 1720000000000, end_ms: null, title: 'HbA1c', category: 'other', detail: null, recorded_at_ms: 1720000001000, source_file: null, source_row: null, glucose_mmol_l: null, sensor_started: null, sensor_glucose_source_id: null, sensor_model_id: null, sensor_warmup_minutes: null, observation_json: JSON.stringify(observation) };
    backup.tables.context_notes = [row]; backup.manifest.counts.context_notes = 1; backup.manifest.totalRecords = 1;
    expect(validateHealthBackupDocument(backup).tables.context_notes[0]).toEqual(row);
    backup.tables.context_notes = [{ ...row, observation_json: JSON.stringify({ ...observation, unit: '%' }) }];
    expect(() => validateHealthBackupDocument(backup)).toThrow();
    const { observation_json: _observation, ...old } = row;
    backup.manifest.version = 18; backup.tables.context_notes = [old];
    expect(validateHealthBackupDocument(backup).tables.context_notes[0]?.observation_json).toBeNull();
  });
  it('round-trips sensor model and expected duration, and migrates version 17 without guessing', () => {
    const backup = emptyBackup();
    const row = { id: 'warmup', source_id: 't1arc-manual', origin: 'manual', start_ms: 1_720_000_000_000,
      end_ms: 1_720_003_600_000, title: 'Started FreeStyle Libre 2', category: 'sensor', detail: null,
      recorded_at_ms: 1_720_100_000_000, source_file: null, source_row: null, glucose_mmol_l: null,
      sensor_started: 1, sensor_glucose_source_id: 'cgm', sensor_model_id: 'libre-2', sensor_warmup_minutes: 60, observation_json: null };
    backup.tables.context_notes = [row];
    backup.manifest.counts.context_notes = 1;
    backup.manifest.totalRecords = 1;
    expect(validateHealthBackupDocument(backup).tables.context_notes[0]).toEqual(row);
    backup.tables.context_notes = [{ ...row, sensor_warmup_minutes: -1 }];
    expect(() => validateHealthBackupDocument(backup)).toThrow(/invalid sensor warm-up/);
    const { sensor_model_id: _model, sensor_warmup_minutes: _minutes, ...old } = row;
    backup.manifest.version = 17;
    backup.tables.context_notes = [old];
    expect(validateHealthBackupDocument(backup).tables.context_notes[0]).toMatchObject({ sensor_model_id: null, sensor_warmup_minutes: null });
  });
  it('preserves explicit sensor changes and migrates old sensor notes without guessing a start', () => {
    ...
    const row = {
      ...
      sensor_model_id: null, sensor_warmup_minutes: null, observation_json: null,
    };
    backup.tables.context_notes = [row];
    backup.manifest.counts.context_notes = 1;
    backup.manifest.totalRecords = 1;
