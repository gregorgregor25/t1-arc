...
describe("Tarv1s conversation data scopes", () => {
  ...
  it("anchors the current demo to today without enabling personal live-data access", () => {
    const context = resolveTarvisLaunchContext({
      dataMode: "demo", isLatestCompletePeriod: true, now: NOW, report: report(),
    });
    expect(context.asOf).toBe(NOW);
    expect(context.liveData).toBe(false);
    expect(context.scope).toMatchObject({ dataMode: "demo", ownerIdentity: "demo-fixture-v1" });
  });

  it("keeps historical and saved demo reviews anchored to their own period", () => {
    for (const selection of [
      { isLatestCompletePeriod: false },
      { isLatestCompletePeriod: true, reviewId: "saved-demo" },
    ]) {
      const context = resolveTarvisLaunchContext({
        dataMode: "demo", now: NOW, report: report(), ...selection,
      });
      expect(context.asOf).toBe(END - 1);
      expect(context.liveData).toBe(false);
    }
