...
describe('Tarv1s exact non-recurring intent ranges', () => {
  ...
  it.each([
    [
      'today',
      { start: midnight('2026-08-07'), end: AS_OF },
      true,
    ],
    [
      'yesterday',
    ...
    [
      'this_week',
      { start: midnight('2026-08-03'), end: AS_OF },
      true,
    ],
    [
      'last_week',
    ...
    [
      'this_month',
      { start: midnight('2026-08-01'), end: AS_OF },
      true,
    ],
    [
      'last_month',
...
describe('Tarv1s previous-period semantics', () => {
  ...
  it('maps last week to the preceding London calendar week', () => {
    const result = expectResolved(
      resolve(
        { kind: 'calendar_period', period: 'last_week' },
        comparison,
      ),
    );
    expect(result.previous).toEqual({
  ...
  it('maps last month to the complete preceding calendar month', () => {
    const result = expectResolved(
      resolve(
        { kind: 'calendar_period', period: 'last_month' },
        comparison,
      ),
    );
    expect(result.previous).toEqual({
  ...
  it.each([
    ...
    (period, expectedStart, expectedEnd) => {
      ...
      expect(result.comparisonBasis).toBe(
