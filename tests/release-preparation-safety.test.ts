...
describe("release safety boundaries", () => {
  it("rejects instrumentation in the actual Play bundle manifest", () => {
    ...
    expect(packaging).toContain("throw 'Test instrumentation must not be shipped in a Play candidate.'");
    expect(packaging.indexOf("SelectSingleNode('instrumentation')")).toBeLessThan(packaging.indexOf('Copy-Item -LiteralPath $bundle'));
  });
