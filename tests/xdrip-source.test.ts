...
describe('xDrip-compatible glucose source', () => {
  ...
  it('advances an unchanged SGV when xDrip reports a newer observation date', async () => {
    const firstDate = Date.UTC(2026, 7, 25, 13, 0);
    const secondDate = firstDate + 5 * 60_000;
    ...
    await source.refresh();
    observedDate = secondDate;
    observedAt = secondDate + 60_000;
    await source.refresh();

    await expect(
      source.getReadings({ start: firstDate, end: secondDate + 1 }),
    ).resolves.toMatchObject([
      {
        id: `${XDRIP_SOURCE_ID}:${firstDate}:108`,
        timestamp: firstDate,
        mmolL: 6,
      },
      {
        id: `${XDRIP_SOURCE_ID}:${secondDate}:108`,
    ...
    await expect(store.getBounds(XDRIP_SOURCE_ID)).resolves.toEqual({
      earliest: firstDate,
      latest: secondDate,
      count: 2,
    });
