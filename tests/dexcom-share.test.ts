...
describe('Dexcom Share connection', () => {
  it.each([
    ['international', 'shareous1.dexcom.com', 'd89443d2-327c-4a6f-89e5-496bbb0317db'],
    ['us', 'share2.dexcom.com', 'd89443d2-327c-4a6f-89e5-496bbb0317db'],
    ['japan', 'share.dexcom.jp', 'd8665ade-9673-4e27-9ff6-92db4ce13d13'],
  ] as const)('uses the %s Share identifier in both authentication requests', async (region, host, applicationId) => {
    const requests: { url: URL; body: Record<string, unknown> }[] = [];
    const responses = ['account-id', 'session-id', []];
    const source = new DexcomShareGlucoseSource(
      { username: 'publisher@example.test', password: 'test-password', region },
    ...
    expect(requests.every(request => request.url.host === host)).toBe(true);
    expect(requests[0]!.body).toMatchObject({ applicationId, accountName: 'publisher@example.test' });
    expect(requests[1]!.body).toMatchObject({ applicationId, accountId: 'account-id' });
    expect(requests[2]!.url.searchParams.get('sessionId')).toBe('session-id');
  });
