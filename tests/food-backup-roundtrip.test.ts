...
async function seedFoods() {
  const personal = await createMyFood({ name: 'Backup oats', servingAmount: 40, photoData: FOOD_PHOTO,
  ...
  await saveFoodRecipe({ name: 'Private recipe', mealType: 'breakfast', servings: 2, photoData: FOOD_PHOTO,
...
function foodRows(database: TestDatabase) {
  return Object.fromEntries(['food_catalog_cache', 'food_recipes', 'food_recipe_items', 'food_photos'].map((table) =>
