...
describe("planned glucose episode evidence", () => {
  it('includes the recorded warm-up explanation and provenance in a requested gap review', async () => {
    const { all, eventStart } = fixtureWithSustainedHigh();
    all.glucose = all.glucose.filter(({ timestamp }) => timestamp < eventStart + 20 * MINUTE_MS || timestamp >= eventStart + 80 * MINUTE_MS);
    all.context.push(createManualContextEvent({ kind: 'sensor-start', timestamp: eventStart + 20 * MINUTE_MS,
      glucoseSourceId: 'libre', sensorModelId: 'libre-2' }, { id: 'warmup-change', recordedAt: GENERATED_AT }));
    const lookup = await loadPlannedGlucoseEpisodeEvidence({ selectedEventKind: 'high', currentRange: CURRENT_RANGE,
      previousRange: PREVIOUS_RANGE, selectedCategories: ['glucose', 'data-quality'], explicitCategories: ['data-quality'],
      explicitDataQualityChecks: ['gaps'], generatedAt: GENERATED_AT, loadTimelineData: async (range) => sliceTimeline(all, range) });
    const finding = lookup.packet.findings.find(({ id }) => id.endsWith('requested-evidence-availability'));
    expect(finding?.summary).toContain('consistent with the expected 60-minute warm-up');
    expect(finding?.evidenceIds.flatMap((id) => lookup.references.get(id)?.recordIds ?? [])).toContain('warmup-change');
  });
