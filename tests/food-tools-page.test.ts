...
vi.mock('react-native', () => ({
  ...
  Keyboard: { isVisible: () => fixture.keyboardVisible, dismiss: fixture.dismissKeyboard },
...
vi.mock('@/components/KeyboardFormScrollView', () => ({ KeyboardFormScrollView: 'ScrollView' }));
...
beforeEach(() => {
  ...
  fixture.keyboardVisible = false;
  fixture.dismissKeyboard.mockClear();
...
describe('food tools page wrapper', () => {
  it('dismisses the keyboard before leaving without changing the draft', () => {
    const view = render();
    fixture.keyboardVisible = true;
    (view.find('Modal').props.onRequestClose as () => void)();
    expect(fixture.dismissKeyboard).toHaveBeenCalledOnce();
    expect(view.props.onBack).not.toHaveBeenCalled();
  ...
  it.each(['android', 'ios'])('keeps scrollable content inside safe areas and uses %s keyboard behavior', platform => {
    ...
    expect(view.find('KeyboardAvoidingView').props.behavior).toBe(platform === 'ios' ? 'padding' : 'height');
