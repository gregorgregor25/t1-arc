...
describe('compact current glucose trace', () => {
  ...
  it('does not turn repeated identical import points into spurious isolated dots', () => {
    const middle = reading(END - 5 * MINUTE, 6.5);
    const input = [reading(END - 10 * MINUTE, 6), middle, { ...middle, id: 'duplicate-import' }, reading(END, 7)];
    const trace = glucoseTraceGeometry(input, END)!;
    expect(trace.readingCount).toBe(3);
    expect(trace.segments).toHaveLength(2);
    expect(trace.areas).toHaveLength(1);
    expect(trace.isolatedPoints).toHaveLength(0);
