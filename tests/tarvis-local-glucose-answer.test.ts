...
describe("local Tarv1s recurring-window glucose answers", () => {
  it("answers the screenshot question from only the exact three completed windows", () => {
    ...
    expect(result.presentation.windows[0]).toMatchObject({
      recordCount: 4,
      metrics: [expect.objectContaining({ id: "average-glucose", value: 7.5 })],
    });

    expect(buildLocalGlucoseAnswer({ asOf: AS_OF, intent, readings })).toEqual(
      result,
    );
  });
  ...
  it("includes when a recurring-window maximum happened", () => {
    const intent = ready(
      "What was my highest glucose over the last two days between midnight and 7 a.m.?",
    );
    const result = buildLocalGlucoseAnswer({
      asOf: AS_OF,
      intent,
      readings: [
        reading("ordinary", "2026-08-06T01:00:00+01:00", 6.1),
        reading("highest", "2026-08-07T06:20:00+01:00", 14.4),
      ],
    });

    expect(result.answer.answer).toContain("14.4 mmol/L");
    expect(result.answer.answer).toContain(
      "at 06:20 on Friday, 7 August 2026",
    );
  });
  ...
  it.each([
    ...
    "uses the shared episode semantics with an exact custom $role threshold",
    (fixture) => {
      ...
      expect(
        result.evidence.calculation?.episodeDefinitionVersion,
      ).toBeTruthy();
  ...
  it.each([
    ...
    "turns a DST boundary failure into a safe capability answer",
    ({ asOf, message, wording }) => {
  ...
  it("persists daylight-saving fold wording separately from missing sensor data", () => {
    ...
    const visualization = clockVisualizationOf(result);
    expect(visualization.coverageSummary).toContain("repeats 60 local minutes");
    expect(visualization.coverageSummary).toContain("not missing sensor data");
  });
});
