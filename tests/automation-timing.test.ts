...
function registered(
  overrides: Partial<
    Record<
      'glucose' | 'glooko' | 'health-connect' | 'hevy' | 'insight-review',
      boolean
    >
  > = {},
) {
...
describe('automatic connector timing', () => {
  it('distinguishes Glooko eligibility from Android execution', () => {
    const timing = buildAutomationTiming({
      ...
      hevy: { connected: false, workoutCount: 0 },
      now: NOW,
  ...
  it('does not let an enabled but ungranted category look overdue', () => {
    ...
    expect(timing['health-connect']).toEqual({
      ...
      detail:
        'Checks every 5 minutes while you use T1 Arc, and automatically when Android allows it.',
      nextEligibleAt: recentAttempt + 5 * 60 * 1000,
    });
  });
  ...
  it('shows the next Hevy import check from durable source state', () => {
    const lastAttemptAt = NOW - 10 * 60 * 1000;
    const timing = buildAutomationTiming({
      glooko: DEFAULT_GLOOKO_SYNC_STATE,
      healthConnect: healthOverview(),
      healthConnectAccess: { available: true, readGranted: false },
      ...
      now: NOW,
      registered: registered({ hevy: true }),
    });

    expect(timing.hevy).toEqual({
      state: 'waiting',
      detail: 'Checks Hevy for completed workouts about every 30 minutes.',
      nextEligibleAt: lastAttemptAt + 30 * 60 * 1000,
    });
  });

  it('does not claim Hevy automation before an account is connected', () => {
    const timing = buildAutomationTiming({
      glooko: DEFAULT_GLOOKO_SYNC_STATE,
      healthConnect: healthOverview(),
      healthConnectAccess: { available: true, readGranted: false },
      hevy: { connected: false, workoutCount: 8 },
      now: NOW,
      registered: registered({ hevy: true }),
    });
