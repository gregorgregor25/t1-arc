...
describe("resolveMostRecentCompletedRecurringWindows", () => {
  it("resolves the latest completed half-open local windows chronologically", () => {
    ...
    expect(windows.every((window) => window.elapsedMinutes === 420)).toBe(true);
  ...
  it("fails closed when a requested boundary is nonexistent or repeated", () => {
    ...
    expect(() =>
      ...
      /occurs more than once because of a daylight-saving clock change/,
    );
  });
});
...
describe("executeScopedGlucoseQuery", () => {
  it("answers the screenshot case from exactly three windows, two with data", () => {
    ...
    expect(
      bundle.evidence.records.every(
        (item) => item.timestamp < london("2026-08-06", 7),
      ),
    ).toBe(true);
    expect(
      bundle.evidence.missingWindows.map((item) => item.anchorDate),
    ).toEqual(["2026-08-04"]);
  ...
  it("reports sparse coverage using capped elapsed observation time", () => {
    ...
    const bundle = executeScopedGlucoseQuery(
      overnightQuery({ windowCount: 1 }),
      [reading("a", start, 6), reading("b", start + 5 * 60_000, 7)],
    );
  ...
  it("preserves both occurrences of an autumn repeated clock bin", () => {
    ...
    expect(
      bundle.chart.aggregatePoints.map((point) => point.clockBinKey),
    ).toEqual(["90:occurrence:1", "90:occurrence:2"]);
    expect(bundle.safeguards.repeatedClockBins).toBe("preserved-by-occurrence");
  });
  ...
  it("pre-segments chart lines across gaps instead of bridging them", () => {
    ...
    expect(
      bundle.windows[0]!.segments.map((segment) => segment.points.length),
    ).toEqual([1, 1]);
    expect(bundle.safeguards.lineGaps).toBe("pre-segmented-never-bridged");
  });
