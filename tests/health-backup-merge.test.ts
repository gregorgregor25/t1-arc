...
import { RECOVERED_OWNER_PROOF_PREFIX } from '@/data/tarvis/recoveredOwnerProof';
...
function streamedNotificationBackup(rows: Record<string, any>[], sourceVersion = HEALTH_BACKUP_VERSION) {
  ...
  const sourceCounts = { ...backup.manifest.counts } as Record<string, number>;
  if (sourceVersion < 20) delete sourceCounts.food_photos;
  const manifest = encoder.encode(JSON.stringify({ ...backup.manifest, counts: sourceCounts, version: sourceVersion }));
...
function streamedPortableConversationMigration(value: string) {
  ...
  const encoder = new TextEncoder();
  const legacyCounts = { ...document.manifest.counts } as Record<string, number>;
  delete legacyCounts.food_photos;
  const manifest = encoder.encode(JSON.stringify({ ...document.manifest, counts: legacyCounts, version: 16 }));
  const payload = encoder.encode(JSON.stringify(row));
...
function streamedVersionedBackup(options: {
  ...
  if (options.version < 20) delete backup.manifest.counts.food_photos;
...
describe('health backup additive merge', () => {
  ...
  it.each(['legacy', 'stream'] as const)('recovers disconnected source identity from the %s manifest', async kind => {
    const sourceOwners = { 't1arc-librelinkup': 'a'.repeat(64) };
    const original = emptyBackup();
    const document = validateHealthBackupDocument({ ...original, manifest: { ...original.manifest, sourceOwners } });
    if (kind === 'legacy') await mergeHealthBackup(document);
    else {
      const bytes = new TextEncoder().encode(JSON.stringify(document.manifest));
      const uri = 'file:///source-owner-recovery.container';
      mocks.files.set(uri, joinBytes([new TextEncoder().encode('T1ARCCN1'), encodeBackupFrameHeader(1, 255, bytes.length), bytes, encodeBackupFrameHeader(0, 255, 0)]));
      await mergePreparedHealthBackup(await readHealthBackupFile(uri));
    }
    expect(JSON.parse(mocks.metadata.get('glucose-source-connection-ownership-v1:t1arc-librelinkup')!))
      .toEqual({ version: 1, changeGeneration: 0, ownerGeneration: 0, connected: false, identityDigest: sourceOwners['t1arc-librelinkup'] });
  });
  ...
  it.each(['legacy', 'stream'] as const)('recovers conversation ownership into an empty store after erase (%s)', async kind => {
    ...
    expect(JSON.parse(mocks.metadata.get(RECOVERED_OWNER_PROOF_PREFIX+TARVIS_CONVERSATION_STORAGE_KEY)!))
      .toMatchObject({version:1,originOwner:sourceOwner,recoveryOwner:targetOwner,ids:['exchange-scoped']});
  });
  ...
  it('restores mixed account conversations without adopting either account or discarding history', async () => {
    mocks.metadata.set(LOCAL_DATA_WRITE_EPOCH_KEY, '7');
    const ownerA = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 3, ownedSources: [{ sourceId: 't1arc-librelinkup', identityDigest: 'a'.repeat(64) }] });
    const ownerB = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 3, ownedSources: [{ sourceId: 't1arc-librelinkup', identityDigest: 'b'.repeat(64) }] });
    const document = JSON.parse(scopedConversation(200, ownerA));
    const other = JSON.parse(scopedConversation(200, ownerB)).exchanges[0];
    document.exchanges.push({ ...other, id: 'other-account', threadId: 'other-thread' });
    await expect(mergePreparedHealthBackup(streamedPortableConversationMigration(JSON.stringify(document)))).resolves.toMatchObject({inserted: 1});
    const restored = JSON.parse(mocks.metadata.get(TARVIS_CONVERSATION_STORAGE_KEY)!);
    expect(restored.exchanges).toHaveLength(2);
    expect(restored.exchanges.map((entry: {scope: {ownerIdentity: string}}) => entry.scope.ownerIdentity)).toEqual([
      ownerA.replace('epoch:3', 'epoch:7'), ownerB.replace('epoch:3', 'epoch:7'),
    ]);
    expect(restored.exchanges.map((entry: {answer: unknown}) => entry.answer)).toEqual(document.exchanges.map((entry: {answer: unknown}) => entry.answer));
    expect(mocks.metadata.has(RECOVERED_OWNER_PROOF_PREFIX + TARVIS_CONVERSATION_STORAGE_KEY)).toBe(false);
  });

  it('restores a mixed-owner notebook while preserving account separation', async () => {
    mocks.metadata.set(LOCAL_DATA_WRITE_EPOCH_KEY, '7');
    const entries = ['a', 'b'].map((letter, index) => ({ id: `note-${index}`, ownerIdentity: resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 3, ownedSources: [{ sourceId: 't1arc-librelinkup', identityDigest: letter.repeat(64) }] }), dataMode: 'live', createdAt: 1_750_000_000_000, title: 'Saved observation', answer: 'Keep this answer', note: 'My note', limitations: [], evidence: [] }));
    const backup = emptyBackup();
    backup.tables.portable_app_state = [{ key: NOTEBOOK_STORAGE_KEY, value: JSON.stringify({ version: 1, entries }) }];
    backup.manifest.counts.portable_app_state = backup.manifest.totalRecords = 1;
    await expect(mergeHealthBackup(validateHealthBackupDocument(backup))).resolves.toMatchObject({inserted: 1});
    expect(JSON.parse(mocks.metadata.get(NOTEBOOK_STORAGE_KEY)!).entries).toEqual(entries.map(entry => ({ ...entry, ownerIdentity: entry.ownerIdentity.replace('epoch:3', 'epoch:7') })));
    expect(mocks.metadata.has(RECOVERED_OWNER_PROOF_PREFIX + NOTEBOOK_STORAGE_KEY)).toBe(false);
  });
  ...
  it.each(['legacy', 'stream'] as const)('repairs an authenticated pre-proof restore without replacing newer messages (%s)', async kind => {
    const digest = 'a'.repeat(64);
    const ownedSources = [{ sourceId: 't1arc-librelinkup' as const, identityDigest: digest }];
    const origin = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 0, ownedSources });
    const recovery = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 7, ownedSources: [] });
    const current = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 7, ownedSources });
    mocks.metadata.set(LOCAL_DATA_WRITE_EPOCH_KEY, '7');
    mocks.metadata.set('glucose-source-connection-ownership-v1:t1arc-librelinkup', JSON.stringify({ version: 1, changeGeneration: 1, ownerGeneration: 1, connected: true, identityDigest: digest }));
    const source = scopedConversation(200, origin);
    const restored = JSON.parse(rebindRecoveredTarvisConversationToDatasetOwner(source, recovery));
    const newer = JSON.parse(rebindRecoveredTarvisConversationToDatasetOwner(scopedConversation(400, current), current)).exchanges[0];
    newer.id = 'newer-chat'; newer.threadId = 'newer-thread';
    mocks.metadata.set(TARVIS_CONVERSATION_STORAGE_KEY, JSON.stringify({ ...restored, updatedAt: 400, exchanges: [...restored.exchanges, newer] }));
    const merge = async () => {
      if (kind === 'stream') return mergePreparedHealthBackup(streamedPortableConversationMigration(source));
      const backup = emptyBackup();
      backup.tables.portable_app_state = [{ key: TARVIS_CONVERSATION_STORAGE_KEY, value: source }];
      backup.manifest.counts.portable_app_state = backup.manifest.totalRecords = 1;
      return mergeHealthBackup(validateHealthBackupDocument(backup));
    };
    await merge();
    const result = mocks.metadata.get(TARVIS_CONVERSATION_STORAGE_KEY)!;
    const parsed = JSON.parse(result);
    expect(parsed.updatedAt).toBe(400);
    expect(parsed.exchanges[0].scope.ownerIdentity).toBe(current);
    expect(parsed.exchanges[0].answer.answer).toBe('A preserved answer.');
    expect(parsed.exchanges[1]).toEqual(newer);
    expect(JSON.parse(mocks.metadata.get(RECOVERED_OWNER_PROOF_PREFIX + TARVIS_CONVERSATION_STORAGE_KEY)!)).toMatchObject({ originOwner: origin, recoveryOwner: recovery, ids: ['exchange-scoped'] });
    await merge();
    expect(mocks.metadata.get(TARVIS_CONVERSATION_STORAGE_KEY)).toBe(result);
  });

  it.each(['account', 'content', 'epoch'] as const)('does not infer pre-proof recovery continuity from a matching ID after a %s change', async changed => {
    const digest = 'a'.repeat(64);
    const origin = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 0, ownedSources: [{ sourceId: 't1arc-librelinkup', identityDigest: digest }] });
    const recovery = resolveTarvisDatasetOwnerIdentity({ dataMode: 'live', localDataEpoch: 7, ownedSources: [] });
    mocks.metadata.set(LOCAL_DATA_WRITE_EPOCH_KEY, changed === 'epoch' ? '8' : '7');
    mocks.metadata.set('glucose-source-connection-ownership-v1:t1arc-librelinkup', JSON.stringify({ version: 1, changeGeneration: 1, ownerGeneration: 1, connected: true, identityDigest: changed === 'account' ? 'b'.repeat(64) : digest }));
    const source = scopedConversation(200, origin);
    const local = JSON.parse(rebindRecoveredTarvisConversationToDatasetOwner(source, recovery));
    local.updatedAt = 400;
    if (changed === 'content') local.exchanges[0].answer.answer = 'Different answer';
    const before = JSON.stringify(local);
    mocks.metadata.set(TARVIS_CONVERSATION_STORAGE_KEY, before);
    await mergePreparedHealthBackup(streamedPortableConversationMigration(source));
    expect(mocks.metadata.get(TARVIS_CONVERSATION_STORAGE_KEY)).toBe(before);
    expect(mocks.metadata.has(RECOVERED_OWNER_PROOF_PREFIX + TARVIS_CONVERSATION_STORAGE_KEY)).toBe(false);
  });

  it('rolls back instead of overwriting newer conversations when recovery ownership cannot be read', async () => {
    const before = conversation(400);
    mocks.metadata.set(TARVIS_CONVERSATION_STORAGE_KEY, before);
    mocks.metadata.set('glooko-sync-state-v1', 'null');
    await expect(mergeHealthBackup(backupWithConversation(200))).rejects.toThrow(/owner cannot be verified/);
    expect(mocks.metadata.get(TARVIS_CONVERSATION_STORAGE_KEY)).toBe(before);
  });
