...
function reading(id: string, timestamp: string | number, mmolL: number): GlucoseReading {
  const instant = typeof timestamp === 'number' ? timestamp : Date.parse(timestamp);
  return {
    id,
    timestamp: instant,
    receivedAt: instant,
...
describe('expanded deterministic exact-range glucose metrics', () => {
  ...
  it('calculates every sample statistic from one canonical sample per timestamp', () => {
    ...
    expect(result.answer.limitations.join(' ')).toContain('shorter than 14 days');
  ...
  it('keeps no-data sample statistics and reading counts unavailable, never zero', () => {
    ...
    const intent = withMetrics(base, ['glucose.low_readings'], 'count_readings');
    const result = buildLocalGlucoseRangeAnswer({
      asOf: AS_OF,
      intent,
      readings: [],
  ...
  it('calculates comparison medians independently for the exact periods', () => {
    const intent = withMetrics(
      ready('Compare my average glucose over the last 24 hours with the previous period.'),
      ['glucose.median'],
      'aggregate',
    );
    const result = buildLocalGlucoseRangeAnswer({
    ...
    expect(result.answerBundle.claims.map(({ metric, value }) => ({ metric, value })))
      .toEqual([
        { metric: 'glucose.median', value: 11 },
        { metric: 'glucose.median', value: 7 },
      ]);
    expect(result.evidence.every(({ visualization }) => visualization === undefined))
      .toBe(true);
  });
  ...
  it('treats 14 complete London calendar days across spring-forward as 14 days', () => {
    ...
    for (let timestamp = start, index = 0; timestamp < end; timestamp += 10 * 60_000, index += 1) {
...
describe('expanded metric language and fail-closed routing', () => {
  it.each([
    ['What was the median of my glucose over the last 30 days?', 'glucose.median'],
    ['What was my median readings over the last 30 days?', 'glucose.median'],
    ['What was my lowest glucose value over the last 30 days?', 'glucose.minimum'],
    ['What was my minimum reading over the last 30 days?', 'glucose.minimum'],
    ['What was my highest glucose value over the last 30 days?', 'glucose.maximum'],
    ['What was my maximum reading over the last 30 days?', 'glucose.maximum'],
    ['How variable were my readings over the last 30 days?', 'glucose.coefficient_of_variation'],
    ['What was the standard deviation of my glucose over the last 30 days?', 'glucose.standard_deviation'],
    ['What was my glucose CV over the last 30 days?', 'glucose.coefficient_of_variation'],
    ['What was my estimated GMI over the last 30 days?', 'glucose.gmi'],
    ['How many low readings below 4 mmol/L did I have over the last 30 days?', 'glucose.low_readings'],
    ['How many low-glucose readings did I have over the last 30 days?', 'glucose.low_readings'],
    ['How many high reading above 12 mmol/L did I have over the last 30 days?', 'glucose.high_readings'],
    ['How many high-glucose reading did I have over the last 30 days?', 'glucose.high_readings'],
    ...
    expect(isReadyTarvisIntent(resolution)).toBe(true);
    if (!isReadyTarvisIntent(resolution)) return;
    expect(resolution.intent.metrics.map(({ value }) => value)).toEqual([metric]);
  ...
  it('recognises shared low/high reading counts with both profile thresholds', () => {
    ...
    expect(resolution.intent.thresholds.map(({ value }) => value)).toMatchObject([
    ...
    expect(result.answerBundle.claims.map(({ value }) => value)).toEqual([1, 1]);
  });
