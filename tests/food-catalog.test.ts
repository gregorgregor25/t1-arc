...
describe('offline UK food catalogue', () => {
  ...
  it('offers everyday pasteurised milk first without hiding explicit specialist searches', () => {
    expect(searchCofidFoods('milk', 3).map(food => food.name)).toEqual([
      'Milk, semi-skimmed, pasteurised, average',
      'Milk, whole, pasteurised, average',
      'Milk, skimmed, pasteurised, average',
    ]);
    expect(searchCofidFoods('human milk', 5).every(food => /human/i.test(food.name))).toBe(true);
    expect(searchCofidFoods('sheeps milk', 1)[0]?.name).toBe('Milk, sheeps, raw');
    expect(searchCofidFoods('milk', 100).some(food => /colostrum/i.test(food.name))).toBe(true);
  });
