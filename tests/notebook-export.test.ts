...
const mocks = vi.hoisted(() => ({ save: vi.fn(), write: vi.fn(), create: vi.fn(), remove: vi.fn(), workingFile: vi.fn(), checkLease: vi.fn() }));
vi.mock("expo-crypto", () => ({ randomUUID: () => "unique-report" }));
vi.mock("expo-file-system", () => ({
  Paths: { cache: "file:///private/cache" },
  File: class {
    exists = true;
    uri = "file:///private/no_backup/encrypted-backups/backup-notebook-unique-report.html";
    create = mocks.create;
    write = mocks.write;
    delete = mocks.remove;
  },
}));
vi.mock("../modules/t1arc-backup-crypto", () => ({ default: {
  createWorkingFileAsync: mocks.workingFile,
  saveTemporaryFileAsync: mocks.save,
} }));
...
beforeEach(() => {
  ...
  mocks.workingFile.mockResolvedValue("file:///private/no_backup/encrypted-backups/backup-notebook-unique-report.html");
});
...
describe("explicit printable notebook export", () => {
  it("writes the previewed HTML into the native-allowlisted private directory and removes it after saving", async () => {
    ...
    expect(mocks.workingFile).toHaveBeenCalledWith('.html');
