...
const READY_CASES: ReadyCase[] = [
  ...
  },
  {
    ...
    question: 'What was the frequency of sustained low-glucose events this month?',
    metrics: ['glucose.low_episodes'],
    operation: 'count_episodes',
    scope: calendar('this_month'),
  },
  ...
  },
  {
    ...
    question: 'What was the frequency of sustained high-glucose events this month?',
    metrics: ['glucose.high_episodes'],
    operation: 'count_episodes',
    scope: calendar('this_month'),
  },
  ...
  },
  {
    ...
    question: 'How many lows and how many highs did I have over the last 14 days?',
...
const FAIL_CLOSED_CASES: FailClosedCase[] = [
  ...
  },
  {
    ...
    question: 'What was my average glucose over the last 3 days between 10 and 3?',
    status: 'needs_clarification',
  ...
  },
  {
    ...
    question: 'What was my average glucose after breakfast over the last seven days?',
    status: 'needs_clarification',
    code: 'ambiguous_time_scope',
    recognisedMetrics: ['glucose.mean'],
  },
  {
    ...
    question: 'What was my average glucose over the last seven days excluding today?',
    status: 'needs_clarification',
    code: 'ambiguous_time_scope',
    recognisedMetrics: ['glucose.mean'],
  },
...
function resolve(question: string, history?: readonly TarvisIntentHistoryEntry[]) {
...
describe('Tarv1s adversarial ready-question corpus', () => {
  it.each(READY_CASES)('$name', (testCase) => {
    ...
    expect(
      resolution.intent.thresholds.map((item) => item.value),
    ).toEqual(testCase.thresholds ?? defaultThresholds(testCase.metrics));
  });
});
...
describe('Tarv1s corrections and conversational follow-ups', () => {
  ...
  it('uses an explicit new metric instead of the historical metric', () => {
    ...
    expect(resolution.intent.temporalScope.value).toEqual(calendar('last_week'));
  });
