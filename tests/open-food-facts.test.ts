...
function unthrottledOptions(signal?: AbortSignal) {
  return {
    signal,
    rateGate: createOpenFoodFactsRateGate({
...
describe('Open Food Facts barcode adapter', () => {
  ...
  it('requests only the fields needed by the logger', async () => {
    ...
    await lookupOpenFoodFactsBarcode(
      '3017624010701',
      fetcher,
      unthrottledOptions(),
    );
  ...
  it('sends explicit non-UK country and language context for barcode lookup', async () => {
    const fetcher = vi.fn(async () =>
      new Response(JSON.stringify(productFixture), { status: 200 }),
    );
    const food = await lookupOpenFoodFactsBarcode(
      '3017624010701',
      fetcher,
      {
        ...unthrottledOptions(),
    ...
    expect(url).toContain('cc=jp');
    expect(url).toContain('lc=ja');
    expect(url).toContain('product_name_ja');
    expect(food.rawPayload).toMatchObject({
      _t1arcRegionalContext: { countryCode: 'JP', languageTag: 'ja' },
    });
  });

  it('retries one transport failure without retrying service responses', async () => {
    const recoveringGate = {
      waitForTurn: vi.fn(async () => undefined),
    ...
    await expect(
      lookupOpenFoodFactsBarcode(
        '3017624010701',
        recoveringFetch,
        { rateGate: recoveringGate },
      ),
    ).resolves.toMatchObject({ name: 'Nutella' });
    expect(recoveringFetch).toHaveBeenCalledTimes(2);
    expect(recoveringGate.waitForTurn).toHaveBeenCalledTimes(2);
    expect(recoveringGate.waitForTurn).toHaveBeenNthCalledWith(
      1,
      'product',
      undefined,
    );
    ...
    await expect(
      lookupOpenFoodFactsBarcode(
        '3017624010701',
        unavailableFetch,
        { rateGate: unavailableGate },
      ),
    ).rejects.toMatchObject({ code: 'service' });
    expect(unavailableFetch).toHaveBeenCalledTimes(1);
    expect(unavailableGate.waitForTurn).toHaveBeenCalledTimes(1);
  });
  ...
  it('cancels a queued search before making a physical request', async () => {
    ...
    const search = searchOpenFoodFactsProducts('baked beans', fetcher, {
      ...unthrottledOptions(controller.signal),
    });
    controller.abort();
