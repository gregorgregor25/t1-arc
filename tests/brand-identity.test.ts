import {
  existsSync,
  readFileSync,
  readdirSync,
  statSync,
} from 'node:fs';
import path from 'node:path';

import { describe, expect, it } from 'vitest';

const root = process.cwd();

function filesUnder(directory: string): string[] {
  if (!statSync(directory, { throwIfNoEntry: false })) return [];
  return readdirSync(directory).flatMap((entry) => {
    const fullPath = path.join(directory, entry);
    if (entry === 'build' || entry === 'node_modules') return [];
    return statSync(fullPath).isDirectory()
      ? filesUnder(fullPath)
      : [fullPath];
  });
}

function text(file: string) {
  return readFileSync(file, 'utf8');
}

function userVisibleAndroidStrings() {
  return [
    ...filesUnder(path.join(root, 'android', 'app', 'src', 'main', 'res')),
    ...filesUnder(path.join(root, 'modules')),
    ...filesUnder(path.join(root, 'wear')),
  ].filter(
    (file) =>
      file.endsWith(`${path.sep}values${path.sep}strings.xml`) &&
      file.includes(`${path.sep}src${path.sep}main${path.sep}`),
  );
}

describe('T1 Arc product identity', () => {
  it('uses T1 Arc for private package metadata and first-party notices', () => {
    const packageConfig = JSON.parse(
      text(path.join(root, 'package.json')),
    ) as {
      name: string;
      repository?: { type?: string; url?: string };
      bugs?: { url?: string };
    };
    const packageLock = JSON.parse(
      text(path.join(root, 'package-lock.json')),
    ) as { name: string; packages: Record<string, { name?: string }> };
    const notices = text(path.join(root, 'THIRD_PARTY_NOTICES.md'));

    expect(packageConfig.name).toBe('t1-arc');
    expect(packageConfig.repository).toEqual({
      type: 'git',
      url: 'git+https://github.com/gregorgregor25/t1-arc.git',
    });
    expect(packageConfig.bugs?.url).toBe(
      'https://github.com/gregorgregor25/t1-arc/issues',
    );
    expect(packageLock.name).toBe('t1-arc');
    expect(packageLock.packages['']?.name).toBe('t1-arc');
    expect(notices).toContain("T1 Arc's first-party LibreLinkUp connector");
    for (const license of [
      't1arc-backup-crypto',
      't1arc-glooko-export',
      't1arc-glucose-display',
      't1arc-notification-source',
    ]) {
      expect(text(path.join(root, 'modules', license, 'LICENSE'))).toContain(
        'Permission is hereby granted',
      );
    }
  });

  it('uses T1 Arc as the installed phone and Expo product name', () => {
    const appConfig = JSON.parse(
      text(path.join(root, 'app.json')),
    ) as {
      expo: {
        name: string;
        slug: string;
        scheme: string | string[];
        description: string;
      };
    };
    const androidStringsPath = path.join(
      root,
      'android',
      'app',
      'src',
      'main',
      'res',
      'values',
      'strings.xml',
    );

    expect(appConfig.expo.name).toBe('T1 Arc');
    expect(appConfig.expo.slug).toBe('t1-arc');
    expect(
      Array.isArray(appConfig.expo.scheme)
        ? appConfig.expo.scheme
        : [appConfig.expo.scheme],
    ).toContain('t1arc');
    expect(appConfig.expo.description).toContain('Type 1 diabetes');
    if (existsSync(androidStringsPath)) {
      expect(text(androidStringsPath)).toContain(
        '<string name="app_name">T1 Arc</string>',
      );
    }
  });

  it('keeps Expo, npm and native Android release versions aligned', () => {
    const appConfig = JSON.parse(
      text(path.join(root, 'app.json')),
    ) as {
      expo: {
        version: string;
        android: { versionCode: number };
      };
    };
    const packageConfig = JSON.parse(
      text(path.join(root, 'package.json')),
    ) as { version: string };
    const androidGradlePath = path.join(
      root,
      'android',
      'app',
      'build.gradle',
    );

    expect(appConfig.expo.version).toBe(packageConfig.version);
    expect(appConfig.expo.android.versionCode).toBeGreaterThan(0);
    if (existsSync(androidGradlePath)) {
      const androidGradle = text(androidGradlePath);
      const nativeVersionName = androidGradle.match(
        /\bversionName\s+["']([^"']+)["']/,
      )?.[1];
      const nativeVersionCode = Number(
        androidGradle.match(/\bversionCode\s+(\d+)/)?.[1],
      );
      expect(nativeVersionName).toBe(packageConfig.version);
      expect(appConfig.expo.android.versionCode).toBe(nativeVersionCode);
    }
  });

  it('uses the spaced product name in user-visible Android and Wear strings', () => {
    const offenders = userVisibleAndroidStrings().flatMap((file) => {
      const values = [
        ...text(file).matchAll(/<string\b[^>]*>([\s\S]*?)<\/string>/g),
      ].map((match) => match[1] ?? '');
      return values.some((value) => /\bT1Arc\b/.test(value))
        ? [path.relative(root, file)]
        : [];
    });

    expect(offenders).toEqual([]);
  });

  it('keeps technical camel case out of rendered phone and Wear copy', () => {
    const renderedSourceFiles = [
      ...filesUnder(path.join(root, 'src', 'components')),
      ...filesUnder(path.join(root, 'src', 'screens')),
      ...filesUnder(path.join(root, 'plugins')),
      ...filesUnder(path.join(root, 'modules')),
      ...filesUnder(path.join(root, 'wear')),
      ...filesUnder(path.join(root, 'android', 'app', 'src', 'main')),
    ].filter(
      (file) =>
        (file.endsWith('.tsx') ||
          file.endsWith('.ts') ||
          file.endsWith('.js') ||
          file.endsWith('.kt') ||
          file.endsWith('.xml')) &&
        (!file.includes(`${path.sep}modules${path.sep}`) ||
          file.includes(
            `${path.sep}android${path.sep}src${path.sep}main${path.sep}`,
          )) &&
        (!file.includes(`${path.sep}wear${path.sep}`) ||
          file.includes(`${path.sep}src${path.sep}main${path.sep}`)),
    );
    const technicalCamelCase = /\bT1Arc\b/g;
    const offenders = renderedSourceFiles.flatMap((file) => {
      const matches = [...text(file).matchAll(technicalCamelCase)].map(
        (match) => match[0],
      );
      return matches.length
        ? [`${path.relative(root, file)}: ${matches.join(', ')}`]
        : [];
    });

    expect(offenders).toEqual([]);
  });

  it('uses display-form T1 Arc throughout product documentation', () => {
    const productDocuments = [
      path.join(root, 'README.md'),
      ...filesUnder(path.join(root, 'docs')).filter((file) =>
        file.endsWith('.md'),
      ),
      path.join(root, 'design-system', 't1arc', 'MASTER.md'),
    ];
    const offenders = productDocuments.flatMap((file) =>
      /\bT1Arc\b/.test(text(file))
        ? [path.relative(root, file)]
        : [],
    );

    expect(text(path.join(root, 'README.md'))).toMatch(/^# T1 Arc\b/);
    expect(offenders).toEqual([]);
  });
});
