...
describe('synthetic repository', () => {
  ...
  it('keeps demo glucose finite with Persian numeral presentation', async () => {
    ...
    const source = new SyntheticGlucoseSource(
      Date.parse('2026-08-29T12:00:00+03:30'),
    );

    expect((await source.getLatestReading())?.mmolL).toSatisfy(Number.isFinite);
  });
});
