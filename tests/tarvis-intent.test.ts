...
describe("Tarv1s literal extraction", () => {
  it("preserves written and digit numbers with their exact source spans", () => {
    ...
    expect(
      literals.numbers.map(({ raw, value, notation }) => ({
        raw,
        value,
        notation,
      })),
  ...
  it("extracts durations including compact 24h and number words", () => {
    ...
    expect(
      literals.durations.map(({ raw, value, unit }) => ({ raw, value, unit })),
    ).toEqual([
  ...
  it("extracts absolute, UK-format, named, and relative local dates", () => {
    ...
    expect(
      literals.dates.map(({ raw, date, kind, yearWasInferred }) => ({
        raw,
        date,
        kind,
        yearWasInferred,
  ...
  it("extracts midnight, noon, 7 a.m., units, thresholds, negation, and comparison", () => {
    ...
    expect(literals.times.map((time) => time.minuteOfDay)).toEqual([
      0, 720, 420,
    ]);
...
describe("Tarv1s high-priority intent resolution", () => {
  it("resolves the reported screenshot question exactly", () => {
    ...
    expect(resolution.literals.numbers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ raw: "three", value: 3 }),
      ]),
    );
  });

  it("uses US month-first numeric dates and accepts month-name-first dates", () => {
    const literals = extractTarvisLiterals(
      "on 08/27/2026, 03/04/2026, August 6 and August 7, 2026",
      {
        now: NOW,
    ...
    expect(literals.dates.map(({ raw, date }) => ({ raw, date }))).toEqual([
      { raw: "08/27/2026", date: "2026-08-27" },
      { raw: "03/04/2026", date: "2026-03-04" },
      { raw: "August 6", date: "2026-08-06" },
      { raw: "August 7, 2026", date: "2026-08-07" },
    ]);
  });

  it("does not guess a trailing-year slash convention for a year-first locale", () => {
    const literals = extractTarvisLiterals("on 03/04/2026", {
      now: NOW,
      timezone: "Asia/Tokyo",
      locale: "ja-JP",
    });

    expect(literals.dates).toEqual([]);
  });

  it("accepts an unambiguous year-first slash date for a Japanese locale", () => {
    const literals = extractTarvisLiterals("on 2026/08/27", {
      now: NOW,
      timezone: "Asia/Tokyo",
      locale: "ja-JP",
    });
  ...
  it("inherits only through an explicit follow-up form and records provenance", () => {
    const previous = expectReady(
      "What was my average glucose in the last 30 days?",
    );
  ...
  it("inherits the exact glucose metric and period for a temporal result follow-up", () => {
    const previous = expectReady(
      "What was my highest glucose reading in the last 14 days?",
    );
    const result = resolveTarvisIntent("But when was that reading?", {
      now: NOW,
      timezone: "Europe/London",
      history: [
        {
    ...
    if (!isReadyTarvisIntent(result)) throw new Error("Expected ready");
    expect(result.intent.metrics[0]).toMatchObject({
      value: "glucose.maximum",
      provenance: { kind: "conversation", turnId: "turn-maximum" },
    });
    expect(result.intent.temporalScope).toMatchObject({
      value: previous.intent.temporalScope.value,
      provenance: { kind: "conversation", turnId: "turn-maximum" },
    });
  });
  ...
  it.each([
    ...
    [
      "How many hypos have I had over the last seven days?",
      "glucose.low_episodes",
    ],
    [
      "How many spikes did I have over the last seven days?",
      "glucose.high_episodes",
    ],
    ["What was my TIR over the last seven days?", "glucose.time_in_range"],
    [
      "What percent of time was in my target range over the last seven days?",
  ...
  it("resolves a custom time-in-range band without confusing it with clock time", () => {
    ...
    expect(
      result.intent.thresholds.map((threshold) => threshold.value),
    ).toEqual([
  ...
  it.each([
    ...
    "rejects a metric-incompatible threshold before producing a ready intent: %s",
    (question) => {
...
describe("Tarv1s temporal semantics", () => {
  it("distinguishes rolling 72 hours from three local calendar days", () => {
    const rolling = expectReady(
      "What was my average glucose over the past 72 hours?",
    );
    const localDays = expectReady(
      "What was my average glucose over the last three days?",
    );
  ...
  it("resolves compact rolling durations at their exact boundary", () => {
    const result = expectReady(
      "What was my average glucose over the last 24h?",
    );
  ...
  it.each([
    ...
    "fails closed instead of dropping an unrepresented filter: %s",
    (question, code) => {
      expect(resolve(question).outcome).toMatchObject({
        status: "needs_clarification",
        code,
  ...
  it("can deterministically infer morning after the midnight keyword", () => {
    ...
    expect(result.intent.clockWindow?.value.end).toEqual({
      hour: 7,
      minute: 0,
    });
  ...
  it.each([
    ...
    const result = resolve(
      `What was my average glucose over the last three days ${clockText}?`,
    );
    expect(result.outcome).toMatchObject({
      status: "needs_clarification",
      code,
  ...
  it.each([
    ...
    "fails closed when non-equivalent time scopes are combined: %s",
    (question) => {
      expect(resolve(question).outcome).toMatchObject({
        status: "needs_clarification",
        code: "ambiguous_time_scope",
...
describe("Tarv1s fail-closed capability outcomes", () => {
  ...
  it("recognises newly executable metrics exactly instead of answering a nearby one", () => {
    const median = expectReady(
      "What was my median glucose over the last 30 days?",
    );
    expect(median.intent.metrics[0]?.value).toBe("glucose.median");

    const samples = expectReady(
      "How many readings were low over the last 30 days?",
    );
  ...
  it("rejects numbered month scopes rather than silently treating them as 90 days", () => {
    const result = resolve(
      "What was my average glucose over the last three months?",
    );
    expect(result.outcome).toMatchObject({
      status: "unsupported",
      code: "unsupported_time_scope",
...
describe("Tarv1s strict intent contract", () => {
  ...
  it("rejects missing nullable fields and additional properties", () => {
    const result = expectReady(
      "What was my average glucose over the last seven days?",
    );
    const { clockWindow: _clockWindow, ...missingNullable } = result.intent;
    const withExtra = { ...result.intent, invented: true };

    expect(validateTarvisIntentV1(missingNullable)).toMatchObject({
      valid: false,
    });
  ...
  it("rejects invalid clock boundaries even when the object shape looks plausible", () => {
    ...
    expect(validation.errors).toEqual(
      expect.arrayContaining([
        expect.stringContaining("does not match any allowed shape"),
      ]),
    );
  });
});
