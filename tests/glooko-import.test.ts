...
2026-07-01 00:00:00,Scheduled,30,,0.6,,PDM-ANONYMISED`,
  ...
  it('converts explicitly labelled US bolus glucose inputs to canonical mmol/L', () => {
    const preview = parseGlookoTextFiles(
      [
        {
          name: 'bolus_data_us.csv',
          text: `Timestamp,Bolus Amount,Blood Glucose Input (mg/dL)
08/20/2026 12:00,2.0,126`,
        },
      ],
      IMPORTED_AT,
      { dateOrder: 'month-first', timeZone: 'America/New_York' },
    );

    expect(preview.boluses).toHaveLength(1);
    expect(preview.boluses[0]?.bloodGlucoseInputMmolL).toBeCloseTo(7, 1);
  });
...
2026-07-01 21:15:00,5.8,Yes,METER-ANONYMISED`,
  ...
  it('converts explicitly labelled US meter checks to canonical mmol/L', () => {
    const preview = parseGlookoTextFiles(
      [
        {
          name: 'bg_data_us.csv',
          text: `Timestamp,Glucose Value (mg/dL),Manual reading
08/20/2026 12:05,126,Yes`,
        },
      ],
      IMPORTED_AT,
      { dateOrder: 'month-first', timeZone: 'America/New_York' },
    ...
    expect(preview.context).toEqual([
      expect.objectContaining({ glucoseMmolL: 7, title: 'Blood glucose check' }),
    ]);
  });

  it.each([39, 40])(
    'rejects ambiguous unitless CGM glucose %s instead of guessing its unit',
    (value) => {
      const preview = parseGlookoTextFiles(
        [
          {
            name: 'cgm_data_unitless.csv',
            text: `Timestamp,Glucose
2026-07-01 12:00:00,${value}`,
          },
        ],
        IMPORTED_AT,
      );
      ...
      expect(preview.glucose).toHaveLength(0);
      expect(preview.recognisedFiles[0]).toMatchObject({ skippedRows: 1 });
    },
  );
...
2026-07-01 23:59:00,18.2,22.4,40.6`,
  ...
  it('interprets naive timestamps in Europe/London across BST', () => {
    ...
    expect(
      parseGlookoTimestamp('Sat, 01 Aug 2026 12:00:00 GMT'),
    ).toBe(Date.parse('2026-08-01T12:00:00Z'));
    expect(
      parseGlookoTimestamp('Sat, 01 Aug 2026 12:00:00'),
    ).toBeUndefined();
  });

  it('uses the supplied timezone for aggregate insulin timestamps and date keys', () => {
    const preview = parseGlookoTextFiles(
      [
        {
          name: 'insulin_data_1.csv',
...
2026-07-01 23:59:00,18.2,22.4,40.6`,
    ...
    expect(preview.dailyInsulinTotals).toEqual([
      expect.objectContaining({
        timestamp: Date.parse('2026-07-02T03:59:00Z'),
        dateKey: '2026-07-01',
      }),
    ]);
  });
...
describe('encrypted health-record store contract', () => {
  ...
  it('only permits deleting user-created context', async () => {
    ...
    expect(await store.deleteManualContext('t1arc-manual:meal:1')).toBe(true);
    expect(await store.deleteManualContext('t1arc-manual:meal:1')).toBe(
      false,
    );
  });
  ...
  it('normalises US Glooko month-first timestamps in the configured IANA zone', () => {
    const settings = {
      timeZone: 'America/New_York',
      dateOrder: 'month-first' as const,
    };
    expect(parseGlookoTimestamp('08/27/2026 09:15:00', settings)).toBe(
      Date.parse('2026-08-27T13:15:00.000Z'),
    );
    const preview = parseGlookoTextFiles(
      [
        {
          name: 'bolus_data.csv',
          text: `Timestamp,Bolus Type,Dose (units),Carbs (g),Notes\n08/27/2026 09:15:00,Normal,3.5,32,Breakfast`,
        },
      ],
      IMPORTED_AT,
      settings,
    ...
    expect(preview.unsafeTimestampLocale).toBe(false);
    expect(preview.boluses).toHaveLength(1);
    expect(preview.boluses[0]?.timestamp).toBe(
      Date.parse('2026-08-27T13:15:00.000Z'),
    );
  });

  it('fails closed for Lord Howe half-hour folds and gaps', () => {
    const settings = {
      timeZone: 'Australia/Lord_Howe',
      dateOrder: 'day-first' as const,
    };

    expect(parseGlookoTimestamp('05/04/2026 01:45:00', settings)).toBeUndefined();
    expect(parseGlookoTimestamp('04/10/2026 02:15:00', settings)).toBeUndefined();

    const folded = parseGlookoTextFiles(
      [
        {
          name: 'bolus_data.csv',
          text: `Timestamp,Insulin delivered (U)\n05/04/2026 01:45:00,1.0`,
        },
      ],
      IMPORTED_AT,
      settings,
    );
    expect(folded.unsafeTimestampLocale).toBe(true);
    expect(folded.boluses).toHaveLength(0);
    expect(folded.warnings).toContain(
      'bolus_data.csv: a repeated local hour in Australia/Lord_Howe could not be uniquely disambiguated from source row order; the file was not normalised.',
    );

    const skipped = parseGlookoTextFiles(
      [
        {
          name: 'bolus_data.csv',
          text: `Timestamp,Insulin delivered (U)\n04/10/2026 02:15:00,1.0`,
        },
      ],
      IMPORTED_AT,
      settings,
    );
    expect(skipped.unsafeTimestampLocale).toBe(true);
    expect(skipped.boluses).toHaveLength(0);
    expect(skipped.warnings).toContain(
      'bolus_data.csv: a local time in Australia/Lord_Howe was skipped by a clock change; the file was not normalised.',
    );
  });
});
