...
describe('bounded Health evidence loading', () => {
  ...
  it.each([
  ...
  ])('sends today\'s selected Sleep from $name through the actual launch scope and repository', async ({ isLatestCompletePeriod, reviewId, currentClock }) => {
    ...
    const launch = resolveTarvisLaunchContext({ dataMode: 'demo', isLatestCompletePeriod, reviewId,
      now: datasetClock, ownerIdentity: 'demo-fixture-v1', report: backgroundReport });
    expect(launch.liveData).toBe(false);
    expect(launch.asOf).toBe(currentClock ? datasetClock : selected.start - 1);
    ...
    // Current demo questions use today. A historical/saved review still needs
    // the selected-record clock before it can inspect today's Sleep record.
    expect(coordinateTarvisContextRequest({ ...request, asOf: launch.asOf }))
      .toMatchObject(currentClock
        ? { kind: 'model-evidence', evidenceRanges: { current: selected } }
        : { kind: 'answer', source: 'evidence-range' });
    ...
    expect(mocks.health).not.toHaveBeenCalled();
    // Without a selected record, ordinary questions retain their launch clock.
