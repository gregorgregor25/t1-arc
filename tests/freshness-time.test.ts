...
describe('glucose freshness', () => {
  ...
  it('tolerates small device clock skew but rejects a clearly future reading', () => {
    expect(glucoseFreshness(now + 2 * MINUTE, now)).toBe('current');
    expect(relativeAge(now + 2 * MINUTE, now)).toBe('Just now');

    expect(glucoseFreshness(now + 3 * MINUTE, now)).toBe('stale');
    expect(relativeAge(now + 3 * MINUTE, now)).toBe('3 min ahead');
  });

  it('uses the active locale digits for relative ages', () => {
    setRuntimeRegionalProfile({
      ...DEFAULT_REGIONAL_PROFILE,
      languageTag: 'ar-EG',
    });

    expect(relativeAge(now - 12 * MINUTE, now)).toBe('١٢ min ago');
    expect(relativeAge(now + 3 * MINUTE, now)).toBe('٣ min ahead');
  });
});
