...
describe('daily Health Connect metrics', () => {
  it('exposes complete nutrition context without inventing missing carbohydrate', () => {
    expect(
      nutritionMetricsFromContextSummary({
        mealCount: 1,
      ...
      nutritionMealCount: 1,
      nutritionCarbohydrateGrams: undefined,
      nutritionEnergyKcal: 440,
      nutritionProteinGrams: 32,
      nutritionFatGrams: 18,
      nutritionFibreGrams: 6,
      nutritionSugarsGrams: 4,
  ...
  it('does not turn records from another day into measured zeroes', () => {
    const metrics = aggregateDailyHealthMetrics(
      [
        record('steps', 7_342, 'com.example.health', 25 * HOUR, 26 * HOUR),
        record('weight', 81.9, 'com.example.health', -HOUR, -HOUR),
        record('heart_rate', 72, 'com.example.health', 24 * HOUR, 24 * HOUR),
      ],
      range,
    );

    expect(metrics.steps).toBeUndefined();
    expect(metrics.weightKilograms).toBeUndefined();
    expect(metrics.averageHeartRateBpm).toBeUndefined();
    expect(metrics.recordCount).toBe(0);
    expect(metrics.selectedRecordIds).toEqual([]);
  });
  ...
  it('never combines blood-pressure components from different measurements', () => {
    const mismatchedTime = aggregateDailyHealthMetrics(
      [
        record(
    ...
    const mismatchedIdentity = aggregateDailyHealthMetrics(
      [
        {
          ...record(
            'blood_pressure_systolic',
    ...
    const ambiguousTimestamp = aggregateDailyHealthMetrics(
      [
        {
          ...record(
            'blood_pressure_systolic',
    ...
    expect(mismatchedTime.bloodPressureSystolic).toBeUndefined();
    expect(mismatchedTime.bloodPressureDiastolic).toBeUndefined();
    expect(mismatchedIdentity.bloodPressureSystolic).toBeUndefined();
    expect(mismatchedIdentity.bloodPressureDiastolic).toBeUndefined();
    expect(ambiguousTimestamp.bloodPressureSystolic).toBeUndefined();
    expect(ambiguousTimestamp.bloodPressureDiastolic).toBeUndefined();
  });

  it('uses the latest complete blood-pressure measurement pair', () => {
    const metrics = aggregateDailyHealthMetrics(
      [
        {
          ...record(
          ...
          measurementId: 'complete-pressure',
        },
        {
          ...record(
            'blood_pressure_diastolic',
