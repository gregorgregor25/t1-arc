...
import { buildInsightReport, InsightWindowSummary } from "@/domain/insights";
...
describe("Tarv1s response-card evidence presentation", () => {
  ...
  it("recognises lows and highs as event-count aliases", () => {
    ...
    expect(
      presentation?.windows[0]?.metrics.map((metric) => metric.id),
    ).toEqual(["low-events", "high-events"]);
  });
  ...
  it("uses a semantic range breakdown and labels comparison windows", () => {
    ...
    expect(
      presentation?.windows[0]?.metrics.map((metric) => metric.id),
    ).toEqual(["time-below-range", "time-in-range", "time-above-range"]);
  ...
  it("drops every hosted field when sparse coverage replaces the prose", () => {
    const sparsePacket = packet();
    sparsePacket.comparison.current = summary({
      glucoseReadings: 240,
      coveragePercent: 42.5,
    });
    const result = applyTarvisCoverageGuardrailResult(
      "What was my average glucose?",
      sparsePacket,
      {
        ...answer(),
        evidenceIds: ["invented-hosted-id"],
        limitations: ["Hosted limitation text"],
      },
    );

    expect(result.replacedHostedProse).toBe(true);
    expect(result.answer.evidenceIds).toEqual(["current-glucose"]);
    expect(result.answer.limitations.join(" ")).not.toContain(
      "Hosted limitation text",
    );
  });
  ...
  it("leaves high-coverage model answers and card labels unchanged", () => {
    ...
    ).toEqual({ answer: original, replacedHostedProse: false });
  });
  ...
  it("validates the compact presentation saved with a conversation", () => {
    ...
    expect(isTarvisEvidencePresentation({ ...presentation, windows: [] })).toBe(
      false,
    );
  });
});
