...
describe("committed encrypted backup restore reporting", () => {
  it("shows only the safe restore code, not SQL, personal content or credentials", () => {
    const error = new BackupRestoreError('food_logs', new Error('FOREIGN KEY constraint failed: private meal and account@example.com'));
    const message = describeEncryptedBackupError(error, 'restore');
    expect(message).toContain('food_logs/missing-related-record');
    expect(message).not.toMatch(/private meal|account@example|FOREIGN KEY/);
  });
  const result = {
    attempted: 5,
    inserted: 3,
