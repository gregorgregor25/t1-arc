...
describe('Health Connect preference batching', () => {
  it('does not invalidate an in-flight import when retry saves unchanged choices', async () => {
    await saveHealthConnectPreferences(['steps', 'sleep']);
    const before = storedState(sqlite);
    database.resetMeasurements();
    await saveHealthConnectPreferences(['sleep', 'steps']);
    expect(storedState(sqlite)).toEqual(before);
    expect(database.runCalls).toHaveLength(0);
    await saveHealthConnectPreferences(['steps']);
    expect(sqlite.prepare('SELECT value FROM app_metadata WHERE key = ?').get(CONFIG_REVISION_KEY)).toEqual({ value: '9' });
  });
