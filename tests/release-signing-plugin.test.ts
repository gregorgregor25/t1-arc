...
describe('T1 Arc Android release signing policy', () => {
  ...
  it('refuses store bundles that are not production-signed', () => {
    ...
    expect(policy).toContain('/(?i)bundle.*release$/');
    expect(policy).toContain('/(?i)(publish|sign).*release.*bundle.*/');
    expect(policy).not.toContain("lowerName.contains('release')");
  ...
  it('requires an explicit switch for a private debug-signed APK', () => {
    ...
    expect(policy).toContain(
      '/(?i)(assemble|package|install|bundle).*release$/',
    );
    expect(policy).toContain('/(?i)(publish|sign).*release.*/');
    expect(policy).toContain(
