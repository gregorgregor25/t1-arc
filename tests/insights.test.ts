...
describe("evidence-backed insights", () => {
  it("compares two complete weeks and links every finding to records", async () => {
    ...
    for (const finding of report.findings) {
      ...
      expect(finding.evidence.every((item) => item.recordIds.length > 0)).toBe(
        true,
      );
    }

    const answer = answerInsightQuestion("Why was my glucose worse?", report);
    ...
    expect(doseAnswer.title).toBe("T1 Arc does not give treatment advice");
  ...
  it("normalises daily context and insulin by London calendar days across BST", async () => {
    ...
    const currentCarbs = current.context
      ...
      .reduce((total, event) => total + (event.carbsGrams ?? 0), 0);
  ...
  it("does not compare carbohydrate when either window has incomplete meal nutrition", async () => {
    const now = Date.parse("2026-07-26T10:00:00+01:00");
    const today = toDateKey(now);
    const todayStart = dayRange(today, now).start;
    const currentStart = dayRange(addDays(today, -7), now).start;
    ...
    const incompleteCurrent: TimelineData = {
      ...current,
      context: current.context.map((event, index) =>
        event.kind === "meal" &&
        index ===
          current.context.findIndex((candidate) => candidate.kind === "meal")
          ? { ...event, carbsGrams: undefined }
          : event,
      ),
    };

    const report = buildInsightReport(incompleteCurrent, previous, now);

    expect(report.current.mealCarbsPerDay).toBeNull();
    expect(report.previous.mealCarbsPerDay).not.toBeNull();
    expect(
      report.findings.some((finding) => finding.id === "food-context"),
    ).toBe(false);
  });

  it("does not compare a native meal carbohydrate subtotal as complete", async () => {
    const now = Date.parse("2026-07-26T10:00:00+01:00");
    const today = toDateKey(now);
    const todayStart = dayRange(today, now).start;
    const currentStart = dayRange(addDays(today, -7), now).start;
    ...
    const partialCurrent: TimelineData = {
      ...current,
      context: current.context.map((event) => {
        if (madePartial || event.kind !== "meal") return event;
        madePartial = true;
        return {
          ...event,
          nutritionDetail: "itemized" as const,
          items: [
            {
              id: "known-food",
              name: "Known food",
              amount: 100,
              unit: "g" as const,
              carbohydrateGrams: event.carbsGrams,
            },
            {
              id: "unknown-food",
              name: "Unknown food",
    ...
    const report = buildInsightReport(partialCurrent, previous, now);

    expect(madePartial).toBe(true);
    expect(report.current.mealCarbsPerDay).toBeNull();
    expect(
      report.findings.some((finding) => finding.id === "food-context"),
    ).toBe(false);
  ...
  it("measures London-clock glucose blocks by elapsed time across DST", () => {
    ...
    expect(parts.reduce((total, part) => total + part.expectedMinutes, 0)).toBe(
      23 * 60,
    );
  });
  ...
  it("withholds insulin comparisons when source totals materially disagree with detailed rows", async () => {
    ...
    expect(report.findings.some((item) => item.id === "insulin-change")).toBe(
      false,
    );
    expect(
  ...
  it("does not report a basal discrepancy when the source exports daily totals but no timed basal stream", async () => {
    const now = Date.parse("2026-07-26T10:00:00+01:00");
    const today = toDateKey(now);
    const todayStart = dayRange(today, now).start;
    const currentStartDate = addDays(today, -7);
    ...
    const firstCurrentDay = dayRange(currentStartDate, now);
    const firstPreviousDate = addDays(today, -14);
    const firstPreviousDay = dayRange(firstPreviousDate, now);
    const sourceDeviceId = "pump-with-daily-totals-only";
    const current = {
      ...loadedCurrent,
      basal: [],
      boluses: [
        {
          ...
          sourceDeviceId,
          timestamp: firstCurrentDay.start + 12 * 60 * 60_000,
          units: 10,
        },
      ],
      dailyInsulinTotals: [
        {
          ...
          sourceId: "glooko-export",
          sourceDeviceId,
          timestamp: firstCurrentDay.end - 1,
          dateKey: currentStartDate,
          basalUnits: 20,
          bolusUnits: 10,
          totalUnits: 30,
      ...
      boluses: [
        {
          ...
          sourceId: "glooko-export",
          sourceDeviceId,
          timestamp: firstPreviousDay.start + 12 * 60 * 60_000,
          units: 8,
        },
      ],
      dailyInsulinTotals: [
        {
          ...
          sourceId: "glooko-export",
          sourceDeviceId,
          timestamp: firstPreviousDay.end - 1,
          dateKey: firstPreviousDate,
          basalUnits: 18,
          bolusUnits: 8,
          totalUnits: 26,
        },
    ...
    expect(
      report.findings.some(
        (finding) => finding.id === "insulin-total-reconciliation",
      ),
    ).toBe(false);
    expect(
      report.findings.some(
  ...
  it("answers medication questions from exact recorded entries", async () => {
    ...
    expect(finding?.evidence[0]?.recordIds).toEqual(["medication-entry-1"]);
    ...
    expect(
      answerInsightQuestion("Should I take another tablet?", report).title,
  ...
  it("groups continuously observed qualifying readings into sustained runs", () => {
    ...
    const reading = (minute: number, mmolL: number): GlucoseReading => ({
      id: `reading-${minute}`,
  ...
  describe("15-minute glucose episode confirmation", () => {
    const start = Date.parse("2026-07-26T10:00:00+01:00");
    const readings = (
      points: [minute: number, mmolL: number][],
    ): GlucoseReading[] =>
    ...
    it("keeps a brief recovery inside one confirmed episode", () => {
      ...
      expect(episodes[0]?.readings.map((reading) => reading.timestamp)).toEqual(
        [0, 5, 10, 15, 20, 35, 40].map((minute) => start + minute * 60_000),
      );
    ...
    it("splits episodes only after a full 15-minute recovery", () => {
      ...
      expect(
        episodes.map(({ start: episodeStart, end }) => [
  ...
  it("keeps sustained high and low run evidence in separate references", () => {
    ...
    expect(
      evidence?.find((reference) => reference.id === "current-low-glucose-runs")
        ?.label,
    ).toBe("Recent sustained low runs");
    expect(
      evidence
        ?.find((reference) => reference.id === "current-high-glucose-runs")
  ...
  it("resolves every meal response record from the repository range", async () => {
    ...
    for (const response of responses) {
      ...
      expect(evidence.recordIds.filter((id) => !available.has(id))).toEqual([]);
    }
  });
  ...
  it("adds source-selected Health Connect activity and heart evidence", async () => {
    ...
    expect(heart?.evidence[1]?.recordIds).toEqual(["previous-resting-heart"]);
    expect(
  ...
  it("compares body composition, hydration and material vital-sign changes with exact evidence", async () => {
    ...
    expect(hydration?.evidence[1]?.recordIds).toEqual(["previous-hydration"]);
    expect(vitals?.summary).toContain("Systolic blood pressure was 130 mmHg");
    ...
    expect(
      answerInsightQuestion("How did my hydration change?", report).findingIds,
  ...
  it("withholds comparisons when personal history coverage is insufficient", () => {
    const emptyWindow = (start: number, end: number): TimelineData => ({
      range: { start, end },
      glucose: [],
      basal: [],
      boluses: [],
  ...
  it("reports material sensor gaps without interpreting them as physiology", () => {
    ...
    const report = buildInsightReport(current, makeWindow(previousStart), end);
    ...
    expect(
      answerInsightQuestion("Were there missing sensor gaps?", report).answer,
