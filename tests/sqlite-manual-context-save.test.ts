...
function createSchema(database: DatabaseSync) {
  database.exec(`
    CREATE TABLE context_notes (
      ...
      sensor_model_id TEXT,
      sensor_warmup_minutes INTEGER,
...
describe('atomic manual context save insight invalidation', () => {
  ...
  it('round-trips and edits structured lab results without changing their identity', async () => {
    const event = createManualContextEvent({ kind: 'lab-result', timestamp: 1750000000000, observation: { kind: 'lab-result', test: 'HbA1c', value: 48, unit: 'mmol/mol', laboratory: 'Clinic' } }, { id: 'lab', recordedAt: 1750000001000 });
    await store.saveManualContext(event, { insightInvalidation: 'mark-dirty' });
    const saved = (await store.getPersonalObservations('lab-result'))[0]!;
    expect(saved).toMatchObject({ id: 'lab', observation: { kind: 'lab-result', value: 48, unit: 'mmol/mol' } });
    expect(manualContextDraftFromEvent(saved)).toMatchObject({ kind: 'lab-result' });
    const corrected = reviseManualContextEvent(saved, { kind: 'lab-result', timestamp: saved.start, observation: { kind: 'lab-result', test: 'HbA1c', value: 47, unit: 'mmol/mol' } });
    await store.saveManualContext(corrected, { insightInvalidation: 'clear-saved-reports' });
    expect((await store.getPersonalObservations('lab-result'))[0]).toMatchObject({ id: 'lab', observation: { value: 47 } });
    expect(count(sqlite, 'context_notes')).toBe(1);
    expect(await store.getPersonalObservations('site-change')).toEqual([]);
  });

  it('keeps extra-injection identity across edits and deletes only the manual dose', async () => {
    const event = createManualInsulinDelivery({ timestamp: 1750000000000, units: 3, insulinType: 'rapid-acting', additionalToPump: true }, { id: 'extra' });
    await store.saveManualInsulin(event);
    expect((await store.getManualInsulinBounds()).count).toBe(1);
    await expect(store.deleteManualInsulin('extra')).resolves.toBe(true);
    expect((await store.getManualInsulinBounds()).count).toBe(0);
  });
  ...
  it('remembers only saved sensor choices for the matching source, including a backdated choice', async () => {
    const event = note({ sourceId: 't1arc-manual', category: 'sensor', sensorStarted: true,
      sensorGlucoseSourceId: 'live-cgm', sensorModelId: 'libre-2', sensorWarmupMinutes: 60,
      start: 1_000, end: 3_601_000, recordedAt: 9_000_000 });
    await store.saveManualContext(event);
    expect(await store.getLastSensorSelection('other-cgm')).toBeUndefined();
    expect(await store.getLastSensorSelection('live-cgm')).toMatchObject({ sensorModelId: 'libre-2', sensorWarmupMinutes: 60, end: 3_601_000 });
    await store.saveManualContext({ ...event, id: 'new-choice', sensorModelId: 'other', sensorWarmupMinutes: undefined, end: undefined, recordedAt: 9_000_001 });
    expect(await store.getLastSensorSelection('live-cgm')).toMatchObject({ sensorModelId: 'other', sensorWarmupMinutes: undefined });
    await store.deleteManualContext('new-choice');
    expect((await store.getLastSensorSelection('live-cgm'))?.sensorModelId).toBe('libre-2');
  });
