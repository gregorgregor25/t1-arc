...
describe('clock-window accessible summary', () => {
  ...
  it('names the selected regional clock when explaining a transition', () => {
    const summary = buildEvidenceClockWindowAccessibilitySummary({
      aggregatePoints: [],
      coverageSummary: '1 occurrence contains readings',
      domain: { startMinute: 0, endMinuteUnwrapped: 240 },
      ...
      windows: [
        {
          clockTransitions: [
            {
              affectedEndMinute: 180,
              affectedStartMinute: 120,
              atTimestamp: Date.parse('2026-03-08T07:00:00Z'),
              changeMinutes: 60,
              kind: 'gap',
              utcOffsetAfterMinutes: -240,
              utcOffsetBeforeMinutes: -300,
            },
          ],
          id: 'spring-forward',
          label: 'Sunday 8 March',
          points: [],
          status: 'missing',
        },
      ],
    ...
    expect(summary).toContain('America/New_York clock gap');
    expect(summary).not.toContain('London');
  });
