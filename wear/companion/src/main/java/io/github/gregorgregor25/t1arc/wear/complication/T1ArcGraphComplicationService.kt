...
import io.github.gregorgregor25.t1arc.wear.data.GlucoseChartWindow
...
open class T1ArcGraphComplicationService : SuspendingComplicationDataSourceService() {
    ...
    override suspend fun onComplicationRequest(request: ComplicationRequest): ComplicationData {
        ...
        val repository = T1ArcWearRepository.get(this)
        return graphData(
            style = graphStyle,
    ...
    private fun graphData(
        ...
        val visibleCount = if (style == GraphStyle.ORBIT) pointsForWindow(history, 3).size
            else GlucoseChartWindow(System.currentTimeMillis()).points(history).size
        val description =
            PlainComplicationText
                .Builder(
                    if (visibleCount == 0) {
                        "No glucose readings in this three hour window"
                    } else {
                        "Three hour glucose history with ${GlucoseSemantics.formattedInteger(visibleCount.toLong(), snapshot)} readings"
                    },
                )
                .build()
    ...
    private fun renderMeridian(
        ...
        val window = GlucoseChartWindow(System.currentTimeMillis())
        val points = window.points(history)
        ...
        chartTicks(bounds, 4).forEach { value ->
            ...
            drawText(canvas, formatScale(value, snapshot), 22f, y + 4f, 12f, 0xFF9DB6BC.toInt(), Paint.Align.RIGHT)
        }
        ...
        if (points.isNotEmpty()) {
            ...
            drawColouredLinearTrace(canvas, points, window, chart, bounds, paint)
            drawEndpoint(
                canvas,
                chartX(last.timestampMs, window, chart),
        ...
        val now = window.endMs
        val locale = snapshotLocale(snapshot)
        val zone =
            runCatching { ZoneId.of(snapshot?.timeZone ?: ZoneId.systemDefault().id) }
                .getOrDefault(ZoneId.systemDefault())
        val time = DateTimeFormatter.ofLocalizedTime(FormatStyle.SHORT).withLocale(locale).withZone(zone)
        drawText(canvas, time.format(Instant.ofEpochMilli(now - 3 * 60 * 60_000L)), chart.left, 98f, 12f, 0xFF9DB6BC.toInt(), Paint.Align.LEFT)
        drawText(canvas, time.format(Instant.ofEpochMilli(now - 2 * 60 * 60_000L)), chart.left + chart.width() / 3f, 98f, 12f, 0xFF9DB6BC.toInt())
        drawText(canvas, time.format(Instant.ofEpochMilli(now - 60 * 60_000L)), chart.left + chart.width() * 2f / 3f, 98f, 12f, 0xFF9DB6BC.toInt())
        drawText(canvas, time.format(Instant.ofEpochMilli(now)), chart.right, 98f, 12f, 0xFF9DB6BC.toInt(), Paint.Align.RIGHT)
        return bitmap
    }
    ...
    private fun renderChronograph(
        ...
        drawText(
            canvas,
            "${GlucoseSemantics.formattedInteger(3L, snapshot)} HOURS",
            94f,
            29f,
            12f,
            cyan,
        ...
        chartTicks(bounds, 3).forEach { value ->
            val y = chartY(value, chart, bounds.first, bounds.second)
            canvas.drawLine(chart.left, y, chart.right, y, paint)
            drawText(canvas, formatScale(value, snapshot), chart.right - 3f, y - 3f, 10f, 0xFFCFD2D2.toInt(), Paint.Align.RIGHT)
        }
        paint.pathEffect = null
        ...
        drawText(
            canvas,
            "${GlucoseSemantics.formattedInteger(-3L, snapshot)}h",
            chart.left,
            137f,
            10f,
            0xFFCFD2D2.toInt(),
        ...
        drawText(
            canvas,
            "${GlucoseSemantics.formattedInteger(-2L, snapshot)}h",
            chart.left + chart.width() / 3f,
            137f,
            10f,
            0xFFCFD2D2.toInt(),
        )
        drawText(
            canvas,
            "${GlucoseSemantics.formattedInteger(-1L, snapshot)}h",
            chart.left + chart.width() * 2f / 3f,
            137f,
            10f,
            0xFFCFD2D2.toInt(),
        )
    ...
    private fun renderOrbit(
        ...
        drawScaleLabel(canvas, formatScale(orbitTicks[3], snapshot), 88f, 92f)
        drawScaleLabel(canvas, formatScale(orbitTicks[2], snapshot), 43f, 181f)
        drawScaleLabel(canvas, formatScale(orbitTicks[1], snapshot), 44f, 286f)
        drawScaleLabel(canvas, formatScale(orbitTicks[0], snapshot), 95f, 379f)
        return bitmap
    }
    ...
    private fun formatScale(
        value: Double,
        snapshot: GlucoseSnapshot?,
    ): String =
        snapshot?.let { GlucoseSemantics.formattedMmolL(value, it) }
            ?: if (kotlin.math.abs(value - kotlin.math.round(value)) < 0.05) {
                GlucoseSemantics.formattedInteger(kotlin.math.round(value).toLong(), snapshot)
            } else {
                String.format(Locale.getDefault(), "%.1f", value)
            }

    private fun snapshotLocale(snapshot: GlucoseSnapshot?): Locale =
        GlucoseSemantics.displayLocale(snapshot)

    private fun drawColouredLinearTrace(
        canvas: Canvas,
        points: List<GlucoseHistoryPoint>,
        window: GlucoseChartWindow,
        chart: RectF,
        bounds: Pair<Double, Double>,
        paint: Paint,
    ) {
        points.zipWithNext().forEach { (start, end) ->
            if (!window.connected(start, end)) return@forEach
            val startX = chartX(start.timestampMs, window, chart)
            val endX = chartX(end.timestampMs, window, chart)
    ...
    private fun chartX(
        ...
        return chart.left + window.fraction(timestampMs) * chart.width()
    }
