...
class T1ArcGraphView(context: Context) : View(context) {
    ...
    private fun drawChart(canvas: Canvas, tone: Int) {
        val window = GlucoseChartWindow(System.currentTimeMillis(), windowHours)
        ...
        fun x(timestamp: Long) =
            chart.left + window.fraction(timestamp) * chart.width()
        fun y(value: Double) =
        ...
        if (points.isNotEmpty()) {
            ...
            points.zipWithNext().forEach { (previous, point) ->
                if (window.connected(previous, point)) {
    ...
    private fun updateDescription() {
        val visibleCount = GlucoseChartWindow(System.currentTimeMillis(), windowHours).points(history).size
        contentDescription =
            if (snapshot == null) {
                "T1 Arc glucose graph. No current reading."
            } else {
                val freshness = GlucoseSemantics.freshness(snapshot)
                "${GlucoseSemantics.statusTitle(snapshot, freshness)} glucose, " +
                    ...
                    "${GlucoseSemantics.formattedInteger(visibleCount.toLong(), snapshot)} readings. Tap to change time range."
            }
    }
}
