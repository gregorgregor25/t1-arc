...
class WatchFaceSmokeInstrumentation : Instrumentation() {
    ...
    private fun renderGallery(runtime: WatchFaceRuntime, report: StringBuilder) {
        ...
        val onlyCase = options.getString("onlyCase")
        ...
        val cases = listOf(
            ...
            .filter { onlyCase == null || it[0] == onlyCase }
        check(cases.isNotEmpty()) { "No matching gallery case." }
        fun capture(name: String) {
            check(shell("dumpsys activity activities").lineSequence().none {
                it.contains("topResumedActivity") && it.contains("ChargingComposeActivity")
            }) { "The emulator charging screen covers the watch face; reject this capture." }
